defmodule Discord.SourceSetup do
  ...
  def cast_source(payload, _opts) when is_map(payload) do
    ...
    oauth2 = payload_map(source, "oauth2")

    with {:ok, id} <- required_string(source, "id"),
         {:ok, application_id} <- required_string(source, "application_id"),
         {:ok, bot_token} <- secret_or_existing(id, source, "bot_token"),
         {:ok, client_secret} <- optional_secret_or_existing(id, source, "client_secret") do
      attrs =
        %{
          "id" => id,
  ...
  def reconcile_sources, do: Discord.SourceSupervisor.reconcile_sources()
  ...
  defp public_source(source, runtime) do
    ...
    |> Map.put("client_secret", secret_status(source["client_secret"]))
    |> Map.put("runtime", runtime_for_source(runtime, source["id"]))
  end

  defp runtime_status do
    case Discord.SourceSupervisor.runtime_status() do
      {:ok, status} -> status
  ...
  defp optional_secret_or_existing(source_id, map, key) do
    case string_field(map, key, nil) do
      value when is_binary(value) -> {:ok, value}
      nil -> {:ok, existing_source_secret(source_id, key)}
    end
  end
