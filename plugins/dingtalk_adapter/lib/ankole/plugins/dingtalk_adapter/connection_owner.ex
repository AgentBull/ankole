defmodule Ankole.Plugins.DingTalkAdapter.ConnectionOwner do
  @moduledoc """
  Per-app owner for the DingTalk Stream long-connection client.

  Builds an immutable dispatcher from all active consumers and starts one
  `DingTalkOpenAPI.Stream.Client`. A changed consumer set restarts the owner so
  the dispatcher is rebuilt rather than mutated on a live socket.
  """
  ...
  def handle_info({:EXIT, pid, reason}, %{ws_pid: pid} = state) do
    Logging.error(
      "dingtalk_adapter.connection_owner.client_exited",
      "dingtalk adapter Stream client exited",
