defmodule Ankole.Plugins.EmailAdapter.ConnectionReconciler do
  @moduledoc false

  alias Ankole.Logging
  alias Ankole.Plugins.ConnectionLifecycle
  alias Ankole.Plugins.EmailAdapter.{Config, ConnectionSupervisor, Inbound}
  ...
  def reconcile_once(opts \\ []) do
    bindings = SignalsGateway.list_enabled_bindings("email", Keyword.take(opts, [:repo]))
  ...
  defp run_reconcile(opts) do
    ...
    if result.errors != [] do
      Logging.warning(
        "email_adapter.connection_reconciliation.completed_with_errors",
        "email connection reconciliation completed with errors",
