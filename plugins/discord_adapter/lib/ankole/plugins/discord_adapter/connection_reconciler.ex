defmodule Ankole.Plugins.DiscordAdapter.ConnectionReconciler do
  @moduledoc false

  alias Ankole.Logging
  alias Ankole.Plugins.ConnectionLifecycle
  alias Ankole.Plugins.DiscordAdapter.{Config, ConnectionSupervisor, Inbound}
  ...
  def reconcile_once(opts \\ []) do
    bindings = SignalsGateway.list_enabled_bindings("discord", Keyword.take(opts, [:repo]))
  ...
  defp run_reconcile(opts) do
    ...
    if result.errors != [] do
      Logging.warning(
        "discord_adapter.connection_reconciliation.completed_with_errors",
        "Discord connection reconciliation completed with errors",
