defmodule BullxTelegram.SourceSupervisor do
  ...
  def init(_opts) do
    children =
      [
        ...
        | Enum.map(
            BullxTelegram.Source.enabled_sources!(),
            &BullxTelegram.SourceRuntime.child_spec/1
          )
      ]
  ...
  def runtime_status do
    with {:ok, sources} <- BullxTelegram.Source.enabled_sources(),
      ...
      {:ok,
       %{
         ready?:
           Enum.all?(
             sources,
             &MapSet.member?(ready_child_ids, BullxTelegram.SourceRuntime.child_id(&1))
           ),
         sources:
           Enum.map(sources, fn source ->
             %{
               ...
               enabled: true,
               ready:
                 MapSet.member?(ready_child_ids, BullxTelegram.SourceRuntime.child_id(source)),
               transport: if(source.start_transport?, do: "polling", else: "disabled")
             }
           end)
       }}
    end
  ...
  def reconcile_sources do
    with pid when is_pid(pid) <- Process.whereis(__MODULE__),
         {:ok, sources} <- BullxTelegram.Source.enabled_sources(),
  ...
  defp stop_retired_children(pid, sources) do
    ...
    wanted_child_ids = MapSet.new(sources, &BullxTelegram.SourceRuntime.child_id/1)
  ...
  defp start_missing_children(pid, sources) do
    ...
    Enum.reduce_while(sources, :ok, fn source, :ok ->
      case MapSet.member?(existing_child_ids, BullxTelegram.SourceRuntime.child_id(source)) do
        true ->
          {:cont, :ok}

        false ->
          case Supervisor.start_child(pid, BullxTelegram.SourceRuntime.child_spec(source)) do
            {:ok, _pid} -> {:cont, :ok}
  ...
  defp source_id_from_child_id({BullxTelegram.SourceRuntime, id, _fingerprint}), do: id
  defp source_id_from_child_id({BullxTelegram.SourceRuntime, id}), do: id
