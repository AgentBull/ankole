defmodule BullxTelegram.ChannelAdapter do
  @moduledoc """
  EventBus channel adapter for trusted Telegram bot sources.

  The adapter normalizes Telegram updates into decoded CloudEvents and performs
  only transport admission plus Principal identity evidence handling before
  EventBus acceptance.
  """
  ...
  def deliver(source_config, reply_channel, outbound, opts) do
    BullxTelegram.Outbound.deliver(source_config, reply_channel, outbound, opts)
  end

  @impl BullX.EventBus.ChannelAdapter
  def consume_stream(source_config, reply_channel, stream_id, opts) do
    BullxTelegram.Streamer.consume(source_config, reply_channel, stream_id, opts)
  end
  ...
  defp handle_mapped({:ok, mapped}, %Source{} = source) do
    case BullX.Principals.match_or_create_human_from_channel(mapped.account_input) do
      ...
      {:error, :principal_disabled} ->
        maybe_reply(mapped.attrs, source, BullX.I18n.t("eventbus.telegram.auth.denied"))
        :ignore

      {:error, error} ->
        {:error, BullxTelegram.Error.map(error)}
    end
  end
  ...
  defp maybe_reply_activation_required(attrs, %Source{} = source) do
    case get_in(attrs, [:data, :routing_facts, "chat_type"]) do
      "private" ->
        maybe_reply(attrs, source, BullX.I18n.t("eventbus.telegram.auth.activation_required"))

      _chat_type ->
        maybe_reply(attrs, source, BullX.I18n.t("eventbus.telegram.auth.direct_command_dm_only"))
    end
  end
