defmodule Ankole.Plugins.TelegramAdapter.Config do
  @moduledoc "Validation and runtime helpers for the first-party Telegram adapter."
  ...
  @key_pattern ~r/\Asignals_gateway\.telegram\.bindings\.[A-Za-z0-9_.:-]+\z/
  ...
  def app_config_patterns do
    [
      AppConfigure.define_pattern(
        id: "signals_gateway.telegram.bindings.*",
        key_pattern: @key_pattern,
        encrypted: true,
        schema: Schema.new(&validate_binding_config/1),
        description: "Encrypted Telegram bot binding configuration."
      )
    ]
  end
  ...
  def binding_config_key(agent_uid, binding_name)
    ...
    "signals_gateway.telegram.bindings.#{id}"
  end
  ...
  def validate_binding_config(_value), do: {:error, :invalid_telegram_binding_config}
  ...
  def validate_binding_assignment(repo, agent_uid, binding_name, config)
    ...
    |> where(
      [binding],
      binding.adapter == "telegram" and binding.enabled == true and
      ...
      case load_config_ref_in_tx(repo, binding.config_ref) do
        {:ok, other} ->
          if secret_fingerprint(other) == fingerprint do
            {:halt,
             {:error, {:telegram_bot_token_already_bound, binding.agent_uid, binding.name}}}
          else
            {:cont, :ok}
          end
        ...
        :error ->
          {:halt,
           {:error, {:telegram_binding_config_unavailable, binding.agent_uid, binding.name}}}

        {:error, reason} ->
          {:halt,
           {:error,
            {:telegram_binding_config_unavailable, binding.agent_uid, binding.name, reason}}}
      end
    end)
  end
  ...
  defp bot_token(%Runtime{bot_token: token}), do: token
  defp bot_token(config) when is_map(config), do: Map.fetch!(config, "botToken")
end

defimpl Inspect, for: Ankole.Plugins.TelegramAdapter.Config.Runtime do
  import Inspect.Algebra

  def inspect(_config, _opts) do
    concat(["#Ankole.Plugins.TelegramAdapter.Config.Runtime<", "[REDACTED]", ">"])
  end
end
