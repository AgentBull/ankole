defmodule Ankole.Plugins.TelegramAdapter.ActionToken do
  @moduledoc false
  ...
  def encode(actor_event_id, index, action)
    ...
    if byte_size(token) <= 64, do: {:ok, token}, else: {:error, :callback_token_too_long}
  end

  def encode(_actor_event_id, _index, _action), do: {:error, :invalid_callback_action}

  @spec resolve(String.t(), String.t(), String.t(), String.t()) ::
          {:ok, map()} | {:error, term()}
  def resolve(token, agent_uid, binding_name, source_entry_id)
    ...
    with {:ok, actor_event_id, index, fingerprint} <- decode(token),
         ...
         true <-
           action_fingerprint(action) == fingerprint || {:error, :invalid_callback_action},
