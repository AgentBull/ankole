defmodule Feishu.Config.Credentials do
  ...
  defp normalize_profile({id, %{} = profile}) when is_binary(id) and id != "" do
    with {:ok, app_id} <- required_string(profile, "app_id"),
         {:ok, app_secret} <- required_string(profile, "app_secret") do
      {:ok, {id, %{"app_id" => app_id, "app_secret" => app_secret}}}
    end
  end
...
defmodule Feishu.Config do
  ...
  Credentials are encrypted as one profile map and referenced from Gateway
  source config by `credential_id`; source config never stores app secrets.
  """
