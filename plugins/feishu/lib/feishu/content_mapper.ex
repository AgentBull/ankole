defmodule Feishu.ContentMapper do
  ...
  @spec render_outbound(term(), Feishu.Source.t() | nil) :: {:ok, map(), [String.t()]} | {:error, map()}
  ...
  def render_outbound(%{"kind" => "card", "body" => %{"format" => format, "payload" => payload}}, _source)
  ...
  defp outbound_file_data(%{"url" => "file://" <> path} = body, kind) do
    read_path(path, body, kind)
  end

  defp outbound_file_data(%{"url" => "/" <> _rest = path} = body, kind) do
    read_path(path, body, kind)
  end
  ...
  defp rendered_media("image", key), do: %{msg_type: "image", content: Jason.encode!(%{image_key: key})}
  ...
  defp outbound_filename(body, kind) do
    Map.get(body, "filename") || Map.get(body, "file_name") || "bullx-#{kind}.#{default_ext(kind)}"
  end
  ...
  defp fallback_text(_type), do: BullX.I18n.t("gateway.feishu.errors.unsupported_message")
