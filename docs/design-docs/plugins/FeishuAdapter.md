...
- plugin placement, extension declarations, and plugin-owned source
  configuration;
- Feishu/Lark WebSocket event push, source listener supervision, connectivity
  checks, inbound normalization, direct
...
| `Feishu.Config` | Plugin-owned `BullX.Config` declarations and source casters. |
| `Feishu.Source` | Runtime source normalization, redacted public projection, and connectivity checks. |
...
| `Feishu.EventMapper` | Feishu WebSocket event normalization into CloudEvents. |
...
| --- | --- | --- | --- |
| `eventbus_sources!/0` | `bullx.plugins.feishu.eventbus_sources` | yes | `[]` |
| `oidc_state_ttl_seconds!/0` | `bullx.plugins.feishu.oidc_state_ttl_seconds` | no | `600` |

`eventbus_sources` is a JSON array of Feishu source entries. It is encrypted by
`BullX.Config` because each source is one configured channel instance and stores
its own Feishu/Lark app credential directly:
...
Principal login provider id. The OIDC redirect URI is BullX-owned and derived
from the current web origin and source id; setup shows it for copying into the
Feishu app console but does not persist it in source config. The source id is
not a Feishu chat id, tenant id, user id, bot
id, shared credential id, OAuth client id, or display label.

`domain` stays in source configuration because it describes the deployment
target for one source. Multiple Feishu or Lark apps are modeled as multiple
sources with the same adapter id and distinct source ids.
...
writing Principal data. It constructs a `FeishuOpenAPI` client from the source
app credential, fetches or validates a tenant access token, and returns only
...
| `capabilities/0` | Declares Feishu WebSocket inbound, send, edit, recall, stream, supported content kinds, and OIDC support. |
...
dispatcher, source-local startup logs, and source-local cache key prefixes.
...
implementation uses Feishu WebSocket mode only. It does not expose Feishu
webhook-mode surfaces, so webhook-mode secrets are not plugin configuration
fields. Provider action Events that arrive through WebSocket are normalized
through the same adapter path as message Events.
...
Trusted profile fields may include `uid`, `display_name`, `avatar_url`, `email`,
`phone`, `open_id`, `union_id`, and `user_id`. Feishu maps contact `user_id` to
the Principal `uid` candidate and maps contact `enterprise_email` to `email`
before falling back to contact `email`. The adapter lowercases and trims email.
It normalizes phone candidates to E.164 before passing them to `BullX.Principals`;
malformed or ambiguous phone values are omitted.
...
| Interactive card action | `bullx.action.submitted` | CloudEvent id is derived from stable WebSocket event fields such as message id, action id, action tag, and actor open id. Sanitized action values stay in `data.content` as structured Event facts. Routing facts include only stable non-private action identifiers. |
...
`/preauth <code>` resolves the Feishu sender through
`contact/v3/users/:user_id` with `user_id_type = open_id` before consuming the
activation code. If the contact profile cannot be fetched, the adapter fails
closed and leaves the activation code unused; it must not create a human
Principal whose display name falls back to a provider id such as
`feishu:ou_xxx`.
...
3. The operator creates Event Routing Rules for Feishu message Events, and may
   ...
   scope fields for the Feishu chat, thread, or action surface.
...
4. Resolve the OAuth subject id from the token response when it contains
   `open_id` or `user_id`; if the token response lacks those fields, use the
   user access token only to resolve the subject id.
5. Fetch the canonical profile through `contact/v3/users/:user_id` with the
   resolved `user_id_type` and the source's tenant access token.
6. Normalize a Principal login subject from the contact profile.
7. Keep the returned user tokens only in the SDK's process-local user-token
   cache, keyed by the Feishu `open_id`; durable Principal identity and display
   data come from the contact profile, not from persisted provider tokens.
8. Pass the login subject to `BullX.Principals` for matching and session
   establishment.
...
If the OAuth subject id or contact profile cannot be resolved, login fails
closed without creating or binding a Principal. `Feishu.OIDCProvider` must not
write `principal_external_identities` directly. On `{:error, :not_bound}`, the
Web surface should direct the user to activate from Feishu with
`/preauth <code>`. On `{:error, :principal_disabled}` or `{:error, :not_human}`,
it fails closed.
...
| Invalid source config or missing source secret | `config` |
...
`FeishuOpenAPI.WS.Client` authentication and SDK frame decoding. The first
implementation does not expose Feishu webhook-mode surfaces.
...
- never persist Feishu user tokens; any reusable user-token state is
  process-local SDK cache data, while Principal profile fields are fetched from
  Feishu contact APIs with the source's tenant token;
...
Provider verification failures, malformed provider occurrences, missing required
Feishu fields, missing source secrets, and unsupported content fail closed. They
...
2. Add Feishu plugin configuration.
   - Owns: `Feishu.Config`, source casters, redaction helpers, and secret-key
     tests.
   - Depends on: Task 1.
   - Acceptance: `bullx.plugins.feishu.eventbus_sources` is secret,
     `eventbus_sources` validates enabled sources, source ids are stable, and
     public projections never reveal app secrets.
...
5. Implement Feishu source connectivity and adapter capabilities.
   ...
   - Depends on: Task 2.
   - Acceptance: capabilities are precise, connectivity checks source secrets
...
6. Implement inbound runtime and normalization.
   - Owns: `Feishu.SourceSupervisor`, `Feishu.Channel`,
     `Feishu.EventMapper`, and `Feishu.ContentMapper`.
   - Depends on: Task 5.
...
- The plugin registers the Feishu `:"bullx.principals.login_provider"`
  ...
  provider ids.
- Feishu source config and source secrets validate through `BullX.Config`.
