...
function ButtonGroupText({ className, render, ...props }: useRender.ComponentProps<"div">) {
  return useRender({
    ...
    props: mergeProps<"div">(
      {
        className: cn(
          "flex items-center gap-2 border border-transparent border-b-input bg-transparent px-2.5 text-xs font-normal tracking-normal normal-case group-has-[>[data-variant=outline]]/button-group:border-border [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-3.5",
          className,
        ),
      },
      props,
