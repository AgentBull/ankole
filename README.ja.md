...
| --- | --- | --- | --- |
| Primary unit | Agentic Loop または assistant session。 | App、bot、RPA flow、workflow run。 | long-term mission、responsibility、Work、EventBus-routed context を持つ Agent。 |
...
| External events | channel、cron、webhook、integration が assistant loop に入る。 | trigger が predefined app または workflow を起動する。 | Event が EventBus に入り、Event Routing Rule に match し、Target を呼び出し、business record を通じて long-running Work を更新する。 |
...
**One event can reach the right work path.** 顧客の budget freeze message は EventBus に入り、Event Routing Rule に match し、Target に届きます。その Target は case を直接処理する AIAgent でも、branching、approval、parallelism、deterministic step を明示する Workflow でもかまいません。
...
BullX は今、Elixir/OTP、PostgreSQL、Phoenix/Inertia の foundation の上で EventBus と AIAgent Target を end-to-end で動かしており、Discord、Feishu (Lark)、Telegram の channel adapter を備えています。Brain、Budget、Work/Task の durable persistence、Workflow Target、trajectory-driven self-evolution は引き続き構築中です。
