...
BullX は今、Elixir/OTP、PostgreSQL、Phoenix/Inertia の foundation の上で IMGateway、MailBox、AIAgent receiver を end-to-end で動かしており、Discord、Feishu (Lark)、Telegram の channel adapter を備えています。Feishu IM messages は normalize され、`im_messages` として保存され、MailBox 経由で routed され、AIAgent に処理され、outbound `im_messages` として返信できます。Brain、Budget、durable Work/Task records、Workflow receiver、trajectory-driven self-evolution は引き続き構築中です。
