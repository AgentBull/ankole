defmodule BullXAIAgent.Reasoning.AgenticLoop.Runner do
  @moduledoc """
  Task-based AgenticLoop runner.

  Produces a lazy event stream via `Stream.resource/3`; runtime state remains
  process-resident for the life of the stream.
  """

  alias BullXAIAgent.PendingInputServer
  ...
  @doc """
  Starts a new AgenticLoop coordinator task and returns a lazy event stream.
  """
  ...
  @doc """
  Continues a AgenticLoop run from an existing runtime state.
  """
    ...
    case start_task(
      ...
      {:error, reason} ->
        Stream.map([reason], fn error ->
          raise "Failed to start AgenticLoop runner task: #{inspect(error)}"
        end)
    end
  end
  ...
  defp coordinator(owner, ref, state, config, opts, stream_opts) do
    ...
    try do
      ...
      {:cancelled, %State{} = current_state, reason} ->
        ...
        send(owner, {:agentic_loop_runner, ref, :done})
  ...
  defp build_turn_request(%State{} = state, %Config{} = config, runtime_context) do
    base_request = %{
      ...
      llm_opts:
        config |> Config.llm_opts() |> maybe_put_previous_response_id(state.llm_response_id),
      tools: config.tools
    }
  ...
  defp maybe_transform_request(
         request,
         _state,
         %Config{request_transformer: nil},
         _runtime_context
       ),
       do: {:ok, request}
  ...
  defp maybe_transform_request(request, %State{} = state, %Config{} = config, runtime_context) do
    ...
    e ->
      {:error,
       {:request_transformer_exception, %{error: Exception.message(e), type: e.__struct__}}}
  end

  defp apply_request_overrides(request, overrides, %Config{} = config)
       when is_map(request) and is_map(overrides) do
    request
    |> maybe_put_request_field(
      :messages,
      Map.get(overrides, :messages, Map.get(overrides, "messages"))
    )
    |> maybe_put_request_field(:tools, Map.get(overrides, :tools, Map.get(overrides, "tools")))
    |> maybe_merge_request_llm_opts(
      config,
      Map.get(overrides, :llm_opts, Map.get(overrides, "llm_opts"))
    )
  end
  ...
  defp normalize_request_messages(%{messages: messages}) when is_list(messages),
    do: {:ok, messages}
  ...
  defp sync_tools_in_llm_opts(%Config{} = config, llm_opts, tools)
       when is_list(llm_opts) and is_map(tools) do
  ...
  defp request_turn_stream(%State{} = state, owner, ref, %Config{} = config, messages, llm_opts) do
    case BullXAIAgent.stream_text(messages, model: config.model, opts: llm_opts) do
      {:ok, stream_response} ->
  ...
  defp request_turn_generate(%State{} = state, %Config{} = config, messages, llm_opts) do
    case BullXAIAgent.generate_text(messages, model: config.model, opts: llm_opts) do
      {:ok, response} ->
  ...
  defp consume_stream(%State{} = state, owner, ref, %Config{} = config, stream_response) do
    ...
    case ReqLLM.StreamResponse.process_stream(
      ...
      {:ok, response} ->
        {:ok, current_stream_state(state_key, state),
         Turn.from_response(response, model: BullXAIAgent.model_label(config.model)),
    ...
    e ->
      {:error, current_stream_state(stream_state_key(ref), state),
       %{error: Exception.message(e), type: e.__struct__}}
  after
  ...
  defp consume_generate(%State{} = state, %Config{} = config, response) do
    turn = Turn.from_response(response, model: BullXAIAgent.model_label(config.model))
  ...
  defp maybe_put_previous_response_id(llm_opts, response_id)
       when is_list(llm_opts) and is_binary(response_id) do
    provider_options =
      llm_opts |> Keyword.get(:provider_options, []) |> normalize_provider_options()

    Keyword.put(
      llm_opts,
      :provider_options,
      Keyword.put(provider_options, :previous_response_id, response_id)
    )
  end

  defp maybe_put_previous_response_id(llm_opts, _response_id), do: llm_opts

  defp normalize_provider_options(options) when is_list(options),
    do: Enum.reject(options, fn {k, _v} -> is_nil(k) end)
  ...
  defp evolve_context_state_snapshot(context, results)
       when is_map(context) and is_list(results) do
  ...
  defp build_runtime_context(context, %State{} = state, %Config{} = config)
  ...
  defp execute_tool_with_retries(%PendingToolCall{} = pending_call, %Config{} = config, context) do
    ...
    case is_atom(module) and function_exported?(module, :name, 0) and
      ...
      _ ->
        {pending_call,
         {:error, %{type: :unknown_tool, message: "Tool '#{pending_call.name}' not found"}, []},
         1, 0}
    end
  end
  ...
  defp emit_event(%State{} = state, owner, ref, kind, data, extra \\ %{}) do
    ...
    send(owner, {:agentic_loop_runner, ref, :event, event})
    {state, event}
  end
  ...
  defp next_event(_owner, %{done?: false, down?: true, ref: ref} = state) do
    receive do
      {:agentic_loop_stream_cancel, _reason} ->
        next_event(nil, state)

      {:agentic_loop_runner, ^ref, :stream_control, control} ->
        next_event(nil, apply_stream_control(state, control))

      {:agentic_loop_runner, ^ref, :progress} ->
        next_event(nil, state)

      {:agentic_loop_runner, ^ref, :event, event} ->
        {[event], state}

      {:agentic_loop_runner, ^ref, :done} ->
  ...
  defp next_event(_owner, %{ref: ref} = state) do
    ...
    receive do
      {:agentic_loop_stream_cancel, reason} ->
  ...
  defp notify_progress(owner, ref) do
    send(owner, {:agentic_loop_runner, ref, :progress})
    :ok
  end

  defp cleanup(_owner, %{pid: pid, monitor_ref: monitor_ref} = state) when is_pid(pid) do
  ...
  defp append_query(%State{} = state, query) when is_binary(query) do
    %{
      state
      | context: AIContext.append_user(state.context, query),
        status: :running,
  ...
  defp maybe_note_owner_signal(true, _owner, _ref, _last_owner_signal_ms, _interval_ms),
    do: monotonic_ms()
  ...
  defp visible_chunk?(%ReqLLM.StreamChunk{type: :content, text: text}, trace_cfg),
    do: delta_captured?(text, trace_cfg)

  defp visible_chunk?(%ReqLLM.StreamChunk{type: :tool_call}, trace_cfg),
    do: trace_cfg[:capture_deltas?] == true

  defp visible_chunk?(_chunk, _trace_cfg), do: false

  defp delta_captured?(text, trace_cfg),
    do: trace_cfg[:capture_deltas?] == true and is_binary(text) and text != ""
  ...
  defp emit_stream_delta(_state_key, _owner, _ref, _chunk_type, text) when text in [nil, ""],
    do: :ok
  ...
  defp maybe_redact_args(arguments, %Config{} = config) do
    case config.observability[:redact_tool_args?] do
      true -> BullXAIAgent.Observe.sanitize_sensitive(arguments)
      _ -> arguments
    end
  end
  ...
  defp check_cancel!(%State{} = state, ref) do
    receive do
      {:agentic_loop_cancel, ^ref, reason} -> throw({:cancelled, state, reason})
    after
      0 -> :ok
    end
  end
  ...
  # finish reason so we do not emit a phantom assistant turn.
  ...
  defp invalid_blank_terminal_finish_reason?(reason) when reason in [nil, :stop, :tool_calls],
    do: false
  ...
  defp seal_pending_input_server(%Config{pending_input_server: server}),
    do: PendingInputServer.seal(server)
  ...
  defp request_stream_cancel(%{pid: pid, ref: ref} = state, reason) when is_pid(pid) do
    ...
    case Process.alive?(pid) do
      true -> send(pid, {:agentic_loop_cancel, ref, reason})
      _ -> :ok
    end
  ...
  defp announce_stream_control(owner, ref, stream_response) do
    case stream_cancel_fun(stream_response) do
      cancel_fun when is_function(cancel_fun, 0) ->
        send(owner, {:agentic_loop_runner, ref, :stream_control, %{cancel: cancel_fun}})
  ...
  defp maybe_capture_stream_control(%{stream_cancel: cancel_fun} = state)
       when is_function(cancel_fun, 0), do: state
  ...
  defp invoke_stream_cancel(%{stream_cancel: cancel_fun} = state)
    ...
    :exit, _reason -> %{state | stream_cancelled?: true}
    _kind, _reason -> %{state | stream_cancelled?: true}
  end
