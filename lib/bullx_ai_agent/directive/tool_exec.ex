defmodule BullXAIAgent.Directive.ToolExec do
  @moduledoc """
  ...
  2. **Name lookup**: When `action_module` is nil, runtime resolves the tool name
     against the current strategy/plugin tool map and executes via `BullXAIAgent.Turn`.
  ...
  @schema Zoi.struct(
            ...
            %{
              ...
              tool_name:
                Zoi.string(
                  description:
                    "Name of the tool (resolved from runtime tool map if action_module not provided)"
                ),
              action_module:
              ...
              context:
                ...
                |> Zoi.default(%{}),
              timeout_ms:
                Zoi.integer(description: "Per-attempt timeout in milliseconds") |> Zoi.optional(),
              max_retries:
...
defimpl Jido.AgentServer.DirectiveExec, for: BullXAIAgent.Directive.ToolExec do
  @moduledoc """
  ...
  Uses `BullXAIAgent.Turn` for execution, which provides consistent error
  ...
  def exec(directive, _input_signal, state) do
    ...
    agent_pid = self()
    task_supervisor = BullXAIAgent.Directive.Helpers.get_task_supervisor(state)
  ...
  defp execute_with_retries(
    ...
    0..max_retries
    |> Enum.reduce_while(
      {{:error, SignalHelpers.error_envelope(:executor, "uninitialized error"), []}, 0},
      fn attempt, _acc ->
        if attempt > 0 do
          maybe_emit(
            obs_cfg,
            Observe.tool(:retry),
            %{duration_ms: 0, retry_count: attempt},
            event_meta
          )
