defmodule BullX.LLM.Providers.AmazonBedrock do
  @moduledoc """
  ...
  **Impact:** Tests or production code using slow models with high token limits may intermittently
  ...
  alias BullX.LLM.Providers.Anthropic
  alias BullX.LLM.Providers.Anthropic.PlatformReasoning
  alias ReqLLM.Step
  ...
  @model_families %{
    "anthropic" => BullX.LLM.Providers.AmazonBedrock.Anthropic,
    "openai" => BullX.LLM.Providers.AmazonBedrock.OpenAI,
    "meta" => BullX.LLM.Providers.AmazonBedrock.Meta
  }

  @embedding_families %{
    "cohere" => BullX.LLM.Providers.AmazonBedrock.Cohere
  }
      ...
      if use_converse do
        ...
        formatter =
          if function_exported?(family_formatter, :requires_converse_api?, 0) and
            ...
            BullX.LLM.Providers.AmazonBedrock.Converse
          end
  ...
  def init_stream_state(model) do
    ...
    case get_model_family(model_id) do
      "anthropic" -> BullX.LLM.Providers.Anthropic.Response.init_stream_state()
      _ -> nil
    end
  end
  ...
  def decode_stream_event(event, model, state) when is_map(event) do
    ...
    cond do
      converse_event?(event) ->
        {decode_formatter_stream_event(BullX.LLM.Providers.AmazonBedrock.Converse, event), state}

      get_model_family(model_id) == "anthropic" ->
        BullX.LLM.Providers.Anthropic.Response.decode_stream_event(%{data: event}, model, state)
  ...
  def flush_stream_state(model, state) do
    ...
    case get_model_family(model_id) do
      "anthropic" -> BullX.LLM.Providers.Anthropic.Response.flush_stream_state(model, state)
      _ -> {[], state}
    end
  end
  ...
  def wrap_response(%BullX.LLM.Providers.AmazonBedrock.Response{} = already_wrapped) do
    # Don't double-wrap
    already_wrapped
  end

  def wrap_response(data) when is_map(data) do
    %BullX.LLM.Providers.AmazonBedrock.Response{payload: data}
  end
    ...
    case model_family do
      "anthropic" ->
        ...
        {translated_opts, warnings} =
          BullX.LLM.Providers.Anthropic.translate_options(operation, model, opts)
  ...
  defp determine_use_converse(model_id, opts) do
    ...
    is_fallback_to_converse = formatter == BullX.LLM.Providers.AmazonBedrock.Converse
