defmodule BullX.LLM.Providers.Google do
  @moduledoc """
  ...
  Gemini models support explicit context caching to reduce costs by up to 90% when reusing large amounts of content:
      ...
      {:ok, cache} = BullX.LLM.Providers.Google.CachedContent.create(
        provider: :google,
  ...
  See `BullX.LLM.Providers.Google.CachedContent` for full API documentation.
  ...
  **Note**: Setting `google_api_version: "v1"` with function calling (tools) or grounding will return an error.
  ...
  def list_models(opts) when is_list(opts) do
    BullX.LLM.ProviderModelDiscovery.list_google(
      Keyword.put(opts, :base_url, model_discovery_base_url(opts))
    )
  end

  defp model_discovery_base_url(opts) do
    case {Keyword.get(opts, :base_url), google_api_version(opts)} do
      {nil, version} ->
  ...
  defp google_api_version(opts) do
    opts
    |> Keyword.get(:opts, [])
    |> Keyword.get(:provider_options, [])
    |> Keyword.get(:google_api_version, "v1beta")
  end
