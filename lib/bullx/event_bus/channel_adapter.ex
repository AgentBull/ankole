defmodule BullX.EventBus.ChannelAdapter do
  @moduledoc """
  Common trusted plugin Channel Adapter contract.

  Adapters normalize provider input into decoded CloudEvents and call
  ...
  def deliver(reply_channel, outbound, opts \\ [])
      when is_map(reply_channel) and is_map(outbound) and is_list(opts) do
  ...
  def accept_inbound(adapter_id, source, provider_input, opts \\ [])
    ...
    event_bus_opts = Keyword.delete(opts, :registry)
    ...
    with {:ok, extension} <- fetch_enabled_adapter(adapter_id, registry),
      ...
      accept_event(extension, event, event_bus_opts, metadata)
    else
      :ignore ->
  ...
  defp configured_registry do
    Application.get_env(:bullx, :event_bus_channel_adapter_registry, BullX.Plugins.Registry)
  end

  defp reply_channel_value(reply_channel, key) do
    case map_value(reply_channel, key) do
      value when is_binary(value) and value != "" -> {:ok, value}
      _value -> {:error, {:invalid_reply_channel, key}}
    end
  end
  ...
  defp accept_event(%Extension{} = extension, event, opts, metadata) do
    ...
    try do
      result = BullX.EventBus.accept(event, opts)
  ...
  defp accept_status({:ok, %BullX.EventBus.Accepted{status: status}}), do: status
  ...
  defp emit(path, measurements, metadata) do
    :telemetry.execute([:bullx, :event_bus, :adapter | path], measurements, metadata)
  end
  ...
  defp normalize_payload(data) do
    %{
      ...
      "reply_channel" => maybe_stringify_map(Map.get(data, :reply_channel)),
