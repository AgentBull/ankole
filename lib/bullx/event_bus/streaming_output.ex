defmodule BullX.EventBus.StreamingOutput do
  @moduledoc """
  Redis-backed weak TargetSession output stream buffer.
  """
  ...
  def create_stream(target_session_id, target_session_entry_id, opts \\ [])
      when is_binary(target_session_id) and is_list(opts) do
    with :ok <- validate_opts(opts),
         stream_id <- BullX.Ext.gen_uuid_v7(),
         now <- now_ms(),
         ttl_ms <- ttl_ms(),
         meta <- meta_key(stream_id),
         fields <- metadata_fields(target_session_id, target_session_entry_id, now, ttl_ms),
         {:ok, _replies} <- Redis.pipeline([["HSET", meta | fields], ["PEXPIRE", meta, ttl_ms]]) do
      emit(:created, %{}, %{stream_id: stream_id, target_session_id: target_session_id})
      {:ok, stream_id}
    else
  ...
  defp metadata_fields(target_session_id, target_session_entry_id, now, ttl_ms) do
    [
      "status",
      @open_status,
      "next_offset",
