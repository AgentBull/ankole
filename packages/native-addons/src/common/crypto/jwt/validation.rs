...
#[napi(object, js_name = "JWTValidation")]
#[derive(Default)]
...
impl From<&JWTValidation> for jsonwebtoken::Validation {
  #[inline]
  fn from(value: &JWTValidation) -> Self {
    ...
    if let Some(algorithms) = &value.algorithms {
      validation.algorithms = algorithms.iter().map(|alg| (*alg).into()).collect();
    }
    if let Some(iss) = &value.iss {
      validation.set_issuer(iss);
    }
    // Note: validate_signature is handled at the decode call site
    // If false, use jsonwebtoken::dangerous::insecure_decode instead
