...
use crate::common::crypto::jwt::{claims::Claims, validation::JWTValidation};
...
impl VerifyTask {
  pub fn verify(token: &str, key: &[u8], validation: &JWTValidation) -> Result<Claims> {
    ...
    let result = if validation.validate_signature == Some(false) {
      // Use insecure decode when signature validation is disabled
      jsonwebtoken::dangerous::insecure_decode::<Claims>(token)
    } else {
      // Normal decode with signature validation
