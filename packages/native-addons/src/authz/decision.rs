...
/// Single-action authorization snapshot supplied by the TypeScript layer.
///
...
/// serde maps it from the `type` JSON field produced by TypeScript.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Candidate permission grant loaded by TypeScript.
///
...
/// Authorization decision returned to TypeScript.
...
pub fn authz_authorize(snapshot: JsonValue) -> Result<JsonValue> {
  let snapshot: AuthzSnapshot = serde_json::from_value(snapshot).map_err(|reason| {
    Error::new(
      Status::InvalidArg,
      format!("invalid authz snapshot: {reason}"),
    )
  })?;

  let decision = authorize_one(&snapshot)?;
  serde_json::to_value(decision).map_err(|reason| {
    Error::new(
      Status::GenericFailure,
      format!("failed to encode authz decision: {reason}"),
    )
  })
}
...
pub fn authz_authorize_all(snapshot: JsonValue) -> Result<JsonValue> {
  let snapshot: AuthzBatchSnapshot = serde_json::from_value(snapshot).map_err(|reason| {
    Error::new(
      Status::InvalidArg,
      format!("invalid authz batch snapshot: {reason}"),
    )
  })?;
  ...
  if snapshot.principal.status != "active" {
    return json_decision("principal_disabled", diagnostics, effective_group_ids, None);
  }
  ...
  json_decision("allow", diagnostics, effective_group_ids, None)
}

fn authorize_one(snapshot: &AuthzSnapshot) -> Result<AuthzDecision> {
  let (effective_group_ids, mut diagnostics) = effective_group_ids(
    &snapshot.principal,
    &snapshot.static_group_ids,
    &snapshot.computed_groups,
  ...
  if snapshot.principal.status != "active" {
    return Ok(AuthzDecision {
      status: "principal_disabled".to_owned(),
      diagnostics,
      effective_group_ids,
      denied_action: None,
    });
  ...
  Ok(AuthzDecision {
    status: if allowed { "allow" } else { "deny" }.to_owned(),
    diagnostics,
    effective_group_ids,
    denied_action: if allowed {
      None
    } else {
...
fn effective_group_ids(
...
) -> Result<(Vec<String>, Vec<AuthzDiagnostic>)> {
  ...
  for group in computed_groups {
    let program =
      match cached_condition_program(format!("computed_group:{}", group.id), &group.condition) {
        Ok(program) => program,
        Err(reason) => {
...
fn grants_allow(
...
) -> Result<(bool, Vec<AuthzDiagnostic>)> {
  let effective_group_ids: HashSet<&str> = effective_group_ids.iter().map(String::as_str).collect();
  ...
  for grant in grants {
    ...
    let program = match cached_condition_program(format!("grant:{}", grant.id), &grant.condition) {
      Ok(program) => program,
      Err(reason) => {
...
/// Returns a compiled CEL program, recompiling when persisted text changes.
...
/// Matches a grant resource pattern using a cached glob matcher.
...
fn json_decision(
  ...
  serde_json::to_value(AuthzDecision {
...
mod tests {
  ...
  fn authorize_direct_grant_allows() {
    ...
    let decision = authorize_one(&snapshot).unwrap();
  ...
  fn authorize_computed_group_grant_allows() {
    let snapshot = AuthzSnapshot {
      ...
      computed_groups: vec![SnapshotComputedGroup {
        ...
        condition: r#"principal.type == "human" && principal.status == "active""#.to_owned(),
      }],
      grants: vec![SnapshotGrant {
        id: "grant-1".to_owned(),
        principal_uid: None,
