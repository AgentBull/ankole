// Topology text reporters. Ported verbatim from openclaw (generic over
// TopologyRecord fields; only relative import extensions changed).

import type { ReportModule, TopologyEnvelope, TopologyRecord } from './types'

function canonicalExportName(record: TopologyRecord): string {
  const finalColon = record.canonicalKey.lastIndexOf(':')
  return finalColon >= 0 ? record.canonicalKey.slice(finalColon + 1) : (record.exportNames[0] ?? '<unknown>')
}
...
function formatUnusedTotal(envelope: TopologyEnvelope): string {
  return envelope.totals.allowlistedUnused > 0
    ? `${envelope.totals.unused} (${envelope.totals.allowlistedUnused} allowlisted)`
    : String(envelope.totals.unused)
}
  ...
  'public-surface-usage': {
    ...
    describe(envelope, limit) {
      ...
      return [
        ...
        `Unused public exports: ${formatUnusedTotal(envelope)}`,
        '',
