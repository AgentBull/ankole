...
export function collectSourceFiles(root: string, options: SourceFileCollectionOptions): string[] {
  ...
  if (stats.isFile()) {
    return options.sourceExtensions.some(extension => repoPath.endsWith(extension)) ? [repoPath] : []
  }
...
export function collectStronglyConnectedComponents(graph: ReadonlyMap<string, readonly string[]>): string[][] {
  let nextIndex = 0
  const stack: string[] = []
  ...
  return components.toSorted(
    (left, right) => right.length - left.length || cycleSignature(left).localeCompare(cycleSignature(right))
  )
}

function findCycleWitness(component: readonly string[], graph: ReadonlyMap<string, readonly string[]>): string[] {
...
export function formatCycle(component: readonly string[], graph: ReadonlyMap<string, readonly string[]>): string {
