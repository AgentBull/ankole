...
defmodule Ankole.Tools.AIGatewayRealProviderE2E do
  ...
  Models are hardcoded on purpose: this runner gates a known-good
  provider/model matrix, so there are no model override environment variables.
  """
  ...
  defp case_models_http(agent) do
    ...
    %{
      count: length(data),
      required_selectors: ["primary", "embedding.default", "rerank.default"]
    }
  end
        ...
        "input" => [
          %{
            ...
            "content" => [
              %{
                "type" => "input_text",
                "text" => "Look at the image and reply with one short sentence."
              },
