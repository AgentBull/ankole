...
- Two identity providers that issue the same opaque account id no longer share one Principal. A new account gets a provider-scoped identifier, while a verified email address, a phone number, or a manual mapping still joins one person across providers. Existing accounts keep their current identifier.
