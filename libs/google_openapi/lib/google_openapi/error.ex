defmodule GoogleOpenAPI.Error do
  @moduledoc "Google OAuth/Admin SDK and transport error."
  ...
  def message(%__MODULE__{reason: reason, status: status}) do
    "google_openapi error: reason=#{inspect(reason)} status=#{inspect(status)}"
  end
  ...
  defimpl Inspect do
    ...
    def inspect(%GoogleOpenAPI.Error{} = error, opts) do
      visible =
      ...
      concat(["#GoogleOpenAPI.Error<", to_doc(visible, opts), ">"])
    end
  end
end
