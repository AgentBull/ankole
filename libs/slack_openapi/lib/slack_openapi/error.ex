defmodule SlackOpenAPI.Error do
  @moduledoc "Slack Web API and transport error."
  ...
  def message(%__MODULE__{reason: reason, status: status}) do
    "slack_openapi error: reason=#{inspect(reason)} status=#{inspect(status)}"
  end
  ...
  defimpl Inspect do
    ...
    def inspect(%SlackOpenAPI.Error{} = error, opts) do
      visible =
      ...
      concat(["#SlackOpenAPI.Error<", to_doc(visible, opts), ">"])
    end
  end
end
