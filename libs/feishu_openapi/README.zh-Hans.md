...
这些辅助方法对齐官方 Node SDK 源码路径：
`authen/v1/oidc/access_token` 和 `authen/v1/oidc/refresh_access_token`。
...
`FeishuOpenAPI.Pagination` 会先读取 `data.page_token`，再回退到
`data.next_page_token`，与官方 Node SDK 生成的迭代器保持一致。
