defmodule FeishuOpenAPI.Auth do
  ...
  Bootstrap token functions call through `FeishuOpenAPI.request/4` with
  `access_token_type: nil`, so they bypass `FeishuOpenAPI.TokenManager` and won't
  recurse when invoked by it. OIDC user-token helpers use the normal client
  request path because the official SDK sends them with tenant-token
  authorization.
  ...
  Uses the OIDC access-token endpoint used by the official Node SDK. Options:
