...
function Input({ className, type, ...props }: React.ComponentProps<'input'>) {
  return (
    <InputPrimitive
      ...
      data-slot="input"
      className={cn(
        'h-10 w-full min-w-0 rounded-none border border-transparent border-b-input bg-field px-4 text-sm leading-5 outline-none placeholder:text-muted-foreground focus-visible:border-b-ring user-invalid:border-b-destructive disabled:pointer-events-none disabled:cursor-not-allowed disabled:bg-muted disabled:text-muted-foreground aria-invalid:border-b-destructive dark:aria-invalid:border-b-destructive/50',
        type === 'file'
