...
- **EventBus** 接收真实世界和系统内部的 Event，按 Event Routing Rule 路由，并把被接受的 Event 送到正确 Target。
- **Target** 承担处理工作：最常见的是负责灵活判断的 AIAgent，或负责显式流程结构的 Workflow。
- **TargetSession** 为运行时工作划出作用域和时间窗口；持久业务事实保存在 Conversation、Work、ApprovalRequest、ChildRun、Brain、Budget 和 domain record 中。
...
| --- | --- | --- | --- |
| 核心单元 | Agentic Loop 或 assistant session。 | App、bot、RPA flow 或 workflow run。 | 拥有长期目标、责任、Work 和 EventBus 路由上下文的 Agent。 |
| 自主性 | 响应 prompt、消息、cron 或用户配置的任务。 | 执行某个具体业务场景的既定流程。 | 观察 Event、排列优先级、请求帮助、委派任务，并围绕长期目标推进工作。 |
...
| 外部事件 | Channel、cron、webhook 和 integration 进入 assistant loop。 | Trigger 启动一个预定义 app 或 workflow。 | Event 进入 EventBus，命中 Event Routing Rule，调用 Target，并通过业务记录更新长期 Work。 |
...
**一个 Event 可以进入正确的工作路径。** 一条客户预算冻结的消息进入 EventBus，命中 Event Routing Rule，并到达 Target。这个 Target 可以是直接处理 case 的 AIAgent，也可以是表达显式分支、审批、并行和确定性步骤的 Workflow。
