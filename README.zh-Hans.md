...
- **一套控制面拥有状态和协调权。** 主体与 AuthZ、SignalsGateway、Schedule、Actor Runtime、Job 生命周期和 AIGateway 都在 Elixir/OTP 中作出持久决策，语义事实写入 PostgreSQL。
...
- **运维控制台。** Agent、Agent Library 全局默认与逐 Agent 覆盖、Control Plane Plugin、模型提供商、模型档案、身份、信号、Worker、Worker 环境和后台 Agent 任务都可以从内置 Web 控制台管理。
...
| 领域 | 状态 |
| --- | --- |
| 控制面 | `app/control_plane` 下的 Phoenix/OTP 应用，负责持久状态、配置、Actor 编排、主体与 AuthZ、AIGateway、SignalsGateway 和运维 API。 |
