defmodule BullX.AuthZ.RequestTest do
  ...
  test "build/4 normalizes request strings and Cedar-compatible context" do
    assert {:ok, request} =
  ...
  test "build/4 rejects malformed subjects, resource wildcards, action colons, and non-Cedar terms" do
    ...
    assert {:error, :invalid_request} = Request.build(@principal_id, "web", "read", %{k: 1.5})
