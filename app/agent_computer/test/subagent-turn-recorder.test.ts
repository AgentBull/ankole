...
import type { SubagentDelegationTurnUpsertRequest } from '../src/lanes/rpc_lane'
import { CODEX_OPT_OUT_NOTIFICATION_METHODS } from '../src/tools/codex/app-server-client'
import { MAX_SUBAGENT_TURN_TRAJECTORY_BYTES, SubagentTurnRecorder } from '../src/tools/subagent/turn-recorder'
...
describe('@ankole/agent-computer durable subagent Turn recorder', () => {
  ...
  it('uses item start only for active progress and completion for one canonical tool pair', async () => {
    ...
    expect(upserts.at(-1)?.trajectory.messages).toEqual([
      expect.objectContaining({ role: 'user', content: '执行命令' })
    ])
    ...
    expect(completed.trajectory.messages.slice(-2)).toEqual([
  ...
  it('counts completed tools before trajectory truncation and keeps tool names sorted', async () => {
    const { recorder, upserts } = fixture()
    ...
    expect(latest.trajectory.metadata).toMatchObject({ content_truncated: true })
    expect(latest.trajectory.messages.length).toBeLessThan(541)
  })
  ...
  it('persists pending request_user_input as the only unmatched tool call', async () => {
    ...
    const messages = upserts.at(-1)!.trajectory.messages
  ...
  it('keeps only reasoning summaries and never raw reasoning text', async () => {
    ...
    const serialized = JSON.stringify(upserts.at(-1)?.trajectory)
  ...
  it('redacts secrets and enforces the full 256 KiB durable trajectory bound', async () => {
    const { recorder, upserts } = fixture()
    ...
    expect(new TextEncoder().encode(serialized).byteLength).toBeLessThanOrEqual(MAX_SUBAGENT_TURN_TRAJECTORY_BYTES)
    expect(trajectory.metadata).toMatchObject({ redacted: true, content_truncated: true })
  })
})

function fixture(delayMs = 5) {
  const upserts: SubagentDelegationTurnUpsertRequest[] = []
  const recorder = new SubagentTurnRecorder({
