...
describe('@ankole/agent-computer runtime', () => {
  ...
  it('handles turn.start with accepted and PONG final proposal envelopes', async () => {
    const responses = await handleActorBusEnvelope({
      protocol_version: 1,
  ...
  it('does not downgrade a real model turn to the ping-pong fixture path', async () => {
    await expect(
      handleActorBusEnvelope({
        protocol_version: 1,
        message_id: 'turn-start-real-1',
        lane: 'LANE_TURN',
        durability: 'CONTROL_REPLAYABLE',
        body: {
          type: 'turn_start',
          turn_start: {
            ...
            inputs: [
              {
                actor_input_id: 'input-real-1',
                broker_sequence: 1,
                type: 'im.message.addressed',
                ingress_event_id: 'event-real-1',
                payload_json: { data: { entry: { text: 'Hello' } } }
              }
            ]
          }
        }
      })
    ).rejects.toThrow(/real LLM turn requires/)
  })
})
