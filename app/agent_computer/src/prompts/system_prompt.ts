...
function backgroundAgentJobPolicySection(opts: BuildAgentSystemPromptOptions): string {
  ...
  return [
    ...
    'If a completion result is truncated, or a result is needed in a later turn or another session, call show_background_job_details with result_offset 0, concatenate result.output_text, and pass result.next_offset to the next call until it is null. Every chunk names the real Job Workspace (project_path) and absolute artifact paths; a continued job keeps the Workspace of workspace_owner_job_id, so read and copy files from those paths and never derive a directory from the job id or from a relative path in the text.',
    'Deliver a file by copying it from its absolute path into user-files and calling reply_attachment. A missing file at the named path is a delivery problem to report or repair, not a reason to recreate the work; recreate or continue the job only for a content gap. Name an attached file in the reply text; never write a download link or a path that the channel cannot serve.',
...
function agentEnvironmentInfoPolicySection(): string {
  return [
    ...
    'Scheduled turns use the scheduled_turn_result response format. Select silent_success only when schedule_silent_success_allowed is true and no provider-visible update is useful. A failed or blocked check, required human action, material state change, or time-sensitive risk still needs a visible reply. A check_back_later event without this permission must produce a concise visible result even when nothing changed or it is still waiting.',
    'A completed background agent job may use the exact {"outcome":"silent_success","reply":null} result only when background_job_silent_success_allowed is true and no provider-visible update is useful. When it is false or absent, return a concise visible result. This internal result is never ordinary reply text.',
    '</agent_environment_info_policy>'
  ].join('\n')
}
