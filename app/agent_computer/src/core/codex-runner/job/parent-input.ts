...
/** Dynamic tool used by a lead Job to pause for information from its caller. */
export const PARENT_INPUT_TOOL_NAME = 'request_parent_input'

// Values accepted by this strict, bounded schema become pending input for the
// parent interaction surface.
...
export function parentInputToolSpec(): DynamicToolSpec {
  return {
    type: 'function',
...
export function pendingParentInputFromDynamicTool(params: DynamicToolCallParams): JSONObject | undefined {
  if (params.tool !== PARENT_INPUT_TOOL_NAME) return undefined
