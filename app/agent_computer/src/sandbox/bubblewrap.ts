...
// One process-wide result keeps startup reporting and later sandboxes on the
// same mode.
...
/**
 * Defines the sandbox view for Worker-owned child processes.
 *
 * Agent Home and Worker share are writable. Image and container system paths
 * are read-only unless the caller supplies an explicit writable bind.
 */
...
function runtimeBinds(): string[] {
  ...
  // the whole tree stays visible read-only. Environments without /repo skip
