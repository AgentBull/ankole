...
import { openTelemetryChannelSpanContext, runWithTracingChannelSpan } from './tracing-channel-publisher'
...
type TelemetryCallbackKey = Exclude<
  keyof {
    [K in keyof TelemetryDispatcher as TelemetryDispatcher[K] extends Callback<any> | undefined ? K : never]: true
  },
 ...
 */
type TelemetryEvent<K extends TelemetryCallbackKey> = TelemetryDispatcher[K] extends Callback<infer EVENT> | undefined
  ? EVENT
  : never
...
export function createTelemetryDispatcher({
  ...
  const mergeTelemetryCallback = <KEY extends TelemetryCallbackKey>(key: KEY): Callback<TelemetryEvent<KEY>> => {
    const integrationCallbacks = (
      ...
      callback =>
        ((event: TelemetryEvent<KEY>) => callback(augmentEvent(event, telemetryMetadata))) as Callback<
          TelemetryEvent<KEY>
        >
    )
  ...
  return {
    ...
    // window.
    onStepEnd: mergeCallbacks(mergeTelemetryCallback('onStepEnd')),
