...
// AssistantMessages and exposes them through a real AI SDK LanguageModel instance, so the
...
export function registerFauxProvider(input: {
  ...
  // Build a ready-to-call Model per id, mirroring the runtime model shape so code under test
...
/**
 * Implements the AI SDK's LanguageModel interface against the scripted response queue.
...
function responseContent(response: AssistantMessage): LanguageModelContent[] {
 ...
 */
function streamParts(response: AssistantMessage): LanguageModelStreamPart[] {
...
function finishReason(response: AssistantMessage): LanguageModelFinishReason {
 ...
 */
function v4Usage(usage: Usage): LanguageModelUsage {
  return {
    inputTokens: {
      total: usage.input,
