import type { JSONObject, LanguageModelFunctionTool } from '@/ai-gateway-client/provider'
 ...
 * @example
 * ```ts
 * import { wrapLanguageModel, addToolInputExamplesMiddleware } from '@/ai-gateway-client';
 *
  ...
  return {
    transformParams: async ({ params }) => {
      ...
      const transformedTools = params.tools.map(tool => {
        ...
        const formattedExamples = tool.inputExamples.map((example, index) => format(example, index)).join('\n')

        const examplesSection = `${prefix}\n${formattedExamples}`

        const toolDescription = tool.description ? `${tool.description}\n\n${examplesSection}` : examplesSection

        return {
          ...tool,
          description: toolDescription,
          inputExamples: remove ? undefined : tool.inputExamples
        } satisfies LanguageModelFunctionTool
      })
