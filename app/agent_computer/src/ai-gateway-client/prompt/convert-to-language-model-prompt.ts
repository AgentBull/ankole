...
import { createDefaultDownloadFunction, type DownloadFunction } from '../util/download/download-function'
import { convertToLanguageModelFilePart } from './file-part-data'
...
/**
 * Convert a ModelMessage to a LanguageModelMessage.
 *
        ...
        content: message.content
          ...
          .map(part => {
            ...
            switch (part.type) {
              ...
              case 'file': {
                const { data, mediaType } = convertToLanguageModelFilePart(part.data)
                return {
                  type: 'file' as const,
                  data,
 ...
 * conversion logic.
 */
function convertImagePartToFilePart(part: TextPart | ImagePart | FilePart): TextPart | FilePart {
  if (part.type !== 'image') {
    return part
  }
  return {
...
export async function downloadAssets(
  ...
  type ConvertedFile = {
    ...
    data: LanguageModelFilePart['data']
  }
  type UrlTaggedFile = {
    mediaType: string | undefined
  ...
  const plannedDownloads = downloadableFiles
    .map((part): ConvertedFile => {
      ...
      const { data } = convertToLanguageModelFilePart(part.data)
  ...
  return Object.fromEntries(
    downloadedFiles
      .map((file, index) =>
        file == null ? null : [plannedDownloads[index].url.toString(), { data: file.data, mediaType: file.mediaType }]
      )
...
/**
 * Convert part of a user message to a LanguageModelPart.
 *
...
function convertPartToLanguageModelPart(
  ...
  downloadedAssets: Record<string, { mediaType: string | undefined; data: Uint8Array }>
): LanguageModelTextPart | LanguageModelFilePart {
  if (part.type === 'text') {
    return {
      type: 'text',
      text: part.text,
  ...
  const { data: normalizedData, mediaType: dataUrlMediaType } = convertToLanguageModelFilePart(part.data)

  let mediaType: string | undefined = dataUrlMediaType ?? part.mediaType
  let data: LanguageModelFilePart['data'] = normalizedData
 ...
 */
function getMediaTypeFromUrl(url: string, fallbackMediaType = 'application/octet-stream'): string {
  try {
