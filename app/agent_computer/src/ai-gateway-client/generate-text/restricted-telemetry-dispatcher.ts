import type { Context, InferToolContext, InferToolSetContext, ToolSet } from '@/ai-gateway-client/provider-utils'
import { createTelemetryDispatcher } from '../telemetry/create-telemetry-dispatcher'
import type { TelemetryDispatcher } from '../telemetry/telemetry'
import type { IncludedContext, IncludedToolsContext, TelemetryOptions } from '../telemetry/telemetry-options'
import type {
...
function filterIncludedContext<CONTEXT extends Context>({
  ...
  return Object.fromEntries(Object.entries(context).filter(([key]) => includeContext?.[key as keyof CONTEXT] === true))
}
...
function filterToolContext<TOOLS extends ToolSet>({
  ...
  const includeToolContext = (
    includeToolsContext as Record<string, IncludedContext<InferToolContext<TOOLS[typeof toolName]>>> | undefined
  )?.[toolName]
