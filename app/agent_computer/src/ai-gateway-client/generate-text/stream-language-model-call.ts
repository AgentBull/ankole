import {
  ...
  type LanguageModelStreamPart as ProviderLanguageModelStreamPart,
  type SharedHeaders
 ...
 */
export async function streamLanguageModelCall<TOOLS extends ToolSet, OUTPUT extends Output = Output>({
  model,
  tools,
  toolOrder,
  output,
  ...
  // LanguageModelPrompt and we only want download URLs at most once.
  // Therefore convertToLanguageModelPrompt can only be called once
  // per step and the resulting LanguageModelPrompt needs to be
  ...
  onStart?: (args: { promptMessages: LanguageModelPrompt }) => Promise<void> | void
} & Prompt &
  ...
  const standardizedStream = languageModelStream.pipeThrough(
    createLanguageModelStreamPartToLanguageModelStreamPartTransform({
      tools,
      instructions: standardizedPrompt.instructions,
      messages: standardizedPrompt.messages,
      repairToolCall,
...
// Java Loves You.
function createLanguageModelStreamPartToLanguageModelStreamPartTransform<TOOLS extends ToolSet>({
  tools,
  instructions,
  messages,
  repairToolCall,
  ...
  return new TransformStream<ProviderLanguageModelStreamPart, LanguageModelStreamPart<TOOLS>>({
 ...
 * and tool calls.
 */
function isOutputChunk(chunk: ProviderLanguageModelStreamPart): boolean {
  return (
