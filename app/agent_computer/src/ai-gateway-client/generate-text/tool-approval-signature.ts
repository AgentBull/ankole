import {
  ...
  convertUint8ArrayToArrayBuffer,
...
async function importKey(secret: string | Uint8Array): Promise<CryptoKey> {
  ...
  return crypto.subtle.importKey(
    'raw',
    convertUint8ArrayToArrayBuffer(keyData),
...
function buildPayload(approvalId: string, toolCallId: string, toolName: string, inputDigest: string): Uint8Array {
...
export async function signToolApproval({
  ...
  const sig = await crypto.subtle.sign('HMAC', key, convertUint8ArrayToArrayBuffer(payload))
...
export async function verifyToolApprovalSignature({
  ...
  return crypto.subtle.verify(
    'HMAC',
    key,
    convertUint8ArrayToArrayBuffer(sigBytes),
    convertUint8ArrayToArrayBuffer(payload)
  )
}
