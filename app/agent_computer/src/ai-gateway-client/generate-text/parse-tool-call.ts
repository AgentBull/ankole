import type { LanguageModelToolCall } from '@/ai-gateway-client/provider'
import {
  asSchema,
  safeParseJSON,
  safeValidateTypes,
...
export async function parseToolCall<TOOLS extends ToolSet>({
  ...
  try {
    ...
    try {
      ...
      let repairedToolCall: LanguageModelToolCall | null = null
...
async function parseProviderExecutedDynamicToolCall(toolCall: LanguageModelToolCall): Promise<DynamicToolCall> {
  const parseResult =
    toolCall.input.trim() === '' ? { success: true as const, value: {} } : await safeParseJSON({ text: toolCall.input })
