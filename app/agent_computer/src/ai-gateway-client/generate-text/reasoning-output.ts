import type { DataContent, ReasoningFilePart, ReasoningPart } from '@/ai-gateway-client/provider-utils'
import type { ProviderMetadata } from '../types/provider-metadata'
import { DefaultGeneratedFile, type GeneratedFile } from './generated-file'

function unwrapReasoningFileData(data: ReasoningFilePart['data']): DataContent | URL {
  ...
  return parts.map(part => {
    if (part.type === 'reasoning') {
      return {
        type: 'reasoning' as const,
        text: part.text,
        ...(part.providerOptions != null ? { providerMetadata: part.providerOptions as ProviderMetadata } : {})
      }
    }
