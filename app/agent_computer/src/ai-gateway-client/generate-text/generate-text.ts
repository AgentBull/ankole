...
import { addLanguageModelUsage, asLanguageModelUsage, type LanguageModelUsage } from '../types/usage'
...
>({
  ...
  const headersWithUserAgent = withUserAgentSuffix(headers ?? {}, `com.ankole.ai-gateway-client`)
  ...
  const runInTracingChannelSpan =
    ...
    (async <T>({ execute }: { execute: () => PromiseLike<T> }) => await execute())
  ...
  const executeGenerateText = async () => {
    ...
    try {
      ...
      const { approvedToolApprovals, deniedToolApprovals: collectedDeniedToolApprovals } = collectToolApprovals<TOOLS>({
      ...
      let currentModelResponse: LanguageModelGenerateResult & {
            ...
            execute: async () => {
              ...
              const stepToolCalls: TypedToolCall<TOOLS>[] = await Promise.all(
                currentModelResponse.content
                  .filter((part): part is LanguageModelToolCall => part.type === 'tool-call')
                  .map(toolCall =>
                    parseToolCall({
                      toolCall,
                      tools,
              ...
              const stepToolApprovalResponses: Record<string, ToolApprovalResponseOutput<TOOLS>> = {}
              ...
              const stepRequest: LanguageModelRequestMetadata = {
                ...
                body: include.requestBody ? currentModelResponse.request?.body : undefined,
              ...
              const stepResponse = {
                ...
                body: include.responseBody ? currentModelResponse.response?.body : undefined
              }
...
> {
  ...
  return toolResults.filter((result): result is NonNullable<typeof result> => result != null)
}
