import { delay as originalDelay, type ToolSet } from '@/ai-gateway-client/provider-utils'
import { InvalidArgumentError, type SharedProviderMetadata } from '@/ai-gateway-client/provider'
...
export function smoothStream<TOOLS extends ToolSet>({
  ...
  if (
    ...
    const chunkingRegex =
      typeof chunking === 'string' ? CHUNKING_REGEXPS[chunking] : chunking instanceof RegExp ? chunking : undefined
  ...
  return () => {
    ...
    let providerMetadata: SharedProviderMetadata | undefined = undefined
