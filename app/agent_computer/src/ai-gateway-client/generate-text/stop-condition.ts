import type { Context, ToolSet } from '@/ai-gateway-client/provider-utils'
 ...
 */
export type StopCondition<TOOLS extends ToolSet, RUNTIME_CONTEXT extends Context = Context> = (options: {
...
export function hasToolCall<TOOLS extends ToolSet>(
  ...
  return ({ steps }) =>
    steps[steps.length - 1]?.toolCalls?.some(toolCall => toolName.includes(toolCall.toolName)) ?? false
}
 ...
 */
export async function isStopConditionMet<TOOLS extends ToolSet, RUNTIME_CONTEXT extends Context = Context>({
  stopConditions,
  steps
}: {
