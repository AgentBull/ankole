import { getErrorMessage, UnsupportedFunctionalityError, type SharedWarning } from '@/ai-gateway-client/provider'
import {
  asArray,
  createIdGenerator,
  DelayedPromise,
...
import { executeToolsFromStream, type ExecuteToolsStreamPart } from './execute-tools-from-stream'
import { executeToolCall } from './execute-tool-call'
import { filterActiveTools, type ActiveToolSubset } from './filter-active-tools'
import type {
...
import type { OnLanguageModelCallEndCallback, OnLanguageModelCallStartCallback } from './language-model-events'
...
import type { OnToolExecutionEndCallback, OnToolExecutionStartCallback } from './tool-execution-events'
 ...
 * @param event - The event that is passed to the callback.
 */
export type StreamTextOnAbortCallback<TOOLS extends ToolSet, RUNTIME_CONTEXT extends Context> = Callback<{
  /**
...
>({
  ...
  include,
  _internal: { now = originalNow, generateId = originalGenerateId, generateCallId = originalGenerateCallId } = {},
  ...settings
  ...
  ToolsContextParameter<TOOLS> & {
     ...
     */
    onStepFinish?: GenerateTextOnStepEndCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>>
  ...
  const resolvedOnStart = onStart
  const resolvedOnStepStart = onStepStart
  const resolvedOnLanguageModelCallStart = onLanguageModelCallStart
  const resolvedOnLanguageModelCallEnd = onLanguageModelCallEnd
  const resolvedOnToolExecutionStart = onToolExecutionStart
  const resolvedOnToolExecutionEnd = onToolExecutionEnd
...
function createOutputTransformStream<TOOLS extends ToolSet, OUTPUT extends Output>(
  ...
  function publishTextChunk({
    ...
    controller: TransformStreamDefaultController<EnrichedStreamPart<TOOLS, InferPartialOutput<OUTPUT>>>
  ...
  return new TransformStream<TextStreamPart<TOOLS>, EnrichedStreamPart<TOOLS, InferPartialOutput<OUTPUT>>>({
...
> implements StreamTextResult<TOOLS, RUNTIME_CONTEXT, OUTPUT> {
  ...
  >()
  private readonly _steps = new DelayedPromise<Awaited<StreamTextResult<TOOLS, RUNTIME_CONTEXT, OUTPUT>['steps']>>()
  private readonly _initialResponseMessages = new DelayedPromise<Array<ResponseMessage>>()
  ...
  constructor({
    ...
    stopConditions: Array<StopCondition<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>>>
    ...
    prepareStep: PrepareStepFunction<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>> | undefined
    now: () => number
    generateId: () => string
    ...
    onError: StreamTextOnErrorCallback
    onEnd: undefined | GenerateTextOnEndCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>>
    onAbort: undefined | StreamTextOnAbortCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>>
    onStepFinish: undefined | GenerateTextOnStepEndCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>>
    onStart: undefined | GenerateTextOnStartCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>, NoInfer<OUTPUT>>
    onStepStart: undefined | GenerateTextOnStepStartCallback<NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>, NoInfer<OUTPUT>>
    onLanguageModelCallStart: undefined | OnLanguageModelCallStartCallback
    onLanguageModelCallEnd: undefined | OnLanguageModelCallEndCallback<NoInfer<TOOLS>>
    ...
    >({
      async transform(chunk, controller) {
        ...
        if (part.type === 'finish-step') {
          ...
          const currentStepResult: StepResult<TOOLS, RUNTIME_CONTEXT> = new DefaultStepResult({
            callId,
            ...
            request: {
              ...recordedRequest,
              messages: include.requestMessages ? cloneModelMessages(recordedRequestMessages) : undefined
            },
            response: {
              ...part.response,
        ...
        async function abort() {
          await notify({
            event: {
              callId,
              steps: recordedSteps,
              ...(abortSignal?.reason !== undefined ? { reason: abortSignal.reason } : {})
            },
          ...
          controller.enqueue({
            ...
            ...(abortSignal?.reason !== undefined ? { reason: getErrorMessage(abortSignal.reason) } : {})
          })
          controller.close()
        }
    ...
    ;(async () => {
      ...
      const { approvedToolApprovals, deniedToolApprovals } = collectToolApprovals<TOOLS>({ messages: initialMessages })
      ...
      if (deniedToolApprovals.length > 0 || approvedToolApprovals.length > 0) {
        ...
        let toolExecutionStepStreamController: ReadableStreamDefaultController<TextStreamPart<TOOLS>> | undefined
      ...
      async function streamStep({ currentStep, usage }: { currentStep: number; usage: LanguageModelUsage }) {
        ...
        try {
          ...
          const stepTools = await prepareTools({
            ...
            toolOrder: stepToolOrder as ToolOrder<ActiveToolSubset<TOOLS, ActiveTools<NoInfer<TOOLS>>>>,
          ...
          const stepInstructions =
            prepareStepResult?.instructions ?? prepareStepResult?.system ?? instructionsForNextStep
            ...
            retry(async () =>
              streamLanguageModelCall({
                ...
                onLanguageModelCallEnd: filterNullable(
                  onLanguageModelCallEnd,
                  telemetryDispatcher.onLanguageModelCallEnd as undefined | OnLanguageModelCallEndCallback<TOOLS>
                ),
          ...
          const runInTracingChannelSpanInStep =
            telemetryDispatcher.runInTracingChannelSpan == null
              ? undefined
              : <T>(
                  options: Parameters<NonNullable<TelemetryDispatcher['runInTracingChannelSpan']>>[0] & {
                    execute: () => PromiseLike<T>
                  }
                ) => runInStepTracingChannelContext(() => telemetryDispatcher.runInTracingChannelSpan!(options))
          ...
          let warnings: SharedWarning[] | undefined
          ...
          let modelCallPerformance: Omit<StepResultPerformance, 'stepTimeMs' | 'toolExecutionMs'> = {
            responseTimeMs: 0,
                ...
                async flush(controller) {
                  ...
                  if (
                    ...
                    ((clientToolCalls.length > 0 &&
                      clientToolCalls.length === clientToolOutputs.length + deniedToolApprovalResponses.length) ||
  ...
  get responseMessages() {
    return Promise.all([this._initialResponseMessages.promise, this.steps]).then(([initialResponseMessages, steps]) => [
  ...
  get stream(): AsyncIterableStream<TextStreamPart<TOOLS>> {
    return createAsyncIterableStream(
      this.teeStream().pipeThrough(
        new TransformStream<EnrichedStreamPart<TOOLS, InferPartialOutput<OUTPUT>>, TextStreamPart<TOOLS>>({
  ...
  get partialOutputStream(): AsyncIterableStream<InferPartialOutput<OUTPUT>> {
    return createAsyncIterableStream(
      this.teeStream().pipeThrough(
        new TransformStream<EnrichedStreamPart<TOOLS, InferPartialOutput<OUTPUT>>, InferPartialOutput<OUTPUT>>({
