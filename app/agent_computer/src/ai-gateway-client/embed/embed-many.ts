import { createIdGenerator, withUserAgentSuffix, type ProviderOptions } from '@/ai-gateway-client/provider-utils'
...
export async function embedMany({
  ...
  const headersWithUserAgent = withUserAgentSuffix(headers ?? {}, `ankole-ai-gatewa`)
  ...
  try {
    ...
    if (maxEmbeddingsPerCall == null || maxEmbeddingsPerCall === Infinity) {
      const { embeddings, usage, warnings, response, providerMetadata } = await retry(async () => {
