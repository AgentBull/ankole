...
const objectOutputStrategy = <OBJECT>(schema: Schema<OBJECT>): OutputStrategy<DeepPartial<OBJECT>, OBJECT, never> => ({
  type: 'object',
...
const arrayOutputStrategy = <ELEMENT>(
  ...
  return {
    ...
    async validateFinalResult(value: JSONValue | undefined): Promise<ValidationResult<Array<ELEMENT>>> {
      // check that the value is an object that contains an array of elements:
      if (!isJSONObject(value) || !isJSONArray(value.elements)) {
        return {
          success: false,
...
const enumOutputStrategy = <ENUM extends string>(enumValues: Array<ENUM>): OutputStrategy<string, ENUM, never> => {
  return {
    type: 'enum',
