...
import type { OnToolExecutionEndCallback, OnToolExecutionStartCallback } from '../generate-text/tool-execution-events'
...
> = LanguageModelCallOptions &
  ...
  ToolsContextParameter<TOOLS> & {
    ...
    prepareCall?: (
      options: Omit<
        AgentCallParameters<CALL_OPTIONS, NoInfer<TOOLS>, NoInfer<RUNTIME_CONTEXT>>,
        'onStepEnd' | 'onStepFinish'
      > &
        Pick<
          ToolLoopAgentSettings<CALL_OPTIONS, TOOLS, RUNTIME_CONTEXT, NoInfer<OUTPUT>>,
          | 'model'
          | 'tools'
          | 'maxOutputTokens'
          | 'temperature'
