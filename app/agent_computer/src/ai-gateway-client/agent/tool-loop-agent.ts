...
> implements Agent<CALL_OPTIONS, TOOLS, RUNTIME_CONTEXT, OUTPUT> {
  ...
  private readonly settings: ToolLoopAgentSettings<CALL_OPTIONS, TOOLS, RUNTIME_CONTEXT, OUTPUT>
    ...
    const preparedCallArgs =
      (await this.settings.prepareCall?.(
        baseCallArgs as Parameters<
          NonNullable<ToolLoopAgentSettings<CALL_OPTIONS, TOOLS, RUNTIME_CONTEXT, OUTPUT>['prepareCall']>
        >[0]
      )) ?? baseCallArgs
  ...
  > {
    ...
    const callbackArgs = {
      ...
      onStart: mergeCallbacks(
        ...
        onStart as GenerateTextOnStartCallback<TOOLS, RUNTIME_CONTEXT, OUTPUT> | undefined
      ),
      onStepStart: mergeCallbacks(
        this.settings.onStepStart,
        onStepStart as GenerateTextOnStepStartCallback<TOOLS, RUNTIME_CONTEXT, OUTPUT> | undefined
      ),
      ...
      onStepEnd: mergeCallbacks(this.settings.onStepEnd, onStepEnd),
