...
export async function generateImage({
  ...
  for (const result of results) {
    ...
    if (result.providerMetadata) {
      for (const [providerName, metadata] of Object.entries<ImageModelProviderMetadata[string]>(
...
async function invokeModelMaxImagesPerCall(model: ImageModel) {
...
function normalizePrompt(prompt: GenerateImagePrompt): Pick<ImageModelCallOptions, 'prompt' | 'files' | 'mask'> {
  ...
  return {
    prompt: prompt.text,
    files: prompt.images.map(toImageModelFile),
    mask: prompt.mask ? toImageModelFile(prompt.mask) : undefined
  }
}

function toImageModelFile(dataContent: DataContent): ImageModelFile {
