...
import type { ParseResult } from '@/ai-gateway-client/provider-utils'
...
export async function throwIfOpenResponsesStreamErrorBeforeOutput<T>({
  stream,
  getError,
  isOutputChunk,
  url,
  ...
  try {
    while (true) {
      ...
      if (errorFrame != null) {
        ...
        throw createOpenResponsesStreamError({
          frame: errorFrame,
          url,
          requestBodyValues,
          responseHeaders
...
function createOpenResponsesStreamError({
  ...
  return new APICallError({
    message: streamError?.message ?? 'OpenResponses stream failed before any output was generated',
    url,
    requestBodyValues,
...
function parseStreamError(frame: unknown): StreamError | undefined {
  ...
  return typeof error.message === 'string' &&
    (asRecord(value.error) != null || typeof error.type === 'string' || 'code' in error || 'param' in error)
    ? {
...
function getStatusCode(error: StreamError): number {
  ...
  if (['insufficient_quota', 'rate_limit'].some(term => discriminator.includes(term))) {
    return 429
  }
  ...
  if (['invalid', 'bad_request', 'context_length'].some(term => discriminator.includes(term))) {
    return 400
  }
