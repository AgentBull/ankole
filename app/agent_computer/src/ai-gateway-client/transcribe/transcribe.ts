import type { JSONObject } from '@/ai-gateway-client/provider'
import { detectMediaType, withUserAgentSuffix, type ProviderOptions } from '@/ai-gateway-client/provider-utils'
...
export async function transcribe({
  ...
  const headersWithUserAgent = withUserAgentSuffix(headers ?? {}, `com.agentbull.ankole-ai-gateway.client`)

  const audioData =
    audio instanceof URL ? (await downloadFn({ url: audio, abortSignal })).data : convertDataContentToUint8Array(audio)
