import type { JSONSchema7, JSONSchema7Definition } from '@/ai-gateway-client/provider'
...
export function addAdditionalPropertiesToJsonSchema(jsonSchema: JSONSchema7): JSONSchema7 {
  ...
  if (jsonSchema.items != null) {
    jsonSchema.items = Array.isArray(jsonSchema.items) ? jsonSchema.items.map(visit) : visit(jsonSchema.items)
  }
