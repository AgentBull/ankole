// @ts-nocheck
import type { ProviderV2, ProviderV3, ProviderV4 } from '@/llm/provider'
...
export function asProviderV4(provider: ProviderV2 | ProviderV3 | ProviderV4): ProviderV4 {
  ...
  const v3Provider: ProviderV3 =
    !('specificationVersion' in provider) || provider.specificationVersion !== 'v3'
      ? asProviderV3(provider as ProviderV2)
      : provider

  return {
    specificationVersion: 'v4',
    languageModel: (modelId: string) => asLanguageModelV4(v3Provider.languageModel(modelId)),
    embeddingModel: (modelId: string) => asEmbeddingModelV4(v3Provider.embeddingModel(modelId)),
    imageModel: (modelId: string) => asImageModelV4(v3Provider.imageModel(modelId)),
    transcriptionModel: v3Provider.transcriptionModel
      ? (modelId: string) => asTranscriptionModelV4(v3Provider.transcriptionModel!(modelId))
      : undefined,
    ...
    rerankingModel: v3Provider.rerankingModel
      ? (modelId: string) => asRerankingModelV4(v3Provider.rerankingModel!(modelId))
      : undefined
  }
}
