...
import { GATEWAY_AUTH_METHOD_HEADER, VERCEL_AI_GATEWAY_TEAM_HEADER } from './gateway-headers'
 ...
 */
export function createGateway(options: GatewayProviderSettings = {}): GatewayProvider {
  ...
  const createAuthHeaders = (auth: { token: string; authMethod: 'api-key' | 'oidc' }) =>
    withUserAgentSuffix(
      {
  ...
  provider.experimental_realtime = Object.assign((modelId: GatewayRealtimeModelId) => createRealtimeModel(modelId), {
