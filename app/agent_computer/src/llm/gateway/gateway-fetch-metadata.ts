...
import { KNOWN_MODEL_TYPES, type GatewayLanguageModelEntry, type KnownModelType } from './gateway-model-entry'
        ...
        .transform(models =>
          models.filter(
            (m): m is typeof m & { modelType?: KnownModelType | null } =>
              m.modelType == null || KNOWN_MODEL_TYPES.includes(m.modelType as KnownModelType)
          )
        )
    })
  )
