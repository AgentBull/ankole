// @ts-nocheck
import { ZodFirstPartyTypeKind, type ZodMapDef, type ZodRecordDef, type ZodTypeAny } from 'zod/v3'
...
export function parseRecordDef(
  ...
  if (def.keyType?._def.typeName === ZodFirstPartyTypeKind.ZodString && def.keyType._def.checks?.length) {
    ...
    const { type: _type, ...keyType } = parseBrandedDef(def.keyType._def, refs) as JsonSchema7StringType
