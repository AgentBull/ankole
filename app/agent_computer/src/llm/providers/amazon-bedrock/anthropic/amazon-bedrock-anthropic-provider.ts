...
import { anthropicTools, AnthropicLanguageModel } from '@/llm/providers/anthropic/internal'
import {
...
export function createAmazonBedrockAnthropic(
  ...
  const createChatModel = (modelId: AmazonBedrockAnthropicModelId) =>
    new AnthropicLanguageModel(modelId, {
      ...
      buildRequestUrl: (baseURL, isStreaming) =>
        `${baseURL}/model/${encodeURIComponent(modelId)}/${isStreaming ? 'invoke-with-response-stream' : 'invoke'}`,

      transformRequestBody: (args, betas) => {
        const { model: _model, stream: _stream, tool_choice, tools, ...rest } = args
        ...
        const transformedTools = tools?.map((tool: Record<string, unknown>) => {
          ...
          if (toolType && toolType in BEDROCK_TOOL_VERSION_MAP) {
            const newType = BEDROCK_TOOL_VERSION_MAP[toolType as keyof typeof BEDROCK_TOOL_VERSION_MAP]
  ...
  const provider = function (modelId: AmazonBedrockAnthropicModelId) {
    if (new.target) {
      throw new Error('The Bedrock Anthropic model function cannot be called with the new keyword.')
    }
