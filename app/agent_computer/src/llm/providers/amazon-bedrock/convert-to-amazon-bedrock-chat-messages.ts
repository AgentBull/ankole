...
function getCachePoint(providerMetadata: SharedV4ProviderMetadata | undefined): AmazonBedrockCachePoint | undefined {
  const cachePointConfig = (providerMetadata?.amazonBedrock?.cachePoint ?? providerMetadata?.bedrock?.cachePoint) as
...
function pushCachePoint(
  content: AmazonBedrockUserMessage['content'] | AmazonBedrockAssistantMessage['content'],
...
async function shouldEnableCitations(providerMetadata: SharedV4ProviderMetadata | undefined): Promise<boolean> {
...
export async function convertToAmazonBedrockChatMessages(
  ...
  const system: AmazonBedrockSystemMessages = []
                      ...
                      case 'text': {
                        ...
                        amazonBedrockContent.push({
                          document: {
                            ...
                            source: {
                              bytes: convertToBase64(new TextEncoder().encode(part.data.text))
                            },
                            ...(enableCitations && {
              ...
              for (const part of content) {
                ...
                switch (output.type) {
                      ...
                      output.value.map(async contentPart => {
                        switch (contentPart.type) {
                          ...
                          case 'file': {
                            ...
                            if (getTopLevelMediaType(fullMediaType) !== 'image') {
                              const enableCitations = await shouldEnableCitations(contentPart.providerOptions)
                  ...
                  case 'execution-denied':
                    toolResultContent = [{ text: output.reason ?? 'Tool call execution denied.' }]
                    break
                  case 'json':
                  case 'error-json':
                  default:
...
function getAmazonBedrockImageFormat(mimeType: string): AmazonBedrockImageFormat {
  const format = BEDROCK_IMAGE_MIME_TYPES[mimeType as AmazonBedrockImageMimeType]
  if (!format) {
    throw new UnsupportedFunctionalityError({
...
function getAmazonBedrockDocumentFormat(mimeType: string): AmazonBedrockDocumentFormat {
  const format = BEDROCK_DOCUMENT_MIME_TYPES[mimeType as AmazonBedrockDocumentMimeType]
  if (!format) {
    throw new UnsupportedFunctionalityError({
...
function trimIfLast(isLastBlock: boolean, isLastMessage: boolean, isLastContentPart: boolean, text: string) {
...
function groupIntoBlocks(prompt: LanguageModelV4Prompt): Array<SystemBlock | AssistantBlock | UserBlock> {
