...
import { convertOpenAICompletionUsage, type OpenAICompletionUsage } from './convert-openai-completion-usage'
...
export class OpenAICompletionLanguageModel implements LanguageModelV4 {
  ...
  static [WORKFLOW_DESERIALIZE](options: { modelId: OpenAICompletionModelId; config: OpenAICompletionConfig }) {
  ...
  async doStream(options: LanguageModelV4CallOptions): Promise<LanguageModelV4StreamResult> {
    ...
    const result = {
      stream: checkedResponse.pipeThrough(
        new TransformStream<ParseResult<OpenAICompletionChunk>, LanguageModelV4StreamPart>({
          start(controller) {
