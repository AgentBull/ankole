...
import { resolveProviderReference, validateTypes, type ToolNameMapping } from '@/llm/provider-utils'
...
import type { OpenAIResponsesFunctionTool, OpenAIResponsesTool } from './openai-responses-api'
...
export async function prepareResponsesTools({
  ...
  const namespaceTools = new Map<string, Extract<OpenAIResponsesTool, { type: 'namespace' }>>()
  const resolvedCustomProviderToolNames = customProviderToolNames ?? new Set<string>()

  for (const tool of tools) {
    switch (tool.type) {
      case 'function': {
        const openaiOptions = tool.providerOptions?.openai as OpenAIToolOptions | undefined
...
function mapShellEnvironment(environment: {
  type?: string
  [key: string]: unknown
}): NonNullable<Extract<OpenAIResponsesTool, { type: 'shell' }>['environment']> {
