// @ts-nocheck
import { TooManyEmbeddingValuesForCallError, type EmbeddingModelV4 } from '@/llm/provider'
import {
  combineHeaders,
...
import { googleEmbeddingModelOptions, type GoogleEmbeddingModelId } from './google-embedding-model-options'
...
export class GoogleEmbeddingModel implements EmbeddingModelV4 {
  ...
  static [WORKFLOW_DESERIALIZE](options: { modelId: string; config: GoogleEmbeddingConfig }) {
  ...
  async doEmbed({
    ...
    abortSignal,
    providerOptions
  }: Parameters<EmbeddingModelV4['doEmbed']>[0]): Promise<Awaited<ReturnType<EmbeddingModelV4['doEmbed']>>> {
    ...
    const mergedHeaders = combineHeaders(this.config.headers ? await resolve(this.config.headers) : undefined, headers)
