...
export type GoogleInteractionsMediaResolution = 'low' | 'medium' | 'high' | 'ultra_high'
      ...
      case 'assistant': {
        ...
        for (const part of message.content) {
          if (part.type === 'text') {
            ...
            flushModelOutput()
            const signature = part.providerOptions?.google?.signature as string | undefined
            steps.push({
              type: 'thought',
              ...(signature != null ? { signature } : {}),
              summary: part.text.length > 0 ? [{ type: 'text', text: part.text }] : undefined
            })
          } else if (part.type === 'file') {
            const fileBlock = convertFilePartToContent({
              part,
      ...
      case 'tool': {
        ...
        for (const part of message.content) {
          ...
          const block = convertToolResultPart({
            ...
            output: part.output,
            signature: part.providerOptions?.google?.signature as string | undefined,
            warnings
          })
          content.push(block)
        }
...
function convertFilePartToContent({
  ...
  const resolutionField =
    mediaResolution != null && (kind === 'image' || kind === 'video') ? { resolution: mediaResolution } : {}
  ...
  for (const message of prompt) {
    if (message.role === 'assistant') {
      const matchesLinkedInteraction = message.content.some(part => {
        const partInteractionId = (part as { providerOptions?: { google?: { interactionId?: string } } })
...
function convertToolResultPart({
  ...
  switch (output.type) {
    ...
    case 'content': {
      const blocks: Array<GoogleInteractionsTextContent | GoogleInteractionsImageContent> = []
