// @ts-nocheck
import type { JSONValue, LanguageModelV4Content } from '@/llm/provider'
...
export function parseGoogleInteractionsOutputs({
  ...
  for (const step of steps) {
    ...
    switch (type) {
      ...
      case 'model_output': {
        const blocks = (step as { content?: Array<{ type?: string; [k: string]: unknown }> }).content ?? []
      ...
      default: {
        if (BUILTIN_TOOL_CALL_TYPES.has(type)) {
          ...
          const toolName =
            type === 'mcp_server_tool_call' ? (call.name ?? 'mcp_server_tool') : builtinToolNameFromCallType(type)
          ...
          const toolName =
            type === 'mcp_server_tool_result' ? (result.name ?? 'mcp_server_tool') : builtinToolNameFromResultType(type)
          content.push({
            type: 'tool-result',
          ...
          const sources = builtinToolResultToSources({
            block: step as unknown as GoogleInteractionsBuiltinToolResultContent,
            generateId
          })
          for (const source of sources) {
            content.push(source)
