...
import { googleInteractionsEventSchema, googleInteractionsResponseSchema } from './google-interactions-api'
import {
...
import { isTerminalStatus, pollGoogleInteractionUntilTerminal } from './poll-google-interactions'
...
export type GoogleInteractionsModelInput = GoogleInteractionsModelId | { agent: string } | { managedAgent: string }
...
export class GoogleInteractionsLanguageModel implements LanguageModelV4 {
  ...
  static [WORKFLOW_DESERIALIZE](options: { modelId: string; agent?: string; config: GoogleInteractionsConfig }) {
  ...
  get supportedUrls() {
    ...
    return {
      ...
      'audio/*': [/^https?:\/\/.+/],
      'video/*': [/^https?:\/\/(www\.)?youtube\.com\/watch\?v=.+/, /^https?:\/\/youtu\.be\/.+/, /^gs:\/\/.+/]
    }
  }
  ...
  private async getArgs(options: LanguageModelV4CallOptions) {
    ...
    const optionSystemInstruction = googleOptions?.systemInstruction ?? undefined
    ...
    if (isAgent) {
      ...
      if (googleOptions?.thinkingLevel != null) droppedFields.push('thinkingLevel')
      ...
      generationConfig = pruneUndefined({
        ...
        stop_sequences:
          options.stopSequences != null && options.stopSequences.length > 0 ? options.stopSequences : undefined,
    ...
    if (isAgent && googleOptions?.agentConfig != null) {
      ...
      if (agentConfigOptions.type === 'deep-research') {
        agentConfig = pruneUndefined({
          ...
          collaborative_planning: agentConfigOptions.collaborativePlanning ?? undefined
    ...
    if (googleOptions?.environment != null) {
      if (!isAgent) {
        ...
        const sources: Array<GoogleInteractionsEnvironmentSource> | undefined = environmentOptions.sources?.map(
          source => {
    ...
    const args: GoogleInteractionsRequestBody = pruneUndefined({
      ...
      tools: toolsForBody,
      response_format: responseFormatEntries.length > 0 ? responseFormatEntries : undefined,
      response_modalities:
        googleOptions?.responseModalities != null
          ? (googleOptions.responseModalities as Array<'text' | 'image' | 'audio' | 'video' | 'document'>)
          : undefined,
      previous_interaction_id: googleOptions?.previousInteractionId ?? undefined,
      service_tier: googleOptions?.serviceTier ?? undefined,
      store: googleOptions?.store ?? undefined,
      generation_config:
        generationConfig != null && Object.keys(generationConfig).length > 0 ? generationConfig : undefined,
  ...
  async doGenerate(options: LanguageModelV4CallOptions): Promise<LanguageModelV4GenerateResult> {
     ...
     */
    const interactionId = typeof response.id === 'string' && response.id.length > 0 ? response.id : undefined
     ...
     */
    const serviceTier = response.service_tier ?? responseHeaders?.['x-gemini-service-tier'] ?? undefined
