// @ts-nocheck
import { AISDKError, type Experimental_VideoModelV4, type SharedV4Warning } from '@/llm/provider'
import {
  combineHeaders,
...
import { googleVideoModelOptionsSchema, type GoogleVideoModelOptions } from './google-video-model-options'
  ...
  async doGenerate(
    ...
    if (options.image != null) {
      if (options.image.type === 'url') {
        ...
        const base64Data =
          typeof options.image.data === 'string' ? options.image.data : convertUint8ArrayToBase64(options.image.data)
    ...
    if (googleOptions != null) {
      ...
      if (opts.personGeneration !== undefined && opts.personGeneration !== null) {
      ...
      for (const [key, value] of Object.entries(opts)) {
        if (
          !['pollIntervalMs', 'pollTimeoutMs', 'personGeneration', 'negativePrompt', 'referenceImages'].includes(key)
        ) {
