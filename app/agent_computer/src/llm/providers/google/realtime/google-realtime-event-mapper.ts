...
async function serializeFunctionCallOutput(item: RealtimeModelV4FunctionCallOutput): Promise<unknown> {
...
export function buildGoogleSessionConfig(
  ...
  if (config?.outputModalities != null) {
    generationConfig.responseModalities = config.outputModalities.map(m => m.toUpperCase())
  } else {
