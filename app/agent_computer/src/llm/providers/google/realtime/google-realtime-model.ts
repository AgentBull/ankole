...
import type { FetchFunction } from '@/llm/provider-utils'
import { GoogleRealtimeEventMapper, buildGoogleSessionConfig } from './google-realtime-event-mapper'
...
export class GoogleRealtimeModel implements RealtimeModelV4 {
  ...
  async doCreateClientSecret(options: RealtimeModelV4ClientSecretOptions): Promise<RealtimeModelV4ClientSecretResult> {
    const fetchFn = this.config.fetch ?? fetch
    const headers = this.config.headers()
    const apiKey = headers['x-goog-api-key']

    if (!apiKey) {
      throw new Error('Google Generative AI API key is required for realtime token creation.')
    }
    ...
    const expireTime = new Date(now + openWindowMs + 30 * 60 * 1000).toISOString()
    ...
    return {
      ...
      expiresAt: data.expireTime ? Math.floor(new Date(data.expireTime).getTime() / 1000) : undefined
    }
  }
  ...
  serializeClientEvent(event: RealtimeModelV4ClientEvent): ReturnType<RealtimeModelV4['serializeClientEvent']> {
    return this.mapper.serializeClientEvent(event, this.modelId)
  }

  buildSessionConfig(config: RealtimeModelV4SessionConfig): Record<string, unknown> {
