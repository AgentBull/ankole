...
import type { AnthropicMessageMetadata, AnthropicUsageIteration } from './anthropic-message-metadata'
import {
...
import { convertAnthropicUsage, type AnthropicUsage } from './convert-anthropic-usage'
...
export class AnthropicLanguageModel implements LanguageModelV4 {
  ...
  static [WORKFLOW_DESERIALIZE](options: { modelId: AnthropicModelId; config: AnthropicLanguageModelConfig }) {
  ...
  private async getArgs({
    ...
    // Merge options
    const anthropicOptions = Object.assign({}, canonicalOptions ?? {}, customProviderOptions ?? {})
    ...
    const supportsStrictTools = (this.config.supportsStrictTools ?? true) && modelSupportsStructuredOutput

    const structureOutputMode = anthropicOptions?.structuredOutputMode ?? 'auto'
    const useStructuredOutput =
      structureOutputMode === 'outputFormat' || (structureOutputMode === 'auto' && supportsStructuredOutput)
    ...
    if (isCustomReasoning(reasoning) && anthropicOptions?.effort == null) {
      ...
      if (reasoningConfig != null) {
        ...
        if (reasoningConfig.effort != null && anthropicOptions.thinking?.type !== 'disabled') {
    ...
    let thinkingBudget = thinkingType === 'enabled' ? anthropicOptions?.thinking?.budgetTokens : undefined
    const thinkingDisplay = thinkingType === 'adaptive' ? anthropicOptions?.thinking?.display : undefined
    ...
    if (
      ...
      if (
        !tools?.some(
          tool =>
            tool.type === 'provider' &&
            (tool.id === 'anthropic.code_execution_20250825' || tool.id === 'anthropic.code_execution_20260120')
        )
      ) {
        warnings.push({
          type: 'other',
    ...
    const defaultEagerInputStreaming = stream && (anthropicOptions?.toolStreaming ?? true)
    ...
    return {
      ...
      betas: new Set([...betas, ...toolsBetas, ...userSuppliedBetas, ...(anthropicOptions?.anthropicBeta ?? [])]),
  ...
  private async getBetasFromHeaders(requestHeaders: Record<string, string | undefined> | undefined) {
    const configHeaders = this.config.headers ? await resolve(this.config.headers) : undefined
  ...
  private buildRequestUrl(isStreaming: boolean): string {
    return this.config.buildRequestUrl?.(this.config.baseURL, isStreaming) ?? `${this.config.baseURL}/messages`
  }

  private transformRequestBody(args: Record<string, any>, betas: Set<string>): Record<string, any> {
      ...
      providerMetadata: (() => {
        ...
        const anthropicMetadata = {
          ...
          container: response.container
            ...
            : null,
          contextManagement: mapAnthropicResponseContextManagement(response.context_management) ?? null
  ...
  async doStream(options: LanguageModelV4CallOptions): Promise<LanguageModelV4StreamResult> {
    ...
    const contentBlocks: Record<
      ...
      | {
          ...
          caller?: {
            type: 'code_execution_20250825' | 'code_execution_20260120' | 'direct'
            toolId?: string
          }
        }
      | { type: 'text' | 'reasoning' }
    ...
    const transformedStream = response.pipeThrough(
      new TransformStream<ParseResult<InferSchema<typeof anthropicChunkSchema>>, LanguageModelV4StreamPart>({
        start(controller) {
        ...
        transform(chunk, controller) {
          ...
          switch (value.type) {
                ...
                case 'server_tool_use': {
                  if (
                    ...
                    const providerToolName =
                      part.name === 'text_editor_code_execution' || part.name === 'bash_code_execution'
                        ? 'code_execution'
                        : part.name
            ...
            case 'message_start': {
              ...
              usage.cache_creation_input_tokens = value.message.usage.cache_creation_input_tokens ?? 0
...
function hasWebTool20260209WithoutCodeExecution(tools: AnthropicTool[] | undefined): boolean {
  if (!tools) {
    return false
  }
