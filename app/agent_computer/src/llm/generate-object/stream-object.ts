...
import type { CallWarning, FinishReason, LanguageModel } from '../types/language-model'
...
import { asLanguageModelUsage, createNullLanguageModelUsage, type LanguageModelUsage } from '../types/usage'
...
> {
  const {
    ...
    onFinish,
    _internal: { generateId = originalGenerateId, currentDate = () => new Date(), now = originalNow } = {},
    ...settings
  } = options
...
class DefaultStreamObjectResult<PARTIAL, RESULT, ELEMENT_STREAM> implements StreamObjectResult<
  PARTIAL,
  RESULT,
  ELEMENT_STREAM
> {
  ...
  private readonly _request = new DelayedPromise<Omit<LanguageModelRequestMetadata, 'messages'>>()
  private readonly _response = new DelayedPromise<Omit<LanguageModelResponseMetadata, 'messages'>>()
  ...
  constructor({
    ...
    const eventProcessor = new TransformStream<ObjectStreamPart<PARTIAL>, ObjectStreamPart<PARTIAL>>({
    ...
    ;(async () => {
      ...
      const transformer: Transformer<LanguageModelV4StreamPart, ObjectStreamInputPart> = {
        transform: (chunk, controller) => {
          switch (chunk.type) {
            case 'text-delta':
