// @ts-nocheck
import type { JSONValue } from '@/llm/provider'
import {
  createIdGenerator,
  withUserAgentSuffix,
  type FlexibleSchema,
...
>(
  ...
  try {
    ...
    const request: Omit<LanguageModelRequestMetadata, 'messages'> = generateResult.request ?? {}
    const response: Omit<LanguageModelResponseMetadata, 'messages'> = responseData
    ...
    const object = await parseAndValidateObjectResultWithRepair(text, outputStrategy, repairText, {
      response,
      usage,
      finishReason
    })
