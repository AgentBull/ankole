// @ts-nocheck
import { safeParseJSON } from '@/llm/provider-utils'
...
export class RealtimeEventReducer {
  ...
  async reduceServerEvent(
    ...
    switch (event.type) {
      ...
      case 'function-call-arguments-done': {
        ...
        const parsedInput = parseResult.success ? (parseResult.value as Record<string, unknown>) : {}
  ...
  private addInputTranscriptionMessage(state: RealtimeState, itemId: string, transcript: string): RealtimeState {
  ...
  private updateToolPartState(state: RealtimeState, callId: string, result: unknown): RealtimeState {
