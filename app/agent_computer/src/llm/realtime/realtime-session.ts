...
export abstract class AbstractRealtimeSession {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.applyState(this.reducer.setStatus(this.state, 'error'))
      this.onError?.(error instanceof Error ? error : new Error(`Connection failed: ${String(error)}`))
    }
  }
  ...
  private async executeToolCall({
    ...
    try {
      ...
      this.onError?.(error instanceof Error ? error : new Error(`Client tool execution failed: ${String(error)}`))
    }
  }
