...
import { validateTypes, type FlexibleSchema } from '@/llm/provider-utils'
    ...
    message:
      lastMessage?.role === 'assistant'
        ...
        : ({
            ...
            metadata: undefined,
            role: 'assistant',
            parts: [] as UIMessagePart<InferUIMessageData<UI_MESSAGE>, InferUIMessageTools<UI_MESSAGE>>[]
          } as UI_MESSAGE),
...
export function processUIMessageStream<UI_MESSAGE extends UIMessage>({
  ...
  runUpdateMessageJob: (
    job: (options: { state: StreamingUIMessageState<UI_MESSAGE>; write: () => void }) => Promise<void>
  ) => Promise<void>
      ...
      async transform(chunk, controller) {
        await runUpdateMessageJob(async ({ state, write }) => {
          ...
          function getToolInvocationByApprovalId(approvalId: string) {
            ...
            const toolInvocation = toolInvocations.find(invocation => invocation.approval?.id === approvalId)
          ...
          function updateDynamicToolPart(
            ...
            if (part != null) {
              ...
              if (providerMetadata != null) {
                if (options.state === 'output-available' || options.state === 'output-error') {
                  const resultPart = part as Extract<DynamicToolUIPart, { state: 'output-available' | 'output-error' }>
