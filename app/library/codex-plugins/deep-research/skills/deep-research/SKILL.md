...
Call `research_validate_delivery` after writing the report. It checks only that `report/report.md` exists and is non-empty. Repair a missing report in this same session; the tool does not inspect working artifacts or judge research quality.
...
When the runtime announces budget exhaustion, stop collecting and deliver the strongest supportable answer from evidence already collected.
