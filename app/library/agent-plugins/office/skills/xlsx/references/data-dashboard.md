...
→ see xlsx §Shell & Execution Discipline for the baseline (quoting, heredoc for `!`, incremental execution).
...
All `xlsx` requirements apply (→ see xlsx §Requirements for Outputs). Dashboards add these:
...
Inherit the xlsx baseline (`view issues`, formula error queries, `validate`, HTML preview scan): → see xlsx §QA minimum cycle.
...
Cross-sheet `!` trap, chart `anchor` / series immutability after create → see xlsx §Known Issues.
