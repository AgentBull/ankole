...
Shared defaults across all four: reference-list paragraphs use `indent=720 hangingIndent=720` (hanging indent 0.5"); add a live TOC if 3+ Heading1s (→ see docx §Table of Contents); static TOC fallback if recipient cannot recalculate (→ see docx §Report-level recipes (f)).
...
→ Base pitfalls (shell escape, `\$ \t \n` literals, table cell formatting order, `pageBreakBefore` belt-and-suspenders, `shd.fill` / `ind.firstLine` schema-invalid forms, TOC cached values, watermark two-step): see docx §Known Issues & Pitfalls.
...
- **TOC cached rendering / static fallback / shell-escape:** → see docx §TOC delivery step, §Report-level recipes (f), §Shell escape.
