...
```bash
# 先预览请求，不实际执行
/repo/app/library/agent-plugins/lark/skills/lark-approvals/scripts/lark-approvals instances cc \
...
如果只有姓名或邮箱，向当前 Turn 发起人索取稳定的 `open_id`。不要猜测，也不要复用其他用户或其他 Turn 的人员信息。
