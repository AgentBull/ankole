...
```
其中，value 包含的是上传文件后返回的 file code。本地文件按 `SKILL.md` 的“上传审批附件或图片”契约逐个上传，使用响应中的 `data.code`。
...
其中，value 包含的是上传文件后返回的 file code。本地图片按 `SKILL.md` 的“上传审批附件或图片”契约逐个上传，使用响应中的 `data.code`。
