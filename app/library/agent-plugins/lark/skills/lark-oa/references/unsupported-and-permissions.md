...
The OA skill is bot-only. In the pinned `lark-cli` v1.0.69 catalog, every generated Approval method is user-only, so Approval is entirely unsupported by this skill: definitions, instances, creation, cancellation, CC, and decisions are all out of scope. Do not relabel an Approval command with `--as bot` and do not switch identity.
