...
Ankole injects `lark-cli` with the app ID and a refreshed tenant-token file derived from this Agent's available Lark signal binding. The bot owns its cloud operations; it does not borrow the operator's storage identity.
...
- never read, print, persist, or copy the token file;
