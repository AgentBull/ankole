...
The prompt contains the authoritative successful forecast dossier from the same Agent and either an explicit boolean result or an instruction to resolve it under the original rule. Do not create a replacement forecast. Use the supplied forecast directly; copying or archiving it is optional working state.
