...
**Abstract concept → icon name** (verify exact names in the chosen Tabler library with `ls | grep`):

| Concept | Tabler candidate |
|---------|------------------|
...
> For self-evident names (home, user, file, search, arrow, etc.) — just search the chosen Tabler library directly without consulting the table.
