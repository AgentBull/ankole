...
> [!IMPORTANT]
> ## Ankole Worker Workspace Contract
>
> - Inside Ankole Agent Computer, all `projects/...` examples in this skill mean `/workspace/user-files/projects/...`.
> - Project outputs that the user should receive MUST remain under `/workspace/user-files`; do not export deliverables only to `/workspace/temp`.
> - For Lark / Feishu delivery, create the final PPTX under `/workspace/user-files/projects/<project>/exports/` and call `reply_attachment` with that `.pptx` path.
> - Image generation, web image search, and cloud TTS provider variables from `.env.example` are not part of the Ankole integration contract unless explicitly configured later.
...
> Bare names ("academic_defense", "招商银行", "agentbull") do NOT trigger Step 3 even if a matching directory exists in the library. The user must give a path. AI must not "helpfully" resolve a name to a path.
...
Field-level micro-adjustment (e.g. "use the AgentBull brand but primary changed to #FF0000") is **not** part of Step 3 fusion — it flows into Strategist Eight Confirmations e–g as a normal user request.

#### Same-kind multiple paths — conflict resolution

When the user gives two paths of the **same kind** (e.g. `brands/acme` + `brands/contoso`), Step 3 surfaces a conflict prompt before fusing — like resolving a git merge conflict:

```
AI: 你给了两个 brand，检测到段级冲突：
    - Color Scheme（brand A primary vs brand B primary）
    - Typography（brand A stack vs brand B stack）
    - Logo（brand A mark vs brand B mark）
    - Voice & Tone（direct vs friendly）
    - Icon Style（stroke vs filled）

    要 (a) 全部按 brand A / (b) 全部按 brand B / (c) 逐段挑？
```
...
> - brand: `templates/brands/agentbull/` （identity override）
> - layout: `templates/layouts/academic_defense/` （structure override）
> - conflicts resolved: Color Scheme from agentbull（user picked a）
```
