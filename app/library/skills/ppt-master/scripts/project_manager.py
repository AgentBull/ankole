...
def default_project_base_dir() -> Path:
    """Return the default project root for standalone and Ankole worker runs."""
    configured = os.getenv("PPT_MASTER_PROJECTS_DIR")
    if configured:
        return Path(configured)

    workspace_root = os.getenv("ANKOLE_WORKSPACE_ROOT")
    if workspace_root:
        return Path(workspace_root) / "user-files" / "projects"
...
class ProjectManager:
    ...
    def __init__(self, base_dir: str | Path | None = None) -> None:
        self.base_dir = Path(base_dir) if base_dir is not None else default_project_base_dir()
