...
Project-local extracted vector illustrations may be referenced without a library
prefix:
...
def _resolve_in_dir(icon_name: str, icons_dir: Path) -> tuple[Path, float]:
    ...
    if '/' in icon_name:
        ...
        # Un-prefixed names are only for project-local extracted vector assets.
        icon_path = icons_dir / f'{icon_name}.svg'
        base_size = DEFAULT_ICON_BASE_SIZE
...
def resolve_icon_path(icon_name: str, icons_dir: Path, fallback_dir: Path | None = None) -> tuple[Path, float]:
    """
    Resolve icon name to file path and base size, e.g. "tabler-filled/home" →
    icons_dir/tabler-filled/home.svg. Un-prefixed names resolve only to a flat
    project-local asset such as icons_dir/layered_slide_06_ill01.svg.
