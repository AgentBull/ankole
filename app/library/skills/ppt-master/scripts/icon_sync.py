...
Examples:
    python3 scripts/icon_sync.py projects/deck tabler-outline/chart
    python3 scripts/icon_sync.py projects/deck
...
def _split_name(icon_name: str) -> tuple[str, str]:
    """`lib/name` -> (lib, name). Library prefix is required."""
    if "/" not in icon_name:
        raise ValueError(f"icon name must include a library prefix: {icon_name}")
    lib, name = icon_name.split("/", 1)
    if not lib or not name:
        raise ValueError(f"icon name must be in <library>/<name> form: {icon_name}")
    return lib, name
...
def sync_icons(project_path: Path, icon_names: list[str], global_dir: Path = _GLOBAL_ICONS_DIR) -> tuple[list[str], list[str]]:
    ...
    copied: list[str] = []
    missing: list[str] = []
    parsed = [_split_name(raw) for raw in icon_names]

    for lib, name in parsed:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("icons", nargs="+", help="Icon names to copy, e.g. tabler-filled/home")
    return parser
