...
|---|---|
| An explicit brand directory path (e.g. `templates/brands/agentbull/`) | Copy `design_spec.md` + logo files + asset subdirectories into `<project_path>/templates/`; Strategist locks identity segment |
| Bare brand name only ("use AgentBull brand"), brand mention without path, or silence | Skip — same mechanical rule as all template kinds: bare names never trigger |
