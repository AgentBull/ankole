#!/usr/bin/env bash
# GitHub environment detection helper for BullX Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif [ -f "/workspace/temp/.bullx/github.env" ] && grep -q "^GITHUB_TOKEN=" "/workspace/temp/.bullx/github.env" 2>/dev/null; then
