...
This is BullX's built-in wrapper around hamelnb. The implementation is the vendored hamelnb `jupyter_live_kernel.py` helper, not a separate BullX notebook engine.
...
- `terminal` for package installs, starting Jupyter, and long-running server processes.
- This skill when you would normally want a Jupyter notebook or stateful Python REPL.

## Runtime

BullX computer images provide system Python, JupyterLab, ipykernel, and the hamelnb helper dependencies. A per-agent Python environment is optional and only for custom packages or version isolation.
...
AGENT_PYTHON=/workspace/user-files/.bullx/python/bin/python
```
...
Use `terminal(background=true)` for this command so the server persists across tool calls. If using the per-agent env, replace `python` with `"$AGENT_PYTHON"`.
...
The upstream hamelnb script supports `uv run "$SCRIPT" ...` through inline metadata. In BullX, prefer `python "$SCRIPT" ...` because the computer image already owns the required dependencies.
