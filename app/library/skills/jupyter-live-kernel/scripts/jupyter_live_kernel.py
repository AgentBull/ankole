...
from jupyter_server.utils import urlencode_unix_socket, url_path_join
...
class ServerInfo:
    ...
    def root_url(self) -> str:
        return f"{urlencode_unix_socket(self.socket_path)}{url_path_join('/', self.base_url, '/')}"
...
class ServerClient:
    ...
    def _prime(self) -> None:
        ...
        try:
            self._request_once(
                "GET", "", params=params, timeout=self.timeout, expect_json=False
            )
        except CommandError:
            return
    ...
    def _request_once(
        ...
        *,
        ...
        request_path = url_path_join("/", self.server.base_url, path)
        if merged_params:
            request_path = f"{request_path}?{urlencode(merged_params)}"
        ...
        connection = UnixHTTPConnection(self.server.socket_path, timeout)
        try:
            ...
            raise CommandError(
                f"{method} {request_path} over {self.server.socket_path} failed: {exc}"
            ) from exc
        finally:
            connection.close()

        if response.status >= 400:
            snippet = response_body[:400].strip()
            raise HTTPCommandError(
                f"{method} {request_path} over {self.server.socket_path} failed with {response.status}: {snippet}",
        ...
        if expect_json:
            try:
                ...
                raise CommandError(
                    f"{method} {request_path} over {self.server.socket_path} returned invalid JSON."
                ) from exc
        return response_body

    def _store_response_cookies(self, response: http.client.HTTPResponse) -> None:
...
def _server_from_raw(raw: dict[str, Any]) -> ServerInfo | None:
    socket_path = raw.get("sock")
    if not socket_path:
        return None
    return ServerInfo(
...
def _server_from_explicit(socket_path: str) -> ServerInfo:
    root_dir = (
...
def _running_server_infos() -> list[ServerInfo]:
    return [
        server
        for raw in list_running_servers()
        if (server := _server_from_raw(raw)) is not None
    ]
...
def _dedupe_servers(servers: list[ServerInfo]) -> list[ServerInfo]:
    ...
    for server in servers:
        if server.socket_path in seen:
            continue
        seen.add(server.socket_path)
...
def discover_servers(timeout: float = DEFAULT_TIMEOUT) -> list[dict[str, Any]]:
    ...
    discovered.sort(
        key=lambda item: (not item["probe"]["reachable"], item["server"]["url"])
    )
    return discovered
...
def _select_server(
    *,
    ...
    if socket_path:
        ...
        candidates.append(_server_from_explicit(socket_path))
        for server in _dedupe_servers(candidates):
            ...
            if probe.reachable and not probe.auth_ok:
                raise CommandError(
                    f"Jupyter server matched socket {socket_path}, but authentication failed. "
                    "Use a server with working auth or refresh the server token."
                )
        raise CommandError(f"No reachable Jupyter server matched socket {socket_path}.")
    ...
    if not reachable_raw:
        raise CommandError(
            "No reachable Unix-socket Jupyter servers with working auth were discovered."
        )
    if len(reachable_raw) > 1:
        sockets = ", ".join(server.socket_path for server in reachable_raw)
        raise CommandError(
            "Multiple reachable Unix-socket Jupyter servers were discovered. "
            f"Pass --socket. Candidates: {sockets}"
        )
    return reachable_raw[0]


def list_sessions(
    server: ServerInfo, timeout: float = DEFAULT_TIMEOUT
) -> list[dict[str, Any]]:
    client = ServerClient(server, timeout=timeout)
...
def combined_open_notebooks(
    server: ServerInfo, timeout: float = DEFAULT_TIMEOUT
) -> dict[str, Any]:
    sessions = [
        item
        for item in list_sessions(server, timeout=timeout)
    ...
    return {
        ...
        "sessions": sessions,
        "workspaces": [
            {"id": ws["id"], "notebooks": ws["notebooks"]} for ws in workspaces
        ],
...
def get_contents(
    ...
    *,
    ...
    for index, cell in enumerate(content.get("cells", [])):
        cells.append(
            _summarize_cell(cell, index=index, include_outputs=include_outputs)
        )
...
def _load_notebook_model(
    server: ServerInfo, path: str, timeout: float = DEFAULT_TIMEOUT
) -> dict[str, Any]:
    client = ServerClient(server, timeout=timeout)
    encoded_path = quote(path, safe="/")
...
def _save_notebook_content(
    ...
    if expected_last_modified is not None:
        ...
        if current.get("last_modified") != expected_last_modified:
            raise CommandError(
                "Notebook changed since it was loaded; reload contents and retry the edit."
            )
...
def _synthetic_cell_id(
    cell: dict[str, Any], *, notebook_path: str, cell_index: int
) -> str:
    payload = json.dumps(
        {
            "path": notebook_path,
...
def _ensure_cell_id(
    cell: dict[str, Any], *, notebook_path: str | None, cell_index: int
) -> None:
    if cell.get("id"):
        return
    if notebook_path:
        cell["id"] = _synthetic_cell_id(
            cell, notebook_path=notebook_path, cell_index=cell_index
        )
        return
    cell["id"] = uuid.uuid4().hex


def _normalize_notebook_content(
    content: dict[str, Any], notebook_path: str | None = None
) -> None:
...
def _summarize_cell(
    ...
    *,
    ...
    if include_outputs and cell.get("cell_type") == "code":
        summary["outputs"] = [
            _summarize_output(output) for output in cell.get("outputs", [])
        ]
    return summary
    ...
    *,
    ...
    if index is not None:
        if index < 0 or index >= len(cells):
            raise CommandError(
                f"Cell index {index} is out of range for notebook with {len(cells)} cells."
            )
        return index
    ...
    if cells[resolved_index].get("source", "") == source:
        return {
            ...
            "cell": _summarize_cell(
                cells[resolved_index], index=resolved_index, include_outputs=False
            ),
...
def insert_cell(
    ...
    *,
    ...
    if at_index < 0 or at_index > len(cells):
        raise CommandError(
            f"Insert index {at_index} is out of range for notebook with {len(cells)} cells."
        )
...
def delete_cell(
    ...
    *,
    ...
    return {
        ...
        "changed": True,
        "deleted_cell": _summarize_cell(
            removed, index=resolved_index, include_outputs=False
        ),
        "cell_count": len(cells),
    }
...
def move_cell(
    ...
    *,
    ...
    if to_index < 0 or to_index >= original_count:
        raise CommandError(
            f"Target index {to_index} is out of range for notebook with {original_count} cells."
        )
    ...
    *,
    ...
    for cell_index in target_indexes:
        ...
        cleared_cells.append(
            _summarize_cell(cell, index=cell_index, include_outputs=False)
        )
...
def _save_run_all_outputs(
    ...
    *,
    ...
    for index, (executed_cell, latest_cell) in enumerate(
        zip(executed_cells, latest_cells)
    ):
...
def _ws_url(
    server: ServerInfo, kernel_id: str, *, session_id: str | None = None
) -> str:
    path = url_path_join("/", server.base_url, f"api/kernels/{kernel_id}/channels")
    base = f"ws://localhost{path}"
    params: dict[str, str] = {}
    if server.token:
...
def _execute_via_websocket(
    ...
    *,
    ...
    try:
        ...
        raw_socket.connect(server.socket_path)
        ...
        while time.time() < deadline:
            ...
            raise CommandError(
                "Timed out waiting for kernel execution to finish over the Unix-socket websocket."
            )
    except (
        CommandError,
        OSError,
        ...
        message = _sanitize_error_text(str(exc), server_token=server.token)
        if request_sent:
            message = (
                "Unix-socket websocket execution may already have reached the kernel; do not blindly retry. "
                f"{message}"
            )
...
def _get_kernel_model(
    ...
    *,
    ...
    client = client or ServerClient(server, timeout=timeout)
    return client.request(
        "GET", f"api/kernels/{quote(kernel_id, safe='')}", timeout=timeout
    )


def _wait_for_kernel_idle(
    server: ServerInfo, kernel_id: str, timeout: float
) -> dict[str, Any]:
    client = ServerClient(server, timeout=timeout)
    deadline = time.time() + timeout
    last_state: str | None = None
...
def restart_kernel(
    ...
    *,
    ...
    client.request(
        "POST",
        f"api/kernels/{quote(target.kernel_id, safe='')}/restart",
        timeout=timeout,
    )
...
def _require_notebook_session(target: KernelTarget, feature: str) -> None:
    ...
    if not target.session_id:
        raise CommandError(
            f"{feature} requires a live notebook session, not just a bare kernel id."
        )
...
def _user_expression_value(reply: dict[str, Any], name: str) -> Any:
    ...
    if not expression_result:
        raise CommandError(
            f"Kernel did not return a value for user expression {name!r}."
        )
...
def _python_variable_list_expression(
    *, limit: int, include_private: bool, include_callables: bool
) -> str:
...
def _ensure_python_kernel(target: KernelTarget, feature: str) -> None:
    if target.kernel_name and "python" in target.kernel_name.lower():
        return
    raise CommandError(
        f"{feature} currently supports Python kernels only. Resolved kernel: {target.kernel_name!r}."
    )
    ...
    *,
    ...
    result = _execute_request(
        ...
        request=ExecuteRequest(
            ...
            store_history=False,
            user_expressions={
                "codex_variable": _python_variable_preview_expression(
                    name, max_chars=max_chars
                )
            },
        ),
        timeout=timeout,
...
def _read_text_argument(value: str | None, file_path: str | None, purpose: str) -> str:
    ...
    raise CommandError(
        f"Provide {purpose} with --{purpose}, --{purpose}-file, or stdin."
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Inspect live Jupyter servers and execute code in running kernels."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    servers_parser = subparsers.add_parser(
        "servers", help="List discovered running Jupyter servers."
    )
    ...
    notebooks_parser = subparsers.add_parser(
        "notebooks",
        help="List live notebook sessions plus notebook tabs discovered from JupyterLab workspaces.",
    )
    ...
    contents_parser = subparsers.add_parser(
        "contents", help="Fetch saved notebook contents through the Contents API."
    )
    ...
    execute_parser = subparsers.add_parser(
        "execute", help="Execute code incrementally in a live notebook kernel."
    )
    ...
    execute_parser.add_argument(
        "--save-outputs",
        action="store_true",
        help="Save cell outputs back to the notebook file. Requires --path and --cell-id. Automatically enabled when --cell-id is provided.",
    )
    execute_parser.add_argument(
        "--no-save-outputs",
        action="store_true",
    ...
    execute_parser.add_argument(
        "--cell-id",
        help="Target cell ID. When provided, outputs are automatically saved back to the notebook file unless --no-save-outputs is set.",
    )
    execute_parser.add_argument("--compact", action="store_true")

    restart_parser = subparsers.add_parser(
        "restart", help="Restart a live notebook kernel."
    )
    ...
    restart_run_all_parser.add_argument(
        "--timeout", type=float, default=DEFAULT_EXEC_TIMEOUT
    )
    restart_run_all_parser.add_argument(
        "--save-outputs",
        action="store_true",
    ...
    edit_parser = subparsers.add_parser(
        "edit", help="Edit saved notebook cells through the Contents API."
    )
    ...
    replace_parser = edit_subparsers.add_parser(
        "replace-source", help="Replace the source of an existing cell."
    )
    ...
    insert_parser = edit_subparsers.add_parser(
        "insert", help="Insert a new cell into the notebook."
    )
    ...
    insert_parser.add_argument(
        "--cell-type", choices=["code", "markdown", "raw"], required=True
    )
    ...
    delete_parser = edit_subparsers.add_parser(
        "delete", help="Delete a cell from the notebook."
    )
    _add_cell_selector(delete_parser)
    delete_parser.add_argument("--compact", action="store_true")

    move_parser = edit_subparsers.add_parser(
        "move", help="Move a cell to a different index."
    )
    ...
    clear_outputs_group = clear_outputs_parser.add_mutually_exclusive_group(
        required=False
    )
    ...
    variables_subparsers = variables_parser.add_subparsers(
        dest="variables_command", required=True
    )

    variables_list_parser = variables_subparsers.add_parser(
        "list", help="List live variables in the kernel."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        server = _select_server(socket_path=args.socket_path, timeout=args.timeout)
        ...
        if args.command == "execute":
            if (
                ...
                if matched_source is None:
                    raise CommandError(
                        f"Cell id {args.cell_id!r} not found in {args.path!r}"
                    )
                code = matched_source
            else:
            ...
            _print(
                execute_code(
                    ...
                    code=code,
                    timeout=args.timeout,
                    save_outputs=(args.save_outputs or bool(args.cell_id))
                    and not args.no_save_outputs,
        ...
        if args.command == "variables":
            if args.variables_command == "list":
                ...
                raise CommandError(
                    f"Unknown variables command {args.variables_command!r}."
                )
            _print(result, args.compact)
            return 0
    except CommandError as exc:
        ...
        print(
            json.dumps(
                {"error": _sanitize_error_text(str(exc), server_token=token)}, indent=2
            ),
            file=sys.stderr,
        )
        return 1
