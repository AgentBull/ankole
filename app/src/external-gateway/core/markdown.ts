...
export function rawPayloadToText(raw: string | Record<string, unknown>): string {
 ...
 * Used when a structured table must degrade to plain text.
 */
 ...
 */
export function walkAst<T extends Content | Root>(node: T, visitor: (node: Content) => Content | null): T {
  if ('children' in node && Array.isArray(node.children)) {
...
export abstract class BaseFormatConverter implements FormatConverter {
  ...
  protected renderList(
    ...
    for (const [i, item] of getNodeChildren(node).entries()) {
      ...
      for (const child of getNodeChildren(item)) {
        if (isListNode(child)) {
          lines.push(this.renderList(child, depth + 1, nodeConverter, unorderedBullet))
          continue
        }
   ...
   * - { raw: string | object }: passed through as raw text or stringified fallback
  ...
  renderPostable(message: PostableMessageInput): string {
    ...
    if ('raw' in message) {
      return rawPayloadToText(message.raw)
    }
