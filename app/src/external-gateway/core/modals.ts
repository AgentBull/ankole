...
export function isModalElement(value: unknown): value is ModalElement {
  return typeof value === 'object' && value !== null && 'type' in value && (value as ModalElement).type === 'modal'
}
...
export function filterModalChildren(children: unknown[]): ModalChild[] {
  const validChildren = children.filter(
    (c): c is ModalChild =>
      ...
      c !== null &&
      'type' in c &&
      VALID_MODAL_CHILD_TYPES.includes((c as { type: string }).type as (typeof VALID_MODAL_CHILD_TYPES)[number])
  )
...
export function ExternalSelect(options: ExternalSelectOptions): ExternalSelectElement {
  return {
    type: 'external_select',
    id: options.id,
    initialOption: options.initialOption,
...
export function SelectOption(options: { label: string; value: string; description?: string }): SelectOptionElement {
  return {
    label: options.label,
    value: options.value,
...
export function fromReactModalElement(element: unknown): AnyModalElement | null {
    ...
    case 'Select':
      return Select({
        ...
        options: convertedChildren.filter(
          (c): c is SelectOptionElement => c !== null && 'label' in c && 'value' in c && !('type' in c)
        ),
