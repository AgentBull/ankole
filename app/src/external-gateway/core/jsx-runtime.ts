...
export interface SelectOptionComponent {
  (options: { label: string; value: string; description?: string }): SelectOptionElement
...
function isJSXElement(value: unknown): value is JSXElement {
  return typeof value === 'object' && value !== null && (value as JSXElement).$$typeof === JSX_ELEMENT
}
 ...
 */
function isExternalSelectProps(props: CardJSXProps): props is ExternalSelectProps {
...
function resolveJSXElement(element: JSXElement): AnyCardElement {
  ...
  if (type === Text) {
    ...
    const content =
      processedChildren.length > 0 ? processedChildren.map(String).join('') : String(textProps.children ?? '')
  ...
  if (type === Actions) {
    ...
    return Actions(processedChildren as (ButtonElement | LinkButtonElement | SelectElement | RadioSelectElement)[])
  }
