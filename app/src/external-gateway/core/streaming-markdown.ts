 ...
 *
 * Outputs markdown (not platform text). Append-only consumers can stream this
 * directly; final provider-visible messages are still projected by outbox.
 */
export class StreamingMarkdownRenderer {
   ...
   * - The final provider-visible message replaces weak progress in durable truth.
   */
  getCommittableText(): string {
    ...
    if (text.length > 0 && !text.endsWith('\n')) {
      ...
      const withoutIncompleteLine = lastNewline >= 0 ? text.slice(0, lastNewline + 1) : ''
