import type { CustomEmojiMap, EmojiFormats, EmojiMapConfig, EmojiValue, WellKnownEmoji } from './types'
...
export function convertEmojiPlaceholders(
  text: string,
  platform: 'slack' | 'gchat' | 'teams' | 'discord' | 'messenger' | 'github' | 'linear' | 'whatsapp',
 ...
 * ```
 */
export function createEmoji<T extends Record<string, { slack: string | string[]; gchat: string | string[] }>>(
  customEmoji?: T
