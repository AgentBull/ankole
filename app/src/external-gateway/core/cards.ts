...
export function isCardElement(value: unknown): value is CardElement {
  return typeof value === 'object' && value !== null && 'type' in value && (value as CardElement).type === 'card'
}
 ...
 * ```
 */
export function Text(content: string, options: { style?: TextStyle } = {}): TextElement {
  return {
    type: 'text',
    content,
    style: options.style
    ...
    case 'Actions':
      return Actions(
        convertedChildren.filter(
          (c): c is ButtonElement | LinkButtonElement | SelectElement | RadioSelectElement =>
            c.type === 'button' || c.type === 'link-button' || c.type === 'select' || c.type === 'radio_select'
        )
      )
