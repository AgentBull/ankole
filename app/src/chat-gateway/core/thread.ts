...
type ThreadImplConfig = ThreadImplConfigWithAdapter;
  ...
  implements Thread<TState>
{
  ...
  private get _stateAdapter(): StateAdapter {
    ...
    throw new Error("Thread has no state store configured");
  }
  ...
  async post(
    ...
    await this.projectSentMessage(result);
  ...
  async postEphemeral(
    ...
    if (this.adapter.openDM && this.adapter.capabilities?.outbound?.includes("post_message")) {
      ...
      const result = await postMessage(dmThreadId, postable);
      return {
        id: result.id,
        threadId: dmThreadId,
        usedFallback: true,
  ...
  /**
   * Handle streaming from a BullX async output stream.
   *
   * Model SDK wire formats must be translated before they reach Chat Gateway.
   * Core accepts plain text chunks and BullX `StreamChunk` objects only.
   */
  private async handleStream(
    rawStream: AsyncIterable<string | StreamChunk>,
    callerOptions?: StreamOptions
  ): Promise<SentMessage> {
    // Normalize plain strings and BullX structured chunks.
    const textStream = normalizeBullXStream(rawStream);
    ...
    // Use adapter-provided streaming only when the channel declares support.
    if (this.adapter.capabilities?.outbound?.includes("streaming") && this.adapter.stream) {
  ...
  private async fallbackStream(
    ...
    const editMessage = requireOutboundCapability(
      this.adapter,
      "edit_message",
      this.adapter.editMessage?.bind(this.adapter)
    );
    let msg: { id: string; threadId: string; raw: unknown } | null =
      placeholderText === null
        ? null
        : await postMessage(this.id, placeholderText);
    ...
    const doEditAndReschedule = async (): Promise<void> => {
      ...
      if (content.trim() && content !== lastEditContent) {
        try {
          await editMessage(threadIdForEdits, msg.id, {
            markdown: content,
          });
    ...
    if (finalContent.trim() && finalContent !== lastEditContent) {
      const raw = await editMessage(threadIdForEdits, msg.id, {
        markdown: accumulated,
      });
      msg = { id: raw.id, threadId: raw.threadId || threadIdForEdits, raw: raw.raw };
    }
  ...
  async refresh(): Promise<void> {
    if (!this.adapter.fetchMessages) {
      throw new NotImplementedError("Fetching thread messages is not supported by this adapter", "fetch_messages");
    }
   ...
   *
   * Reconstructs a ThreadImpl from serialized data with explicit runtime
   * dependencies. BullX does not use a process-global Chat singleton, so callers
   * must provide the owning adapter and PostgreSQL state store.
   *
   * @param json - Serialized thread data
   * @param adapter - Adapter named by `json.adapterName`
   * @param stateAdapter - BullX Chat Gateway state store
   */
   ...
   *
   * Workflow serde cannot provide the owning adapter or state store. Use
   * `chat.reviver()` for JSON payloads that need live Thread handles.
   */
  static [WORKFLOW_DESERIALIZE](data: SerializedThread): ThreadImpl {
    throw new Error(
      `Cannot deserialize ThreadImpl "${data.id}" without explicit Chat Gateway runtime context`
    );
  }
  ...
  private createSentMessage(
    ...
    const sentMessage: SentMessage = {
      ...
      async edit(
        ...
        const rawMessage = await editMessage(threadId, messageId, postable);
        const edited = self.createSentMessage(messageId, postable, rawMessage.threadId || threadId, rawMessage.raw);
      ...
      async addReaction(emoji: string): Promise<void> {
        ...
        await addReaction(threadId, messageId, emoji);
      },
      ...
      async removeReaction(emoji: string): Promise<void> {
        ...
        await removeReaction(threadId, messageId, emoji);
      },
    };
  ...
  private async projectSentMessage(message: Omit<Message, "subject">): Promise<void> {
    await chatGatewayProjectionSink.projectMessage({
      thread: this.threadProjectionFor(message.threadId),
      message,
    });
  }
  ...
  private async projectSentDelete(threadId: string, messageId: string): Promise<void> {
    await chatGatewayProjectionSink.projectDelete({
      thread: this.threadProjectionFor(threadId),
      messageId,
    });
  }

  private threadProjectionFor(threadId: string): ThreadImpl<TState> {
    if (threadId === this.id) return this;
...
function extractMessageContent(message: AdapterPostableMessage): {
  ...
  if ("raw" in message) {
    // Raw payloads are passed through to adapters; this fallback is for history/plain text.
