import { decodeCallbackValue, postToCallbackUrl, resolveCallbackUrl } from './callback-url'
...
import { createBullXChatStateStore } from './state-postgres'
 ...
 *   },
 *   stateKeyPrefix: "bullx-agent:agent_uid",
 * });
 *
...
> implements ChatInstance {
  ...
  private readonly _concurrencyConfig: Required<Omit<ConcurrencyConfig, 'strategy'>>
  private readonly _concurrentSlots = new Map<string, { inFlight: number; waiters: Array<() => void> }>()
  ...
  private readonly ambientMessageHandlers: AmbientMessageHandler<TState>[] = []
  ...
  private readonly messageEditedHandlers: MessageEditedHandler[] = []
   ...
   * @example
   * chat.webhooks.slack(request, { backgroundTask: runInBackground });
   */
  readonly webhooks: Webhooks<TAdapters>

  constructor(config: ChatConfig<TAdapters>) {
    this.userName = config.userName
    this._stateAdapter = createBullXChatStateStore({ keyPrefix: config.stateKeyPrefix })
    this.adapters = new Map()
    this._streamingUpdateIntervalMs = config.streamingUpdateIntervalMs ?? 500
    this._fallbackStreamingPlaceholderText =
      config.fallbackStreamingPlaceholderText !== undefined ? config.fallbackStreamingPlaceholderText : '...'
    ...
    if (concurrency) {
      if (typeof concurrency === 'string') {
        ...
        if (concurrency.maxConcurrent !== undefined && concurrency.maxConcurrent < 1) {
          throw new Error(`concurrency.maxConcurrent must be >= 1 (got ${concurrency.maxConcurrent})`)
        }
        if (concurrency.maxConcurrent !== undefined && concurrency.strategy !== 'concurrent') {
          this.logger.warn(
    ...
    this._threadHistory = new ThreadHistoryCache(this._stateAdapter, config.threadHistory ?? config.messageHistory)
    ...
    if (config.transcripts) {
      ...
      this._transcripts = new TranscriptsApiImpl(this._stateAdapter, config.transcripts)
    } else {
      this._identity = config.identity
    }
    ...
    for (const [name, adapter] of Object.entries(config.adapters)) {
      ...
      webhooks[name] = (request: Request, options?: WebhookOptions) => this.handleWebhook(name, request, options)
    }
   ...
   */
  private async handleWebhook(adapterName: string, request: Request, options?: WebhookOptions): Promise<Response> {
  ...
  /**
   * Register a handler for non-addressed group messages.
   *
   * Ambient messages are ordinary messages that reached the adapter but did not
   * directly address the bot: they are not DMs and do not @-mention the bot.
   * BullX uses this path to mirror group context without replying.
   */
  onAmbientMessage(handler: AmbientMessageHandler<TState>): void {
    this.ambientMessageHandlers.push(handler)
    this.logger.debug('Registered ambient message handler')
  }

  /**
   * Register a handler for messages in subscribed threads.
   *
   * Called for follow-up messages in threads that have been subscribed via
   * `thread.subscribe()` when no ambient handler claims the message first.
   *
   * Does NOT fire for:
   * - The message that triggered the subscription (e.g., the initial @mention)
   * - Messages sent by the bot itself
   * - Non-addressed group messages when `onAmbientMessage()` handlers are registered
   *
   * @example
   * ```typescript
   * chat.onSubscribedMessage(async (thread, message) => {
   *   // Handle addressed follow-up messages
   *   if (message.isMention) {
  ...
  /**
   * Register a handler for message edit lifecycle events.
   */
  onMessageEdited(handler: MessageEditedHandler): void {
    this.messageEditedHandlers.push(handler)
    this.logger.debug('Registered message edited handler')
  }

  /**
   * Register a handler for message delete/recall lifecycle events.
   */
  ...
  onReaction(emojiOrHandler: EmojiFilter[] | ReactionHandler, handler?: ReactionHandler): void {
  ...
  onAction(actionIdOrHandler: string | string[] | ActionHandler, handler?: ActionHandler): void {
  ...
  onOptionsLoad(actionIdOrHandler: string | string[] | OptionsLoadHandler, handler?: OptionsLoadHandler): void {
    if (typeof actionIdOrHandler === 'function') {
  ...
  onModalSubmit(callbackIdOrHandler: string | string[] | ModalSubmitHandler, handler?: ModalSubmitHandler): void {
  ...
  onModalClose(callbackIdOrHandler: string | string[] | ModalCloseHandler, handler?: ModalCloseHandler): void {
    if (typeof callbackIdOrHandler === 'function') {
  ...
  onSlashCommand(
    ...
    if (typeof commandOrHandler === 'function') {
      ...
      const normalizedCommands = commands.map(cmd => (cmd.startsWith('/') ? cmd : `/${cmd}`))
  ...
  /**
   * Get a JSON.parse reviver function bound to this Chat instance.
   *
   * Use this when parsing JSON that contains serialized Thread, Channel, or
   * Message objects. Thread/Channel handles are rebuilt with this instance's
   * explicit adapter binding and PostgreSQL state store instead of a global
   * singleton.
   *
  ...
  reviver(): (key: string, value: unknown) => unknown {
    return createReviver({
      getAdapter: name => this.adapters.get(name),
      getState: () => this._stateAdapter
    })
  }
   ...
   * Handles runInBackground registration and error catching internally.
   * Adapters should call this instead of handleIncomingMessage directly.
   */
  processMessage(
    adapter: Adapter,
    threadId: string,
    messageOrFactory: InboundMessageInput | (() => Promise<InboundMessageInput>),
    options?: WebhookOptions
  ): Promise<void> {
    const task = (async () => {
      const messageInput = typeof messageOrFactory === 'function' ? await messageOrFactory() : messageOrFactory
      const message = this.toMessage(messageInput)
      await this.handleIncomingMessage(adapter, threadId, message)
    })()

    // Track via runInBackground with errors swallowed (existing webhook semantics —
    ...
    if (options?.runInBackground) {
      options.runInBackground(tracked)
    }
  ...
  /**
   * Process an incoming message edit from an adapter.
   *
   * This bypasses ordinary message dedupe because edited messages intentionally
   * reuse the same platform message id.
   */
  processMessageEdited(
    event: Omit<MessageEditedEvent, 'adapter' | 'thread' | 'message'> & {
      adapter: Adapter
      message: InboundMessageInput
    },
    options?: WebhookOptions
  ): Promise<void> {
    const task = this.handleMessageEditedEvent(event)
    const tracked = task.catch(err => {
      this.logger.error('Message edit processing error', {
        error: err,
  ...
  /**
   * Process an incoming message delete/recall from an adapter.
   *
   * This bypasses ordinary message dedupe because delete/recall is lifecycle
   * work for an existing message, not a fresh inbound message.
   */
  processMessageDeleted(
    event: Omit<MessageDeletedEvent, 'adapter' | 'thread' | 'message'> & {
      adapter: Adapter
      message?: InboundMessageInput
    },
    options?: WebhookOptions
  ): Promise<void> {
    const task = this.handleMessageDeletedEvent(event)
    const tracked = task.catch(err => {
  ...
  async processOptionsLoad(event: OptionsLoadEvent, _options?: WebhookOptions): Promise<OptionsLoadResult | undefined> {
    const matchingHandlers = [
      ...this.optionsLoadHandlers.filter(({ actionIds }) => actionIds.length > 0 && actionIds.includes(event.actionId)),
      ...this.optionsLoadHandlers.filter(({ actionIds }) => actionIds.length === 0)
    ]
  ...
  async processModalSubmit(
    event: Omit<ModalSubmitEvent, 'relatedThread' | 'relatedMessage' | 'relatedChannel'>,
    contextId?: string,
    options?: WebhookOptions
  ): Promise<ModalResponse | undefined> {
    const { callbackUrl, relatedThread, relatedMessage, relatedChannel } = await this.retrieveModalContext(
      event.adapter.name,
      contextId
    )
  ...
  processModalClose(
    event: Omit<ModalCloseEvent, 'relatedThread' | 'relatedMessage' | 'relatedChannel'>,
    contextId?: string,
    options?: WebhookOptions
  ): void {
    const task = (async () => {
      const { relatedThread, relatedMessage, relatedChannel } = await this.retrieveModalContext(
        event.adapter.name,
        contextId
      )
    ...
    const fullEvent: SlashCommandEvent<TState> = {
      ...
      openModal: async modal => {
        ...
        if (!(options?.onOpenModal || event.adapter.openModal)) {
          this.logger.warn(`Cannot open modal: ${event.adapter.name} does not support modals`)
          return undefined
        }
        ...
        if (event.triggerId && event.adapter.openModal) {
          return event.adapter.openModal(event.triggerId, modalElement, contextId)
        }
        return undefined
      }
    }
  ...
  private async retrieveModalContext(
    ...
    const adapter = this.adapters.get(adapterName)
    if (!adapter && (stored.thread || stored.channel)) {
      throw new ChatError(`Adapter "${adapterName}" not found for stored modal context`, 'ADAPTER_NOT_FOUND')
    }

    // Reconstruct thread with adapter directly (if present)
    let relatedThread: Thread | undefined
    if (stored.thread) {
      relatedThread = ThreadImpl.fromJSON(stored.thread, adapter!, this._stateAdapter) as Thread
    }
    ...
    if (stored.message && relatedThread) {
      ...
      relatedMessage = (relatedThread as ThreadImpl<TState>).createSentMessageFromMessage(message)
    }

    // Reconstruct channel if present
    let relatedChannel: Channel | undefined
    if (stored.channel) {
      relatedChannel = ChannelImpl.fromJSON(stored.channel, adapter!, this._stateAdapter) as Channel
    }
  ...
  private async handleActionEvent(
    ...
    const thread = event.threadId
      ? await this.createThread(event.adapter, event.threadId, messageForThread, isSubscribed)
      : null
    ...
    const fullEvent: ActionEvent = {
      ...
      openModal: async modal => {
        ...
        const contextId = crypto.randomUUID()
        const channel = thread ? ((thread as ThreadImpl<TState>).channel as ChannelImpl<TState>) : undefined
        await this.storeModalContext(
          event.adapter.name,
          contextId,
   ...
   */
  private async handleMessageEditedEvent(
    event: Omit<MessageEditedEvent, 'adapter' | 'thread' | 'message'> & {
      adapter: Adapter
      message: InboundMessageInput
    }
    ...
    const thread = await this.createThread(event.adapter, event.threadId, message, isSubscribed)

    const fullEvent: MessageEditedEvent = {
      ...event,
      adapter: event.adapter,
      message,
      thread
    ...
    for (const handler of this.messageEditedHandlers) {
      await handler(fullEvent)
    }
  }

  private async handleMessageDeletedEvent(
    event: Omit<MessageDeletedEvent, 'adapter' | 'thread' | 'message'> & {
      adapter: Adapter
      message?: InboundMessageInput
    }
  ): Promise<void> {
    const message = event.message ? this.toMessage(event.message) : undefined
    if (message) setMessageAdapter(message, event.adapter)
    ...
    const fullEvent: MessageDeletedEvent = {
      ...event,
      adapter: event.adapter,
      message,
      thread
  ...
  /**
   * Core owns the concrete Message class. Adapter plugins emit normalized facts
   * as plain data so they do not depend on app-local vendored implementation
   * details; all inbound lifecycle paths become canonical here.
   */
  private toMessage<TRawMessage>(input: InboundMessageInput<TRawMessage>): Message<TRawMessage> {
  ...
  private async handleReactionEvent(
    ...
    const isSubscribed = await this._stateAdapter.isSubscribed(event.threadId)
    const thread = await this.createThread(event.adapter, event.threadId, event.message, isSubscribed)
    ...
    for (const { emoji: emojiFilter, handler } of this.reactionHandlers) {
      ...
      this.logger.debug('Reaction filter check', {
        filterEmoji: emojiFilter.map(e => (typeof e === 'string' ? e : e.name)),
  ...
  /**
   * Open a direct message conversation through an explicit channel adapter.
   *
   * BullX must choose proactive/outbound channels from agent channel bindings
   * and Principal external identities. Guessing the adapter from a raw user id
   * is ambiguous across Slack, Teams, GitHub, Lark, and future platforms.
   */
  async openDM(adapterName: keyof TAdapters & string, user: string | Author): Promise<Thread<TState>> {
    const userId = typeof user === 'string' ? user : user.userId
    const adapter = this.adapters.get(adapterName)
    if (!adapter) {
      throw new ChatError(`Adapter "${adapterName}" not found`, 'ADAPTER_NOT_FOUND')
    }
    if (!adapter.openDM) {
      throw new ChatError(`Adapter "${adapter.name}" does not support openDM`, 'NOT_SUPPORTED')
    }
  ...
  /**
   * Look up user information through an explicit channel adapter.
   */
  async getUser(adapterName: keyof TAdapters & string, user: string | Author): Promise<UserInfo | null> {
    const userId = typeof user === 'string' ? user : user.userId
    const adapter = this.adapters.get(adapterName)
    if (!adapter) {
      throw new ChatError(`Adapter "${adapterName}" not found`, 'ADAPTER_NOT_FOUND')
    ...
    if (!adapter.getUser) {
      throw new ChatError(`Adapter "${adapter.name}" does not support getUser`, 'NOT_SUPPORTED')
    }
  ...
  channel(channelId: string): Channel<TState> {
    ...
    if (!adapterName) {
      throw new ChatError(`Invalid channel ID: ${channelId}`, 'INVALID_CHANNEL_ID')
    }

    const adapter = this.adapters.get(adapterName)
    if (!adapter) {
      throw new ChatError(`Adapter "${adapterName}" not found for channel ID "${channelId}"`, 'ADAPTER_NOT_FOUND')
    }
  ...
  thread(threadId: string): Thread<TState> {
    ...
    if (!adapterName) {
      throw new ChatError(`Invalid thread ID: ${threadId}`, 'INVALID_THREAD_ID')
    }

    const adapter = this.adapters.get(adapterName)
    if (!adapter) {
      throw new ChatError(`Adapter "${adapterName}" not found for thread ID "${threadId}"`, 'ADAPTER_NOT_FOUND')
    }
   ...
   */
  private async getLockKey(adapter: Adapter, threadId: string): Promise<string> {
  ...
  async handleIncomingMessage(adapter: Adapter, threadId: string, message: Message): Promise<void> {
    ...
    if (strategy === 'queue' || strategy === 'debounce' || strategy === 'burst') {
      await this.handleQueueOrDebounce(adapter, threadId, lockKey, message, strategy)
      return
    }
   ...
   */
  private async handleDrop(adapter: Adapter, threadId: string, lockKey: string, message: Message): Promise<void> {
    let lock = await this._stateAdapter.acquireLock(lockKey, DEFAULT_LOCK_TTL_MS)
    if (!lock) {
      ...
      if (!lock) {
        ...
        throw new LockError(`Could not acquire lock on thread ${threadId}. Another instance may be processing.`)
      }
    }
  ...
  private async handleQueueOrDebounce(
    ...
    // Try to acquire lock
    const lock = await this._stateAdapter.acquireLock(lockKey, DEFAULT_LOCK_TTL_MS)
    ...
    if (!lock) {
      ...
      this.logger.info(strategy === 'debounce' ? 'message-debounce-reset' : 'message-queued', {
        threadId,
        lockKey,
        messageId: message.id,
   ...
   */
  private async debounceLoop(lock: Lock, adapter: Adapter, threadId: string, lockKey: string): Promise<void> {
   ...
   */
  private async drainQueue(lock: Lock, adapter: Adapter, threadId: string, lockKey: string): Promise<void> {
    while (true) {
   ...
   */
  private async handleConcurrent(adapter: Adapter, threadId: string, message: Message): Promise<void> {
  ...
  private acquireConcurrentSlot(threadId: string, maxConcurrent: number): Promise<void> {
  ...
  private async dispatchToHandlers(
    ...
    // Adapter-provided `false` is platform truth on structured-mention systems
    // like Lark. Only fall back to text matching when the adapter omitted the
    // field entirely.
    ...
    if (this.ambientMessageHandlers.length > 0) {
      this.logger.debug('Ambient group message received - calling handlers', {
        threadId,
        handlerCount: this.ambientMessageHandlers.length
      })
      await this.runHandlers(this.ambientMessageHandlers, thread, message, context)
      return
    }

    // Compatibility path for callers that still use Chat SDK subscriptions
    // without registering ambient handlers.
    if (isSubscribed) {
      this.logger.debug('Message in subscribed thread - calling handlers', {
        threadId,
        handlerCount: this.subscribedMessageHandlers.length
      })
      await this.runHandlers(this.subscribedMessageHandlers, thread, message, context)
      return
    }
  ...
  private createThread(
    ...
    return new ThreadImpl<TState>({
      ...
      threadHistory: adapter.persistThreadHistory || adapter.persistMessageHistory ? this._threadHistory : undefined
    })
  }
  ...
  private detectMention(adapter: Adapter, message: Message): boolean {
    ...
    if (botUserId) {
      ...
      const discordPattern = new RegExp(`<@!?${this.escapeRegex(botUserId)}>`, 'i')
   ...
   */
  private rehydrateMessage(raw: Message | Record<string, unknown>, adapter?: Adapter): Message {
    ...
    if (obj._type === 'chat:Message') {
      ...
      msg = new Message({
        ...
        metadata: {
          dateSent: dateSent instanceof Date ? dateSent : new Date(dateSent as string),
          edited: metadata.edited as boolean,
          editedAt: editedAt
            ? new Date(editedAt instanceof Date ? editedAt.toISOString() : (editedAt as string))
            : undefined
        },
    ...
    if (rehydrate && msg.attachments.length > 0) {
      msg.attachments = msg.attachments.map(att => (att.fetchData ? att : rehydrate(att)))
    }
  ...
  private async runHandlers(
    handlers: Array<(thread: Thread<TState>, message: Message, context?: MessageContext) => void | Promise<void>>,
