...
export interface ChatGatewayProjectMessageInput<TRawMessage = unknown> {
   ...
   */
  message: Omit<Message<TRawMessage>, 'subject'>
}
...
interface NormalizedReactionEvent {
   ...
   *
   * Chat SDK exposes `emoji` as a cross-platform convenience value for handlers,
   * but the IM mirror has to preserve the platform-visible reaction bucket. Lark
...
function normalizeReactionEvent(input: ReactionEvent): NormalizedReactionEvent {
  ...
  // labels and Chat SDK handler semantics.
  return {
    added: input.added,
...
function normalizedEmojiName(emoji: ReactionEvent['emoji']): string {
