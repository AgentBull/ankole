...
export { ChatError, LockError, NotImplementedError, RateLimitError, UnsupportedChannelCapabilityError } from './errors'
 ...
 */
export interface ChatConfig<TAdapters extends Record<string, Adapter> = Record<string, Adapter>> {
  ...
  onLockConflict?:
    | 'force'
    | 'drop'
    | ((threadId: string, message: Message) => 'force' | 'drop' | Promise<'force' | 'drop'>)
  /**
   * Namespace applied to BullX Chat Gateway's PostgreSQL runtime state rows.
   *
   * State storage is intentionally not pluggable anymore: BullX Agent runs on
   * Bun + PostgreSQL, and core owns the lock/queue/cache/subscription store.
   * Runtime creates one Chat instance per agent and passes
   * `bullx-agent:<agent_uid>` here so coordination state never crosses agents.
   */
...
export interface WebhookOptions {
   ...
   */
  onOpenModal?: (modal: ModalElement, contextId: string) => Promise<{ viewId: string } | undefined>
  /**
   * Function to run message handling in the background.
   * Use this when the platform expects a fast webhook response but BullX still
   * needs to finish durable projection, reply reconciliation, or handler work.
   * The host decides whether the task is awaited, detached, or delegated to a
   * runtime-specific background mechanism.
   */
...
export type ChannelInboundCapability =
  | 'message_receive'
  | 'message_edit'
  | 'message_delete'
  | 'message_recall'
...
export type ChannelOutboundCapability =
  | 'post_message'
  | 'edit_message'
  | 'delete_message'
  | 'add_reaction'
...
export type ChannelHistoryCapability =
  | 'fetch_message'
...
/**
 * Capability declaration for one chat channel adapter.
 *
 * Feishu/Lark supports a relatively broad lifecycle surface, but future
 * adapters such as GitHub webhooks may only deliver receive/edit/delete facts.
 * Core must consult this declaration before attempting a side effect so an
 * unsupported action is explicit instead of being guessed from Feishu behavior.
 */
export interface ChannelAdapterCapabilities {
  history?: readonly ChannelHistoryCapability[]
  inbound?: readonly ChannelInboundCapability[]
  outbound?: readonly ChannelOutboundCapability[]
}

/**
 * Normalized inbound message fact emitted by an adapter.
 *
 * Adapters may return a real core {@link Message} when they live inside the app,
 * but plugin adapters should normally return plain data. Core owns the concrete
 * Message class so plugins do not depend on vendored implementation internals.
 */
export type InboundMessageInput<TRawMessage = unknown> = Message<TRawMessage> | MessageData<TRawMessage>
...
export interface Adapter<TThreadId = unknown, TRawMessage = unknown> {
  /** Capabilities this adapter can actually perform or observe. */
  ...
  parseMessage(raw: TRawMessage): InboundMessageInput<TRawMessage>
  /**
...
export interface ChatInstance {
   ...
   * @param options - Webhook options including runInBackground
   */
  processAction(
  ...
  processMessage(
    adapter: Adapter,
    threadId: string,
    message: InboundMessageInput | (() => Promise<InboundMessageInput>),
    options?: WebhookOptions
  ): Promise<void>

  /**
   * Process an inbound message edit from an adapter.
   *
   * Edits reuse the platform message id, so they intentionally bypass ordinary
   * message deduplication and route only to edit lifecycle handlers.
   */
  ...
  /**
   * Process an inbound message delete/recall from an adapter.
   *
   * Deletes are lifecycle events, not new messages. They intentionally bypass
   * ordinary message deduplication and route only to delete lifecycle handlers.
   */
 ...
 * @template TRawMessage - Platform-specific raw message type
 */
export interface Postable<TState = Record<string, unknown>, TRawMessage = unknown> {
 ...
 * @template TRawMessage - Platform-specific raw message type
 */
export interface Channel<TState = Record<string, unknown>, TRawMessage = unknown> extends Postable<
  TState,
  TRawMessage
> {
 ...
 * @template TRawMessage - Platform-specific raw message type
 */
export interface Thread<TState = Record<string, unknown>, TRawMessage = unknown> extends Postable<TState, TRawMessage> {
  /**
   ...
   *
   * Supports text, markdown, cards, and BullX streaming chunks. Provider SDK
   * streams must be translated before they reach Chat Gateway.
   *
   ...
   * await thread.post(agentTextStream);
   *
   * // Stream with options via StreamingPlan PostableObject
   * const stream = new StreamingPlan(agentBullXStream, {
   *   groupTasks: "plan",
...
export interface SentMessage<TRawMessage = unknown> extends Omit<Message<TRawMessage>, 'subject'> {
 ...
 * - `{ raw: string | object }` - Explicit raw payload, passed through as-is
 ...
 * - `AsyncIterable<string | StreamChunk>` - BullX streaming text or structured chunks
 */
...
export interface PostableRaw {
  ...
  /** Files to upload */
  files?: FileUpload[]
  /** Raw text or platform-native payload passed through as-is to the adapter */
...
/**
 * Handler for non-addressed group messages that should be observed but should
 * not automatically trigger an agent reply.
 */
export type AmbientMessageHandler<TState = Record<string, unknown>> = (
  thread: Thread<TState>,
  message: Message,
  context?: MessageContext
) => void | Promise<void>
 ...
 *
 * Called for addressed messages in threads that have been subscribed via
 * `thread.subscribe()`. If ambient handlers are registered, non-addressed group
 * follow-ups are routed to `onAmbientMessage` first so observation does not
 * imply a reply.
 *
 * Does NOT fire for:
...
/**
 * Event fired when a platform reports that a previously observed message was
 * edited.
 */
export interface MessageEditedEvent<TRawMessage = unknown> {
  /** The adapter that received the event */
  adapter: Adapter
  /** Updated normalized message snapshot */
  message: Message<TRawMessage>
  /** The edited message id */
  messageId: string
  /** Platform-specific raw event data */
  raw?: unknown
  /** When the edit happened or was observed */
  editedAt?: Date
  /** The thread where the edit occurred */
...
export interface MessageDeletedEvent<TRawMessage = unknown> {
  /** The adapter that received the event */
  adapter: Adapter
  /** Optional last-known message snapshot */
  message?: Message<TRawMessage>
  /** The deleted/recalled message id */
  messageId: string
  /** Platform-specific raw event data */
  raw?: unknown
  /** When the delete/recall happened or was observed */
  deletedAt?: Date
  /** Distinguishes platform delete from recall where adapters can tell */
  kind?: 'deleted' | 'recalled'
  /** The thread where the delete/recall occurred */
  thread: Thread<TRawMessage>
  /** The thread ID */
  threadId: string
}

export type MessageEditedHandler = (event: MessageEditedEvent) => void | Promise<void>

export type MessageDeletedHandler = (event: MessageDeletedEvent) => void | Promise<void>
...
export type ModalCloseHandler = (event: ModalCloseEvent) => void | Promise<void>
...
export type AssistantThreadStartedHandler = (event: AssistantThreadStartedEvent) => void | Promise<void>
...
export type AssistantContextChangedHandler = (event: AssistantContextChangedEvent) => void | Promise<void>
...
export type AppHomeOpenedHandler = (event: AppHomeOpenedEvent) => void | Promise<void>
...
export type MemberJoinedChannelHandler = (event: MemberJoinedChannelEvent) => void | Promise<void>
 ...
 */
export type IdentityResolver = (context: IdentityContext) => string | null | Promise<string | null>
