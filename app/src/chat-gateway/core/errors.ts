...
export class UnsupportedChannelCapabilityError extends ChatError {
  readonly adapterName: string;
  readonly capability: string;

  constructor(adapterName: string, capability: string, cause?: unknown) {
    super(`Adapter "${adapterName}" does not support channel capability "${capability}"`, "UNSUPPORTED_CHANNEL_CAPABILITY", cause);
