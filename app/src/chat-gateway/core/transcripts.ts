 ...
 *
 * `appendToList(key, tombstone, { maxLength: 1 })` evicts every prior entry in
 * the BullX PostgreSQL state store. The remaining single tombstone is filtered
 * out by `list()` and `count()`, and is pushed out naturally on the next append
 * once `maxPerUser` writes have happened.
 */
