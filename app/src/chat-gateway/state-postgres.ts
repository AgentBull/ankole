...
/**
 * Options for the local Chat SDK PostgreSQL state adapter.
 */
export interface DrizzlePostgresStateAdapterOptions {
  /**
...
/**
 * Drizzle/Bun SQL implementation of Chat SDK's `StateAdapter`.
 *
 * This is local code instead of a package dependency on `@chat-adapter/state-pg`
 * because BullX Agent already owns a Bun SQL + Drizzle database boundary. The
 * table model and atomic semantics follow the official PostgreSQL adapter, but
 * schema creation is handled by Drizzle migrations rather than `connect()`.
 */
export class DrizzlePostgresStateAdapter implements StateAdapter {
  private readonly keyPrefix: string
  private readonly logger: Logger
  private connected = false
  private connectPromise: Promise<void> | null = null

  constructor(options: DrizzlePostgresStateAdapterOptions = {}) {
   ...
   *
   * Unlike the upstream state-pg package, this does not create tables. Missing
   * tables should fail fast as a migration/deployment problem.
   */
  ...
  async appendToList(key: string, value: unknown, options?: { maxLength?: number; ttlMs?: number }): Promise<void> {
    ...
    if (options?.maxLength) // Keep the newest maxLength list entries. This intentionally uses SQL
  ...
  private ensureConnected(): void {
    if (!this.connected) throw new Error('DrizzlePostgresStateAdapter is not connected. Call connect() first.')
  }
}

/**
 * Convenience factory mirroring Chat SDK adapter package naming.
 */
export function createDrizzlePostgresState(
  options: DrizzlePostgresStateAdapterOptions = {}
): DrizzlePostgresStateAdapter {
  return new DrizzlePostgresStateAdapter(options)
}
