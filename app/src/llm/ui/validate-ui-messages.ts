// @ts-nocheck
import { TypeValidationError, type JSONObject } from '@/llm/provider'
import { lazySchema, validateTypes, zodSchema, type FlexibleSchema, type Tool } from '@/llm/provider-utils'
import { z } from 'zod/v4'
import { InvalidArgumentError } from '../error'
import { jsonValueSchema } from '../types/json-value'
import { providerMetadataSchema } from '../types/provider-metadata'
import type { DataUIPart, InferUIMessageData, InferUIMessageTools, ToolUIPart, UIMessage } from './ui-messages'
      ...
      for (const [msgIdx, message] of validatedMessages.entries()) {
        for (const [partIdx, part] of message.parts.entries()) {
          ...
          if (tools && part.type.startsWith('tool-')) {
            const toolPart = part as ToolUIPart<InferUIMessageTools<UI_MESSAGE>>
