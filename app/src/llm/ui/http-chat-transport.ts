// @ts-nocheck
import { normalizeHeaders, resolve, type FetchFunction, type Resolvable } from '@/llm/provider-utils'
...
export abstract class HttpChatTransport<UI_MESSAGE extends UIMessage> implements ChatTransport<UI_MESSAGE> {
  ...
  async sendMessages({ abortSignal, ...options }: Parameters<ChatTransport<UI_MESSAGE>['sendMessages']>[0]) {
    ...
    const headers = preparedRequest?.headers !== undefined ? normalizeHeaders(preparedRequest.headers) : baseHeaders
    const body =
