...
export abstract class AbstractChat<UI_MESSAGE extends UIMessage> {
  ...
  sendMessage = async (
    ...
    if ('text' in message || 'files' in message) {
      ...
      uiMessage = {
        parts: [
          ...fileParts,
          ...('text' in message && message.text != null ? [{ type: 'text' as const, text: message.text }] : [])
        ]
      } as UI_MESSAGE
    } else {
      uiMessage = message
  ...
  addToolApprovalResponse: ChatAddToolApproveResponseFunction = async ({ id, approved, reason, options }) =>
