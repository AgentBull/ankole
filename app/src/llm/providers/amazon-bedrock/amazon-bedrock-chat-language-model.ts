...
import { getModelCapabilities } from '@/llm/providers/anthropic/internal'
...
import { convertAmazonBedrockUsage, type AmazonBedrockUsage } from './convert-amazon-bedrock-usage'
...
export class AmazonBedrockChatLanguageModel implements LanguageModelV4 {
  ...
  static [WORKFLOW_DESERIALIZE](options: { modelId: string; config: AmazonBedrockChatConfig }) {
  ...
  private async getArgs({
    ...
    const jsonResponseTool: LanguageModelV4FunctionTool | undefined =
      responseFormat?.type === 'json' && responseFormat.schema != null && !useNativeStructuredOutput
        ? {
            type: 'function',
            name: 'json',
    ...
    const thinkingBudget = thinkingType === 'enabled' ? amazonBedrockOptions.reasoningConfig?.budgetTokens : undefined
    const thinkingDisplay = thinkingType === 'adaptive' ? amazonBedrockOptions.reasoningConfig?.display : undefined
    ...
    const maxReasoningEffort = amazonBedrockOptions.reasoningConfig?.maxReasoningEffort
    ...
    if (!hasAnyTools) {
      ...
      if (hasToolContent) {
        filteredPrompt = prompt
          ...
          .filter(message => message.role === 'system' || message.content.length > 0) as typeof prompt
    ...
    const { system, messages } = await convertToAmazonBedrockChatMessages(filteredPrompt, isMistral)
  ...
  private async getHeaders({ headers }: { headers: Record<string, string | undefined> | undefined }) {
  ...
  async doGenerate(options: LanguageModelV4CallOptions): Promise<LanguageModelV4GenerateResult> {
    ...
    const { value: response, responseHeaders } = await postJsonToApi({
      ...
      successfulResponseHandler: createJsonResponseHandler(AmazonBedrockResponseSchema),
    ...
    const providerMetadataPayload =
      ...
      stopSequence
        ? {
            ...
            ...((response.usage?.cacheWriteInputTokens != null || response.usage?.cacheDetails != null) && {
              usage: {
    ...
    return {
      content,
      finishReason: {
        unified: mapAmazonBedrockFinishReason(response.stopReason as AmazonBedrockStopReason, isJsonResponseFromTool),
      ...
      response: {
        ...
        timestamp: responseHeaders?.['date'] != null ? new Date(responseHeaders['date']) : undefined,
        modelId: this.modelId,
  ...
  async doStream(options: LanguageModelV4CallOptions): Promise<LanguageModelV4StreamResult> {
    ...
    const { value: response, responseHeaders } = await postJsonToApi({
      ...
      successfulResponseHandler: createAmazonBedrockEventStreamResponseHandler(AmazonBedrockStreamSchema),
      abortSignal: options.abortSignal,
      fetch: this.config.fetch
    })
    ...
    return {
      stream: response.pipeThrough(
        new TransformStream<ParseResult<z.infer<typeof AmazonBedrockStreamSchema>>, LanguageModelV4StreamPart>({
          start(controller) {
          ...
          transform(chunk, controller) {
            ...
            if (value.messageStop) {
              ...
              stopSequence = value.messageStop.additionalModelResponseFields?.delta?.stop_sequence ?? null
            }
            ...
            if (value.metadata) {
              ...
              const cacheUsage =
                value.metadata.usage?.cacheWriteInputTokens != null || value.metadata.usage?.cacheDetails != null
                  ? {
                      usage: {
          ...
          flush(controller) {
            ...
            if (isJsonResponseFromTool || stopSequence != null) {
              const updatePayload = {
                ...(providerMetadata?.amazonBedrock ?? providerMetadata?.bedrock),
                ...(isJsonResponseFromTool && {
                  isJsonResponseFromTool: true
                }),
                stopSequence
...
const AmazonBedrockStopReasonSchema = z.union([z.enum(BEDROCK_STOP_REASONS), z.string()])
...
const AmazonBedrockResponseSchema = z.object({
  ...
  usage: z.object({
    ...
    cacheDetails: z.array(z.object({ inputTokens: z.number(), ttl: z.string() })).nullish()
  })
})
...
const amazonBedrockReasoningEffortMap: Partial<Record<string, 'low' | 'medium' | 'high' | 'max'>> = {
  minimal: 'low',
  low: 'low',
  medium: 'medium',
  high: 'high',
