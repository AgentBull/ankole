...
export function createSigV4FetchFunction(
  getCredentials: () => AmazonBedrockCredentials | PromiseLike<AmazonBedrockCredentials>,
  fetch?: FetchFunction,
  ...
  return async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    const originalHeaders = combineHeaders(normalizeHeaders(request?.headers), normalizeHeaders(init?.headers))
 ...
 */
export function createApiKeyFetchFunction(apiKey: string, fetch?: FetchFunction): FetchFunction {
  return async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    // avoid caching globalThis.fetch in case it is patched by other libraries
    const effectiveFetch = fetch ?? globalThis.fetch
