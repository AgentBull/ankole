...
export class AmazonBedrockEmbeddingModel implements EmbeddingModelV4 {
  ...
  static [WORKFLOW_DESERIALIZE](options: { modelId: string; config: AmazonBedrockEmbeddingConfig }) {
  ...
  async doEmbed({
    ...
    const args = isNovaModel
      ? {
          ...
          singleEmbeddingParams: {
            embeddingPurpose: amazonBedrockOptions.embeddingPurpose ?? 'GENERIC_INDEX',
    ...
    const { value: response, responseHeaders } = await postJsonToApi({
      ...
      successfulResponseHandler: createJsonResponseHandler(AmazonBedrockEmbeddingResponseSchema),
    ...
    const headerTokenCount = Number(responseHeaders?.['x-amzn-bedrock-input-token-count'])
    const tokens =
