// @ts-nocheck
import { convertBase64ToUint8Array, safeParseJSON, type FetchFunction } from '@/llm/provider-utils'
...
export function createAmazonBedrockAnthropicFetch(baseFetch: FetchFunction): FetchFunction {
  return async (url, options) => {
    ...
    if (contentType?.includes('application/vnd.amazon.eventstream') && response.body != null) {
      const transformedBody = transformAmazonBedrockEventStreamToSSE(response.body)
...
function transformAmazonBedrockEventStreamToSSE(body: ReadableStream<Uint8Array>): ReadableStream<Uint8Array> {
  const textEncoder = new TextEncoder()

  return createAmazonBedrockEventStreamDecoder(body, async (event, controller) => {
    if (event.messageType === 'event') {
      if (event.eventType === 'chunk') {
        ...
        if (bytes) {
          const anthropicEvent = new TextDecoder().decode(convertBase64ToUint8Array(bytes))
      ...
      controller.enqueue(textEncoder.encode(`data: ${JSON.stringify({ type: 'error', error: event.data })}\n\n`))
    }
  })
}
