// @ts-nocheck
import { OpenAIChatLanguageModel, OpenAIResponsesLanguageModel } from '@/llm/providers/openai/internal'
import { NoSuchModelError, type LanguageModelV4, type ProviderV4 } from '@/llm/provider'
import {
  loadOptionalSetting,
  loadSetting,
...
import type { BedrockMantleChatModelId, BedrockMantleResponsesModelId } from './bedrock-mantle-options'
 ...
 */
export function createBedrockMantle(options: BedrockMantleProviderSettings = {}): BedrockMantleProvider {
  ...
  const getHeaders = (): Record<string, string | undefined> =>
    withUserAgentSuffix(options.headers ?? {}, `ai-sdk/amazon-bedrock/${VERSION}`)

  const url = ({ path }: { path: string; modelId: string }): string => `${getBaseURL()}${path}`
  ...
  const provider = function (modelId: BedrockMantleChatModelId) {
    if (new.target) {
      throw new Error('The Bedrock Mantle model function cannot be called with the new keyword.')
    }
