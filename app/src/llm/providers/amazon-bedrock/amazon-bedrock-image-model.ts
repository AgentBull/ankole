// @ts-nocheck
import type { ImageModelV4, ImageModelV4File, SharedV4Warning } from '@/llm/provider'
import {
  combineHeaders,
...
import { modelMaxImagesPerCall, type AmazonBedrockImageModelId } from './amazon-bedrock-image-settings'
...
export class AmazonBedrockImageModel implements ImageModelV4 {
  ...
  static [WORKFLOW_DESERIALIZE](options: { modelId: string; config: AmazonBedrockImageModelConfig }) {
  ...
  async doGenerate({
    ...
    const amazonBedrockOptions = (providerOptions?.amazonBedrock ?? providerOptions?.bedrock) as
      | Record<string, any>
      | undefined
    ...
    const imageGenerationConfig = {
      ...
      ...(seed ? { seed } : {}),
      ...(n ? { numberOfImages: n } : {}),
      ...(amazonBedrockOptions?.quality ? { quality: amazonBedrockOptions.quality } : {}),
      ...(amazonBedrockOptions?.cfgScale ? { cfgScale: amazonBedrockOptions.cfgScale } : {})
    }
    ...
    if (hasFiles) {
      ...
      const taskType = amazonBedrockOptions?.taskType ?? (hasMask || hasMaskPrompt ? 'INPAINTING' : 'IMAGE_VARIATION')
      ...
      switch (taskType) {
        case 'INPAINTING': {
          const inPaintingParams: Record<string, unknown> = {
            ...
            ...(prompt ? { text: prompt } : {}),
            ...(amazonBedrockOptions?.negativeText ? { negativeText: amazonBedrockOptions.negativeText } : {})
          }
        ...
        case 'OUTPAINTING': {
          const outPaintingParams: Record<string, unknown> = {
            ...
            ...(prompt ? { text: prompt } : {}),
            ...(amazonBedrockOptions?.negativeText ? { negativeText: amazonBedrockOptions.negativeText } : {}),
            ...(amazonBedrockOptions?.outPaintingMode ? { outPaintingMode: amazonBedrockOptions.outPaintingMode } : {})
          }
    ...
    const { value: response, responseHeaders } = await postJsonToApi({
      ...
      successfulResponseHandler: createJsonResponseHandler(amazonBedrockImageResponseSchema),
      abortSignal,
    ...
    if (!response.images || response.images.length === 0) {
      throw new Error('Amazon Bedrock returned no images. ' + (response.status ? `Status: ${response.status}` : ''))
    }
