...
import { resolveProviderOptionsKey, toCamelCase, warnIfDeprecatedProviderOptionsKey } from '../utils/to-camel-case'
...
export class OpenAICompatibleChatLanguageModel implements LanguageModelV4 {
  ...
  static [WORKFLOW_DESERIALIZE](options: { modelId: string; config: OpenAICompatibleChatConfig }) {
  ...
  private convertUsage(usage: z.infer<typeof openaiCompatibleTokenUsageSchema>): LanguageModelV4Usage {
    ...
    return {
      ...
      args: {
        ...
        reasoning_effort:
          ...
          (isCustomReasoning(reasoning) && reasoning !== 'none' ? reasoning : undefined),
  ...
  async doGenerate(options: LanguageModelV4CallOptions): Promise<LanguageModelV4GenerateResult> {
    ...
    if (completionTokenDetails?.accepted_prediction_tokens != null) {
      providerMetadata[metadataKey].acceptedPredictionTokens = completionTokenDetails?.accepted_prediction_tokens
    }
    if (completionTokenDetails?.rejected_prediction_tokens != null) {
      providerMetadata[metadataKey].rejectedPredictionTokens = completionTokenDetails?.rejected_prediction_tokens
    }
  ...
  async doStream(options: LanguageModelV4CallOptions): Promise<LanguageModelV4StreamResult> {
    ...
    const processToolCallDelta = (toolCallDelta: OpenAICompatibleStreamingToolCallDelta) => {
    ...
    const convertUsage = (usage: z.infer<typeof openaiCompatibleTokenUsageSchema>) => this.convertUsage(usage)

    return {
      stream: response.pipeThrough(
        new TransformStream<ParseResult<z.infer<typeof this.chunkSchema>>, LanguageModelV4StreamPart>({
          start(controller) {
            toolCallTracker = new StreamingToolCallTracker<OpenAICompatibleStreamingToolCallDelta>(controller, {
              generateId,
              extractMetadata: delta => {
                const thoughtSignature = delta.extra_content?.google?.thought_signature

                return thoughtSignature ? { [providerOptionsName]: { thoughtSignature } } : undefined
              },
