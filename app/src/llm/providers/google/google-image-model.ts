// @ts-nocheck
import type { ImageModelV4, LanguageModelV4Prompt, SharedV4Warning } from '@/llm/provider'
import {
  combineHeaders,
  convertToBase64,
...
import type { GoogleImageModelId, GoogleImageSettings } from './google-image-settings'
...
export class GoogleImageModel implements ImageModelV4 {
  ...
  static [WORKFLOW_DESERIALIZE](options: { modelId: string; config: GoogleImageModelConfig }) {
    ...
    if (files != null && files.length > 0) {
      throw new Error(
        'Google Gemini API does not support image editing with Imagen models. ' +
          'Use Google Vertex AI (@/llm/providers/google-vertex) for image editing capabilities.'
      )
    }
  ...
  private async doGenerateGemini(
    ...
    const languageModelPrompt: LanguageModelV4Prompt = [{ role: 'user', content: userContent }]
    ...
    const { googleSearch: _strippedGoogleSearch, ...passthroughGoogleOptions } = providerOptions?.google ?? {}
      ...
      providerOptions: {
        google: {
          ...
          imageConfig: aspectRatio
            ? {
                aspectRatio: aspectRatio as NonNullable<GoogleLanguageModelOptions['imageConfig']>['aspectRatio']
              }
            : undefined,
          ...(passthroughGoogleOptions as Omit<GoogleLanguageModelOptions, 'responseModalities' | 'imageConfig'>)
    ...
    const languageModelGoogleMetadata = (result.providerMetadata?.google as Record<string, unknown> | undefined) ?? {}
