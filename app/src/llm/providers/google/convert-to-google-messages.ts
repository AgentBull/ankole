...
import type { GoogleContent, GoogleContentPart, GoogleFunctionResponsePart, GooglePrompt } from './google-prompt'
...
function convertUrlToolResultPart(url: string): GoogleFunctionResponsePart | undefined {
...
function appendToolResultParts(
  ...
  toolName: string,
  outputValue: Extract<LanguageModelV4ToolResultOutput, { type: 'content' }>['value'],
  toolCallId?: string
): void {
...
export function convertToGoogleMessages(
  ...
  const readProviderOpts = (part: {
    ...
    if (isVertexLike) {
      return part.providerOptions?.google as Record<string, unknown> | undefined
    }
    return (part.providerOptions?.googleVertex ?? part.providerOptions?.vertex) as Record<string, unknown> | undefined
  }
    ...
    switch (role) {
              ...
              switch (part.type) {
                ...
                case 'tool-call': {
                  const serverToolCallId =
                    providerOpts?.serverToolCallId != null ? String(providerOpts.serverToolCallId) : undefined
                  const serverToolType =
                    providerOpts?.serverToolType != null ? String(providerOpts.serverToolType) : undefined
                  const effectiveThoughtSignature =
                    thoughtSignature ?? (isGemini3Model ? injectSkipSignature(part.toolName) : undefined)
      ...
      case 'tool': {
        ...
        for (const part of content) {
          ...
          const serverToolCallId =
            partProviderOpts?.serverToolCallId != null ? String(partProviderOpts.serverToolCallId) : undefined
          const serverToolType =
            partProviderOpts?.serverToolType != null ? String(partProviderOpts.serverToolType) : undefined

          if (serverToolCallId && serverToolType) {
            const serverThoughtSignature =
              partProviderOpts?.thoughtSignature != null ? String(partProviderOpts.thoughtSignature) : undefined
            ...
            parts.push({
              functionResponse: {
                ...
                response: {
                  name: part.toolName,
                  content:
                    output.type === 'execution-denied' ? (output.reason ?? 'Tool call execution denied.') : output.value
                }
              }
            })
          }
  ...
  return {
    systemInstruction:
      systemInstructionParts.length > 0 && !isGemmaModel ? { parts: systemInstructionParts } : undefined,
    contents
  }
}
