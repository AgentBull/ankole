// @ts-nocheck
import type { SpeechModelV4, SharedV4Warning } from '@/llm/provider'
import {
  combineHeaders,
...
export class GoogleSpeechModel implements SpeechModelV4 {
  ...
  static [WORKFLOW_DESERIALIZE](options: { modelId: GoogleSpeechModelId; config: GoogleSpeechModelConfig }) {
  ...
  async doGenerate(
    ...
    const {
      ...
      headers: combineHeaders(this.config.headers ? await resolve(this.config.headers) : undefined, options.headers),
      body: requestBody,
