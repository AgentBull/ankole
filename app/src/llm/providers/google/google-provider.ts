 ...
 */
export function createGoogle(options: GoogleProviderSettings = {}): GoogleProvider {
  const baseURL = withoutTrailingSlash(options.baseURL) ?? 'https://generativelanguage.googleapis.com/v1beta'
  ...
  const createImageModel = (modelId: GoogleImageModelId, settings: GoogleImageSettings = {}) =>
  ...
  const createInteractionsModel = (
    ...
    new GoogleInteractionsLanguageModel(modelIdOrAgent as GoogleInteractionsModelInput, {
