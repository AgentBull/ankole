...
import { convertGoogleUsage, type GoogleUsageMetadata } from './convert-google-usage'
...
import { GoogleJSONAccumulator, type PartialArg } from './google-json-accumulator'
...
export class GoogleLanguageModel implements LanguageModelV4 {
  ...
  static [WORKFLOW_DESERIALIZE](options: { modelId: string; config: GoogleConfig }) {
  ...
  private async getArgs(
    ...
    const providerOptionsNames: readonly string[] = this.config.provider.includes('vertex')
    ...
    if (tools?.some(tool => tool.type === 'provider' && tool.id === 'google.vertex_rag_store') && !isVertexProvider) {
      warnings.push({
        type: 'other',
        message:
    ...
    const vertexPaygoHeaders: Record<string, string> | undefined =
      isVertexProvider && (googleOptions?.sharedRequestType || googleOptions?.requestType)
        ? {
        ...
        : undefined
    const bodyServiceTier = isVertexProvider ? undefined : googleOptions?.serviceTier
    ...
    const streamFunctionCallArguments =
      isStreaming && isVertexProvider ? (googleOptions?.streamFunctionCallArguments ?? false) : undefined

    const toolConfig =
      googleToolConfig || streamFunctionCallArguments || googleOptions?.retrievalConfig
        ? {
            ...googleToolConfig,
  ...
  async doGenerate(options: LanguageModelV4CallOptions): Promise<LanguageModelV4GenerateResult> {
    ...
    const wrapProviderMetadata = (payload: Record<string, unknown>) =>
      Object.fromEntries(providerOptionsNames.map(name => [name, payload])) as SharedV4ProviderMetadata
  ...
  async doStream(options: LanguageModelV4CallOptions): Promise<LanguageModelV4StreamResult> {
    ...
    const finishActiveStreamingToolCall = (controller: TransformStreamDefaultController<LanguageModelV4StreamPart>) => {
    ...
    return {
      stream: response.pipeThrough(
        new TransformStream<ParseResult<ChunkSchema>, LanguageModelV4StreamPart>({
          start(controller) {
            ...
            if (content != null) {
              ...
              for (const part of parts) {
                if ('executableCode' in part && part.executableCode?.code) {
                  ...
                  const responseToolCallId = lastServerToolCallId ?? part.toolResponse.id ?? generateId()
                  const serverMeta = wrapProviderMetadata({
                    ...(part.thoughtSignature ? { thoughtSignature: part.thoughtSignature } : {}),
                    serverToolCallId: responseToolCallId,
                    serverToolType: part.toolResponse.toolType
              ...
              for (const part of parts) {
                ...
                const isStreamingChunk =
                  ...
                  (part.functionCall.name != null && part.functionCall.willContinue === true)
                ...
                if (isStreamingChunk) {
                  if (part.functionCall.name != null) {
                    ...
                    if (part.functionCall.partialArgs != null) {
                      const partialArgs = part.functionCall.partialArgs as PartialArg[]
...
type GoogleThinkingConfig = NonNullable<InferSchema<typeof googleLanguageModelOptions>['thinkingConfig']>
...
function resolveGemini3ThinkingConfig({
  ...
  reasoning: Exclude<LanguageModelV4CallOptions['reasoning'], 'provider-default' | undefined>
  warnings: SharedV4Warning[]
...
export type PromptFeedbackSchema = NonNullable<InferSchema<typeof responseSchema>['promptFeedback']>

export type UsageMetadataSchema = NonNullable<InferSchema<typeof responseSchema>['usageMetadata']>
