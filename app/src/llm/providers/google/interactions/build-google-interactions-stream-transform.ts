...
import type { ParseResult } from '@/llm/provider-utils'
import type { GoogleInteractionsEvent, GoogleInteractionsUsage } from './google-interactions-api'
import { convertGoogleInteractionsUsage } from './convert-google-interactions-usage'
import { annotationsToSources, builtinToolResultToSources } from './extract-google-interactions-sources'
...
export function buildGoogleInteractionsStreamTransform({
   ...
   */
  serviceTier?: string
}): TransformStream<ParseResult<GoogleInteractionsEvent>, LanguageModelV4StreamPart> {
  ...
  return new TransformStream<ParseResult<GoogleInteractionsEvent>, LanguageModelV4StreamPart>({
    start(controller) {
    ...
    transform(chunk, controller) {
      ...
      switch (eventType) {
        case 'interaction.created': {
          const event = value as Extract<GoogleInteractionsEvent, { event_type: 'interaction.created' }>
           ...
           * stays clean.
           */
          interactionId = interaction?.id != null && interaction.id.length > 0 ? interaction.id : undefined

          const created = (interaction as { created?: string } | undefined)?.created
        ...
        case 'step.start': {
          const event = value as Extract<GoogleInteractionsEvent, { event_type: 'step.start' }>
          ...
          if (stepType === 'model_output') {
            ...
            const toolCallId = step?.id ?? blockId
            const state: Extract<OpenBlockState, { kind: 'builtin_tool_call' }> = {
              kind: 'builtin_tool_call',
              id: blockId,
              blockType: stepType,
              toolCallId,
            ...
            const state: Extract<OpenBlockState, { kind: 'builtin_tool_result' }> = {
              kind: 'builtin_tool_result',
              id: blockId,
              blockType: stepType,
              callId,
        ...
        case 'step.delta': {
          const event = value as Extract<GoogleInteractionsEvent, { event_type: 'step.delta' }>
          ...
          if (
            dtype === 'image' &&
            (open.kind === 'pending_model_output' || open.kind === 'text' || open.kind === 'image')
          ) {
            const imageDelta = event.delta as { data?: string; mime_type?: string; uri?: string } | undefined
            const google: Record<string, string> = {}
            if (interactionId != null) google.interactionId = interactionId
            const providerMetadata = Object.keys(google).length > 0 ? { google } : undefined
          ...
          if (open.kind === 'text' && delta?.type === 'text') {
            ...
            open.kind === 'text' &&
            (delta?.type === 'text_annotation' || delta?.type === 'text_annotation_delta')
          ) {
        ...
        case 'step.stop': {
          const event = value as Extract<GoogleInteractionsEvent, { event_type: 'step.stop' }>
          const open = openBlocks.get(event.index)
          if (open == null) break

          if (open.kind === 'text') {
            const textProviderMetadata = interactionId != null ? { google: { interactionId } } : undefined
            controller.enqueue({
              type: 'text-end',
              id: open.id,
        ...
        case 'interaction.requires_action': {
          const event = value as Extract<
            ...
            {
              event_type: 'interaction.status_update' | 'interaction.in_progress' | 'interaction.requires_action'
            }
          >
        ...
        case 'interaction.completed': {
          const event = value as Extract<GoogleInteractionsEvent, { event_type: 'interaction.completed' }>
        ...
        case 'error': {
          const event = value as Extract<GoogleInteractionsEvent, { event_type: 'error' }>
          finishStatus = 'failed'
