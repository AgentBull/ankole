// @ts-nocheck
import type { LanguageModelV4Source } from '@/llm/provider'
import type {
...
export function builtinToolResultToSources({
  ...
  switch (block.type) {
    case 'url_context_result': {
      const result = (block as GoogleInteractionsURLContextResultContent).result ?? []
    ...
    case 'google_search_result': {
      const result = (block as GoogleInteractionsGoogleSearchResultContent).result ?? []
      for (const entry of result) {
        const url = entry?.url
    ...
    case 'google_maps_result': {
      const result = (block as GoogleInteractionsGoogleMapsResultContent).result ?? []
      for (const entry of result) {
...
export function annotationsToSources({
  ...
  annotations: Array<GoogleInteractionsAnnotation | { type: string }> | null | undefined
  generateId: () => string
}): Array<LanguageModelV4Source> {
