// @ts-nocheck
import { UnsupportedFunctionalityError, type LanguageModelV4CallOptions, type SharedV4Warning } from '@/llm/provider'
...
export function prepareTools({
  ...
  const isLatest = (
    ['gemini-flash-latest', 'gemini-flash-lite-latest', 'gemini-pro-latest'] as const satisfies GoogleModelId[]
  ).some(id => id === modelId)
  const isGemini2orNewer =
    modelId.includes('gemini-2') || modelId.includes('gemini-3') || modelId.includes('nano-banana') || isLatest
  ...
  if (toolChoice == null) {
    return {
      ...
      toolConfig: hasStrictTools ? { functionCallingConfig: { mode: 'VALIDATED' } } : undefined,
      toolWarnings
    }
  }
