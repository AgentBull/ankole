// @ts-nocheck
import type { SkillsV4, SharedV4Warning } from '@/llm/provider'
import {
  combineHeaders,
...
import { anthropicSkillResponseSchema, anthropicSkillVersionResponseSchema } from './anthropic-skills-api'
...
export class AnthropicSkills implements SkillsV4 {
  ...
  private async fetchVersionMetadata({
    ...
    const { value: versionResponse } = await getFromApi({
      ...
      successfulResponseHandler: createJsonResponseHandler(anthropicSkillVersionResponseSchema),
  ...
  async uploadSkill(
    ...
    const { value: response } = await postFormDataToApi({
      ...
      formData,
      failedResponseHandler: anthropicFailedResponseHandler,
      successfulResponseHandler: createJsonResponseHandler(anthropicSkillResponseSchema),
      fetch: this.config.fetch
    })
