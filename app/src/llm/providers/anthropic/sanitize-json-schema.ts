// @ts-nocheck
import type { JSONSchema7, JSONSchema7Definition } from '@/llm/provider'
...
function sanitizeSchema(schema: JSONSchema7): JSONSchema7 {
  ...
  if (schema.definitions != null) {
    result.definitions = Object.fromEntries(
      Object.entries(schema.definitions).map(([name, definition]) => [name, sanitizeDefinition(definition)])
    )
  }
  ...
  if (schemaWithDefs.$defs != null) {
    ...
    resultWithDefs.$defs = Object.fromEntries(
      Object.entries(schemaWithDefs.$defs).map(([name, definition]) => [name, sanitizeDefinition(definition)])
    )
  }
  ...
  if (schema.type === 'object' || schema.properties != null) {
    if (schema.properties != null) {
      result.properties = Object.fromEntries(
        Object.entries(schema.properties).map(([name, definition]) => [name, sanitizeDefinition(definition)])
      )
    }
  ...
  if (constraintDescription != null) {
    result.description =
      result.description == null ? constraintDescription : `${result.description}\n${constraintDescription}`
  }
