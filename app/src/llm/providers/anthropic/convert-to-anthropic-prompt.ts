...
export async function convertToAnthropicPrompt({
  ...
  async function shouldUseContainerUpload(providerMetadata: SharedV4ProviderMetadata | undefined): Promise<boolean> {
    const anthropicOptions = await parseProviderOptions({
      provider: 'anthropic',
      providerOptions: providerMetadata,
      schema: anthropicFilePartProviderOptions
              ...
              for (let i = 0; i < content.length; i++) {
                ...
                const outputProviderOptions =
                  'providerOptions' in output
                    ...
                    : output.type === 'content'
                      ? output.value.find(contentPart => contentPart.providerOptions != null)?.providerOptions
                      : undefined
                    ...
                    contentValue = output.value
                      .map(contentPart => {
                        switch (contentPart.type) {
                          ...
                          case 'file': {
                            const topLevel = getTopLevelMediaType(contentPart.mediaType)
                          ...
                          case 'custom': {
                            const anthropicOptions = contentPart.providerOptions?.anthropic as
          ...
          for (let k = 0; k < content.length; k++) {
            ...
            switch (part.type) {
              case 'text': {
                ...
                const textMetadata = part.providerOptions?.anthropic as { type?: string } | undefined
              ...
              case 'tool-call': {
                if (part.providerExecuted) {
                  ...
                  if (isMcpToolUse) {
                    ...
                    (part.input.type === 'bash_code_execution' || part.input.type === 'text_editor_code_execution')
                  ) {
                    anthropicContent.push({
                ...
                const caller = callerOptions?.caller
                  ? (callerOptions.caller.type === 'code_execution_20250825' ||
                      callerOptions.caller.type === 'code_execution_20260120') &&
                    callerOptions.caller.toolId
                    ? {
                        type: callerOptions.caller.type as 'code_execution_20250825' | 'code_execution_20260120',
              ...
              case 'tool-result': {
                ...
                if (mcpToolUseIds.has(part.toolCallId)) {
                  ...
                  anthropicContent.push({
                    ...
                    content: output.value as unknown as string | Array<{ type: 'text'; text: string }>,
                    cache_control: cacheControl
                  })
                  ...
                  if (output.type === 'error-json') {
                    anthropicContent.push({
                      ...
                      content: {
                        ...
                        error_code: extractErrorValue(output.value).errorCode ?? 'unavailable'
                      },
                      cache_control: cacheControl
                    })
