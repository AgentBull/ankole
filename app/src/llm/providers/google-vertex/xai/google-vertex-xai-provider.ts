// @ts-nocheck
import { NoSuchModelError, type LanguageModelV4, type LanguageModelV4Usage, type ProviderV4 } from '@/llm/provider'
import { createOpenAICompatible, type OpenAICompatibleProvider } from '@/llm/providers/openai-compatible'
import {
  loadOptionalSetting,
  loadSetting,
...
function convertGoogleVertexXaiUsage(usage: GoogleVertexXaiUsage): LanguageModelV4Usage {
  if (usage == null) {
    return {
      inputTokens: {
        total: undefined,
 ...
 */
export function createGoogleVertexXai(options: GoogleVertexXaiProviderSettings = {}): GoogleVertexXaiProvider {
  const loadLocation = () =>
    loadOptionalSetting({
  ...
  const loadBaseURL = () => withoutTrailingSlash(options.baseURL ?? '') || constructBaseURL()

  let cachedProvider: OpenAICompatibleProvider<GoogleVertexXaiModelId, string, string, string> | undefined
  const getProvider = () =>
  ...
  const createChatModel = (modelId: GoogleVertexXaiModelId) => getProvider().languageModel(modelId)

  const provider = function (modelId: GoogleVertexXaiModelId) {
    if (new.target) {
      throw new Error('The Google Vertex xAI model function cannot be called with the new keyword.')
    }
  ...
  provider.chatModel = (modelId: GoogleVertexXaiModelId) => getProvider().chatModel(modelId)
