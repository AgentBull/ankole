...
import { openaiChatChunkSchema, openaiChatResponseSchema, type OpenAIChatChunk } from './openai-chat-api'
import { openaiLanguageModelChatOptions, type OpenAIChatModelId } from './openai-chat-language-model-options'
...
export class OpenAIChatLanguageModel implements LanguageModelV4 {
  ...
  static [WORKFLOW_DESERIALIZE](options: { modelId: OpenAIChatModelId; config: OpenAIChatConfig }) {
  ...
  async doStream(options: LanguageModelV4CallOptions): Promise<LanguageModelV4StreamResult> {
    ...
    const result = {
      stream: checkedResponse.pipeThrough(
        new TransformStream<ParseResult<OpenAIChatChunk>, LanguageModelV4StreamPart>({
          start(controller) {
