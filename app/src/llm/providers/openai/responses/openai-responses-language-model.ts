...
import type { toolSearchInputSchema, toolSearchOutputSchema } from '../tool/tool-search'
import type { webSearchOutputSchema } from '../tool/web-search'
import { convertOpenAIResponsesUsage, type OpenAIResponsesUsage } from './convert-openai-responses-usage'
 ...
 */
function extractApprovalRequestIdToToolCallIdMapping(prompt: LanguageModelV4Prompt): Record<string, string> {
...
export class OpenAIResponsesLanguageModel implements LanguageModelV4 {
  ...
  static [WORKFLOW_DESERIALIZE](options: { modelId: OpenAIResponsesModelId; config: OpenAIConfig }) {
  ...
  private async getArgs({
    ...
    const resolvedReasoningEffort =
      openaiOptions?.reasoningEffort ?? (isCustomReasoning(reasoning) ? reasoning : undefined)
    ...
    const { input, warnings: inputWarnings } = await convertToOpenAIResponsesInput({
      ...
      toolNameMapping,
      systemMessageMode:
        openaiOptions?.systemMessageMode ?? (isReasoningModel ? 'developer' : modelCapabilities.systemMessageMode),
      providerOptionsName,
    ...
    const baseArgs = {
      ...
      ...(isReasoningModel &&
        (resolvedReasoningEffort != null || openaiOptions?.reasoningSummary != null) && {
          reasoning: {
    ...
    const shellToolEnvType = (
      tools?.find(tool => tool.type === 'provider' && tool.id === 'openai.shell') as
        | { args?: { environment?: { type?: string } } }
        | undefined
  ...
  async doStream(options: LanguageModelV4CallOptions): Promise<LanguageModelV4StreamResult> {
    ...
    const result = {
      stream: checkedResponse.pipeThrough(
        new TransformStream<ParseResult<OpenAIResponsesChunk>, LanguageModelV4StreamPart>({
          start(controller) {
...
function isResponseFunctionCallArgumentsDeltaChunk(chunk: OpenAIResponsesChunk): chunk is OpenAIResponsesChunk & {
  type: 'response.function_call_arguments.delta'
} {
  return chunk.type === 'response.function_call_arguments.delta'
}

function isResponseCustomToolCallInputDeltaChunk(chunk: OpenAIResponsesChunk): chunk is OpenAIResponsesChunk & {
  type: 'response.custom_tool_call_input.delta'
} {
  return chunk.type === 'response.custom_tool_call_input.delta'
}

function isResponseImageGenerationCallPartialImageChunk(chunk: OpenAIResponsesChunk): chunk is OpenAIResponsesChunk & {
  type: 'response.image_generation_call.partial_image'
} {
  return chunk.type === 'response.image_generation_call.partial_image'
}

function isResponseCodeInterpreterCallCodeDeltaChunk(chunk: OpenAIResponsesChunk): chunk is OpenAIResponsesChunk & {
  type: 'response.code_interpreter_call_code.delta'
} {
  return chunk.type === 'response.code_interpreter_call_code.delta'
}

function isResponseCodeInterpreterCallCodeDoneChunk(chunk: OpenAIResponsesChunk): chunk is OpenAIResponsesChunk & {
...
function isResponseAnnotationAddedChunk(chunk: OpenAIResponsesChunk): chunk is OpenAIResponsesChunk & {
  type: 'response.output_text.annotation.added'
} {
  return chunk.type === 'response.output_text.annotation.added'
}

function isErrorChunk(chunk: OpenAIResponsesChunk): chunk is OpenAIResponsesChunk & { type: 'error' } {
