// @ts-nocheck
import type { Context, InferToolSetContext, ReasoningFilePart, ReasoningPart, ToolSet } from '@/llm/provider-utils'
import type {
  CallWarning,
  FinishReason,
...
import { convertFromReasoningOutputs, type ReasoningFileOutput, type ReasoningOutput } from './reasoning-output'
...
export class DefaultStepResult<TOOLS extends ToolSet, RUNTIME_CONTEXT extends Context = Context> implements StepResult<
  TOOLS,
  RUNTIME_CONTEXT
> {
  ...
  get staticToolCalls() {
    return this.toolCalls.filter((toolCall): toolCall is StaticToolCall<TOOLS> => toolCall.dynamic !== true)
  }
  ...
  get staticToolResults() {
    return this.toolResults.filter((toolResult): toolResult is StaticToolResult<TOOLS> => toolResult.dynamic !== true)
  }
