// @ts-nocheck
import type { FilesV4, FilesV4UploadFileCallOptions, ProviderV4 } from '@/llm/provider'
import { convertBase64ToUint8Array, detectMediaType } from '@/llm/provider-utils'
...
export async function uploadFile({
  ...
  const data: FilesV4UploadFileCallOptions['data'] =
    dataArg instanceof Uint8Array || typeof dataArg === 'string' ? { type: 'data', data: dataArg } : dataArg

  const mediaType =
    mediaTypeArg ??
    (data.type === 'text'
      ? 'text/plain'
      : (detectMediaType({ data: data.data }) ?? (isLikelyText(data.data) ? 'text/plain' : 'application/octet-stream')))
