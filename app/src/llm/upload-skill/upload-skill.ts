// @ts-nocheck
import type { ProviderV4, SkillsV4, SkillsV4File, SkillsV4UploadSkillCallOptions } from '@/llm/provider'
...
export async function uploadSkill({
  ...
  const normalizedFiles: SkillsV4File[] = files.map(file => ({
    ...file,
    data:
      file.data instanceof Uint8Array || typeof file.data === 'string' ? { type: 'data', data: file.data } : file.data
  }))
