  ...
  zodSchema(
    z.object({
      ...
      source_policy: z
        ...
        .optional()
        .describe('Source policy for controlling which domains to include/exclude and freshness.'),
      ...
      fetch_policy: z
        .object({
          max_age_seconds: z
            .number()
            .optional()
            .describe('Maximum age in seconds for cached content. Set to 0 to always fetch fresh content.')
        })
        .optional()
...
const parallelSearchOutputSchema = lazySchema(() =>
  zodSchema(
    z.union([
      ...
      // Error response
      z.object({
        error: z.enum(['api_error', 'rate_limit', 'timeout', 'invalid_input', 'configuration_error', 'unknown']),
...
export const parallelSearch = (config: ParallelSearchConfig = {}): ReturnType<typeof parallelSearchToolFactory> =>
