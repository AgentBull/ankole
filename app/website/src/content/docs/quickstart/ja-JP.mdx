      ...
      steps: [
        {
          ...
          fields: [
            { name: 'flowStatus', description: 'テキスト。カードの状態。書き込み中は 2、完了時は 3、失敗時は 5。' },
        ...
        {
          title: 'コンポーネントを配置してカード状態をバインドする',
          body: [
            'AI カードコンテナ内にコンポーネントをこの順序で配置してください。meta と state は上部の小さなテキストとして、plan はテキストとして、thought と activity はそれぞれ折りたたみ領域に、answer はメインの Markdown ブロックとして、そのコンテンツ変数タイプを Markdown に設定してストリーミングを有効にしてください。answer は Ankole がストリーミングの速度で更新する唯一のブロックです。results と receipts はテキストとして、actions はアクション領域にバインドし、各ボタンの値をそのまま渡してください。',
            'AI カードコンテナを選択し、そのフロー状態変数（flowStatusVar）を flowStatus にバインドしてください。書き込み中と失敗の状態を有効にしたままにしてください。'
          ],
          caution:
            'このバインディングこそがカードに何かを表示させるものです。コンテナは flowStatus を読み、どの状態を描画するかを選択します。状態変数がバインドされていないコンテナは、初期状態のまま永遠に留まり、多くの場合空の本文を描画します。'
        },
        {
          ...
          bullets: [
            'カードが空白、または返信終了後も入力中インジケータが残る： コンテナの状態変数が flowStatus にバインドされていません。バインドして再度公開してください。',
