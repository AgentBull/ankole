...
export const modelAdvanced = {
  title: '선택적 모델 프로필과 provider 튜닝',
  summary: 'Agent가 필요로 할 때까지 선택적 기능 슬롯을 비워 둡니다.',
  steps: [
    {
      title: '선택적 기능 프로필 추가',
      fields: [
        {
          ...
          description:
            '내구성이 있는 Background Agent Jobs를 실행합니다. 저장된 프로필 키는 현재 coding입니다. 메시지에 코드가 많이 포함되어 있으므로 일반 대화는 이 프로필로 전환되지 않습니다.'
        },
        {
        ...
        },
        {
          ...
          description: '검색과 결과 정렬을 위한 모델입니다.'
        },
        {
  ...
  "scopes": {
    ...
    advanced: {
      ...
      steps: [
        ...
        {
          title: 'Lark 앱 소유권 규칙 준수',
          bullets: [
            '하나의 Agent는 활성화된 Lark binding을 최대 하나만 가질 수 있습니다.',
            '하나의 appID는 하나의 Agent에만 바인딩할 수 있습니다.'
          ],
          body: ['추가 Agent마다 별도의 Lark 또는 Feishu 앱을 만드세요.']
        }
      ]
    }
  },
      ...
      steps: [
        {
          ...
          fields: [
            { name: 'flowStatus', description: 'Text. 카드 상태: 작성 중 2, 완료 3, 실패 5.' },
        ...
        {
          title: '컴포넌트 배치 및 카드 상태 바인딩',
          body: [
            'AI 카드 컨테이너 안에 컴포넌트를 이 순서로 배치하세요. meta와 state는 상단의 작은 텍스트로, plan은 텍스트로, thought와 activity는 각각 접힌 영역에, answer는 콘텐츠 변수 유형을 Markdown으로 설정하고 streaming을 켠 메인 Markdown 블록으로 — Ankole이 streaming 속도로 업데이트하는 유일한 블록입니다. results와 receipts는 텍스트로, actions는 액션 영역에 바인딩하고 각 버튼 값을 그대로 전달하세요.',
            'AI 카드 컨테이너를 선택하고 flow-status 변수(flowStatusVar)를 flowStatus에 바인딩하세요. 작성 중과 실패 상태는 켜 둡니다.'
          ],
          caution:
            '이 바인딩이 카드가 무엇이든 표시하게 만드는 핵심입니다. 컨테이너는 flowStatus를 읽어 어떤 상태를 그릴지 선택합니다. 상태 변수가 바인딩되지 않은 컨테이너는 초기 상태에 영원히 머물며 빈 본문을 렌더링하는 경우가 많습니다.'
        },
        {
          ...
          bullets: [
            '카드가 비어 있거나 답변 종료 후에도 작성 인디케이터가 남아 있음: 컨테이너 상태 변수가 flowStatus에 바인딩되지 않았습니다. 바인딩하고 다시 게시하세요.',
...
export const signalAdvanced = {
  title: '그룹 동작, 기밀 memory, subject namespace',
  summary: '안전한 첫 binding은 지명된 경우에만 응답하고 기본 공유 memory scope를 유지합니다.',
  steps: [
    {
    ...
    {
      title: '기밀 memory는 민감한 channel에서만 사용',
      body: [
        '기본 shared scope는 Agent가 대화를 공유 knowledge로 사용하게 합니다. 기밀 memory는 해당 channel의 회상된 history를 channel scope 안에 유지합니다.'
      ]
    },
    {
...
프라이빗 Ankole deployment instance는 control plane 하나와 하나 이상의 Agent Computer Worker로 구성됩니다. control plane은 관리 플랫폼입니다. Worker는 Agent 작업을 실행하고 Agent의 작업 컴퓨터 역할을 합니다.
...
현재 릴리스는 직접 연결을 사용합니다. 하나의 라우팅 규칙이 하나의 Channel Provider를 하나의 Agent에 연결합니다. 규칙은 adapter, 앱 자격 증명, 대상 Agent, 그룹 메시지 동작, memory 동작을 기록합니다. bot마다 별도의 규칙을 만드세요.
...
| **Confidential memory** | 첫 테스트에서는 끄기 |
