      ...
      steps: [
        {
          ...
          fields: [
            { name: 'flowStatus', description: '文本。卡片状态：书写中 2、完成 3、失败 5。' },
        ...
        },
        {
          title: '排布组件，并绑定卡片状态',
          body: [
            '在 AI 卡片容器内按顺序放组件：meta 与 state 用小号文本放顶部；plan 用文本组件；thought 和 activity 各放进一个折叠区；answer 作为主 Markdown 组件，把内容变量类型设为 markdown 并开启流式，它是唯一按流式频率更新的组件；results 与 receipts 用文本组件；actions 绑定到操作区，并原样透传每个按钮的 value。',
            '选中 AI 卡片容器，把它的流程状态变量（flowStatusVar）绑定到 flowStatus，保持书写态与失败态启用。'
          ],
          caution:
            '这条绑定决定卡片是否显示得出内容：容器靠读取 flowStatus 选择绘制哪个状态。状态变量未绑定的容器会永远停在初始状态，而且经常渲染出空白正文。'
        },
        {
          ...
          bullets: [
            '卡片空白，或回复结束后仍显示输入指示：容器状态变量没有绑定 flowStatus，重新绑定并发布。',
