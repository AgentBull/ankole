...
## レバー 7: Agent ごとの token quota

上の 6 つのレバーは、1 つの Turn がいくらかかるかを形づくります。token quota は合計に上限を設けます。1 つの Agent が繰り返しの期間内に消費できる token を制限し、使用量が上限に達すると、期間が終わるまで AIGateway がその Agent のモデル呼び出しを拒否します。

Agent ページで設定します。期間（日数）、期間の開始時刻、token の上限です。Agent ページには、上限に対する使用済みの割合と現在の期間の終了時刻が表示されます。上限に達した状態で届いたチャットメッセージには、使用済み token、上限、期間の終了時刻を含む返信が返ります。上限に達した Background Agent Job は、待つのではなく失敗します。正当な急増を続ける必要がある場合は、**期間をリセット** ですぐに新しい期間を開始します。

quota は予算プランナーではなく、停止装置として使ってください。上限に近づいても Agent は減速せず、1 回の呼び出しがその使用量の分だけ上限を超えて終わることがあります。下記の観測された消費から、通常の最も重い週に余裕を持たせてサイズを決めてください。フィールドは [Agents](../agents/) を参照してください。
