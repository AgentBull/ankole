...
Agent 是一位长期承担业务职能的 AI 同事。每个 Agent 都有自己的身份、负责人、工作要求、模型、能力和文件空间，并通过信号路由规则接收来自聊天渠道的消息。
...
停用 Agent 后，它不会继续处理新工作，但仍会以“已停用”状态留在智能体列表中。你可以随时重新启用它，也可以永久删除它——删除仅对已停用的 Agent 可用，会连同它的会话、任务与相关记录一并删除。若只是想暂时停止某个聊天入口，停用对应的信号路由规则即可，不必停用整个 Agent。
