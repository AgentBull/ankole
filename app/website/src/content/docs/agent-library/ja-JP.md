...
## Agent の永続ドキュメントと Skill overlay

能力に加えて、library は Agent 自身の書き込み可能なドキュメントと Skill のカスタマイズを保持します。

- **Agent の永続ドキュメント**は `mission`、`soul`、`design` の 3 つで、コンテナテーブルが受け入れる 3 つの `source_kind` 値です。最初の 2 つは責任と振る舞いを定義します。`design` はビジュアル作業のためのデザインシステムを保存します。これらは `agent_library_container_entries` に置かれ、content hash を使用します。
- **Skill overlay** は `agent_skill_overlays` の意味論的な行で、`(agent, skill)` ごとに 1 行です。オペレーターは、Skill bundle を fork することなく、1 つの Agent に対する Skill の振る舞いをカスタマイズできます。overlay は compare-and-swap 置換をサポートするため、並行編集は決定的に解決されます。

Skill ビューは Skill のファイルと、その Agent が持つ overlay を読みます。したがって Agent が見るのは 1 つの一貫した Skill であり、bundle と別の patch の組み合わせではありません。
...
| `GET` | `/agents/:agent_uid/library-documents` | その Agent の mission/soul/design を一覧表示 |
