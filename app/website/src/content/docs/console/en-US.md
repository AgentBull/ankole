---
title: Console operator flows
description: The stateless bearer-token REST API an administrator uses to configure providers, model profiles, Agents, Signal bindings, library capabilities, WorkerEnv secrets, and plugins.
section: Operations
order: 12
---

The Console is how an operator turns a running Ankole installation into a working one. Behind the React shell is one stateless REST API — the `console_api` pipeline — and every configuration surface in the system is a route on it. This page is the operator's map of that API: what the gate is, what the configuration surfaces are, and what each one is for.
