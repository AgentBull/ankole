...
Agent Computer Worker 负责运行模型循环、工具、Skills、浏览器、终端和文件操作，也执行 automation job 的脚本运行。它不自行发明持久业务状态，而是接受控制面的调度，并把执行结果提交回控制面。
...
外部 MCP server 以 Skill 为单位接入：Skill 声明连接，Worker 在执行时按次生成配置，执行结束即回收，模型不面对一份常驻的全量 MCP 工具清单。发布内置的少量 Direct MCP 工具（例如图表渲染）是唯一例外，随 Worker 发布并以受限目录直接投影给模型。
