...
Agent Computer Worker 负责运行模型循环、工具、Skills、浏览器、终端和文件操作，也执行 automation job 的脚本。它不自行发明持久业务状态，而是接受控制面的调度，并把执行结果提交回控制面。
...
Cron、Checkback 和 webhook endpoint 是同一体系里的三种触发器。每个触发器默认唤醒一个 Agent 会话，也可以绑定一个 automation job：一段由 Agent 编写的确定性脚本。触发事件保持不变，改变的只是收件人。

脚本静默完成机械检查，每次运行留下可查的记录；需要判断时，它经 `emitEvent` 把事件发回归属会话，再由 Agent 复核和行动。模型因此不必为轮询空转。参见 [Automation Job](../automation-jobs/)。
...
Brain 不是把聊天原文堆进向量数据库。它维护的是持续演化的世界模型：新证据可以补充、修正或取代已有 Claim。证据发生冲突时，Dreaming 会记录矛盾并交给人复核，不会静默改写相关 Claim。

知识可以来自 Agent 参与的对话、符合条件且没有人专门对它说的群聊消息，以及已注册的文件或 URL Source。

同一实例中的所有 Agent 共用一个知识空间。每个受保护的正文段落、Claim 和 Timeline 事件使用 `world`、权限组或单个 Principal 的披露范围。Source 或 Channel 只记录出处，不授予学习产物的访问权。

运行中的 Agent 通过统一召回获得当前任务需要的长期上下文。离线的 Dreaming 会整理新证据、发现模式、为已有预测判分，并标记等待复核的冲突。

[Brain](../brain/) 负责“世界现在是什么样”。[Skill Lessons](../skill-lessons/)保存 Agent 从反复出现的工作证据中学到的过程护栏。它们随对应 Skill 一同提供，但不改写 Skill 来源；不再适用时会退场。

这套结构对应首页所说的“合作越久，它越懂你们的规则”：积累不是简单增加日志，而是让下一次工作的初始认知和做法都发生变化。

在用户看来，这套机制就是长期记忆；存储、Dreaming 与写入权限由 Brain 负责。
...
Agent 模型档案为 Agent 执行选择模型。Brain 使用 AppConfigure 中五项实例级模型设置。控制面加密保存 Provider 凭证，AIGateway 在请求上游服务时使用它们；Agent 与 Worker 只接触允许使用的模型和调用结果。

AIGateway 同时支持不保存历史的调用和可以继续的有状态会话。它负责请求转换、流式事件、工具结果、上下文压缩、用量记录和最终结果提交，Worker 不需要为每家 Provider 重写一套生命周期。

工具目录很大时，模型可以经 Tool Search 按需检索工具，或提交一段程序，在受限沙盒中成批调用工具后只带回结果。原生支持这些能力的 Provider 直接透传；其余 Provider 由 AIGateway 在网关内补齐同一能力。一个 Provider 还可以配置多个凭证组成凭据池（包括 ChatGPT 订阅这类账号凭证），失败归属、重试和用量记录都按单个凭证计。
...
外部 MCP server 以 Skill 为单位接入：Skill 声明连接，Worker 在执行时按次生成配置，执行结束即回收，模型不面对一份常驻的全量 MCP 工具清单。

参见 [主体与权限组](../principal-and-groups/)、[信号路由规则](../signal-bindings/)、[Agent 能力库](../skills/)和 [MCP server 参考](../mcp/)。
