---
title: アーキテクチャ
description: control plane、Agent Computer Worker、Actor Runtime、Brain、Background Agent Job がどのように Ankole を構成するかを説明します。
...
Ankole はセルフホスト型の AI Workforce OS です。AI Agent を、業務機能を自律的に遂行し、成果で評価される労働力に変えます。

1 つの Agent は、1 つの業務機能を数時間にわたって実行し、共有の作業 context を使い、人が検査と採点を行える成果を納品できます。

これには、chat endpoint の背後にある model 以上のものが求められます。システムは、誰が行動するか、誰がアクセス権を持つか、長時間の仕事がどう復旧するか、context がどう最新に保たれるか、1 つの修正が後の仕事をどう変えるかを把握している必要があります。
...
## Brain: 修正を続ける世界 model

Brain は chat の抜粋を詰め込んだ vector database ではありません。進化し続ける世界 model を維持します。新しい証拠は古い結論を拡張、修正、退役させることができ、矛盾する memory は時刻、出所、confidence によって裁定されます。

知識は、Agent との会話、誰も Agent に宛てなかった group chat、document、connector、その他の外部 source から得られます。

memory は shared、Agent、direct message、channel の scope に分離され、context が誤った境界を越えないようにします。

turn の間、unified recall が現在の仕事に必要な長期 context を供給します。オフラインの Dreaming は新しい証拠を整理し、繰り返される事実を圧縮し、矛盾を解決し、以前の予測と後の結果を比較します。

Brain は「世界が今どのような状態か」を所有します。Skill Overlay は「この作業を次回はどう行うべきか」を所有します。Agent は失敗や修正の後に Skill の変更を提案できます。人が承認してから、後の session がそれを使用します。

この構造は、Agent が時間とともにチームの規則を学ぶというホームページ上の約束を実現します。蓄積は、より長い log を意味しません。次の Job がより良い知識とより良い方法から始まることを意味します。

ユーザー向けの説明は [長期 memory](../memory/) を、storage、Dreaming、書き込み権限は [Brain](../brain/) を参照してください。
...
model profile が仕事の種類ごとに model を選択します。control plane は Provider の credential を暗号化して保存し、AIGateway は上流への request でそれらを使用します。
...
論理的な境界はどの形態でも同じです。instance は control plane を 1 つ持ち、Worker を水平に追加できます。完全な手順は [クイックスタート](../quickstart/#1-deploy-ankole) を参照してください。
