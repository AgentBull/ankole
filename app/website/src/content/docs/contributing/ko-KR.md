...
- 버전은 앞에 0이 없는 `MAJOR.MINOR.PATCH`를 사용합니다. 기능 수준 변경에는 `MINOR`를 올리고 `PATCH`를 `0`으로 재설정합니다. 의존성 업그레이드, 버그 수정, 문서 변경, 또는 제품 기능을 추가하거나 바꾸지 않는 기타 유지보수 변경에는 `PATCH`를 올립니다. 하나의 커밋에 두 유형이 모두 포함되면 minor 증가분을 사용합니다. `MAJOR`는 명시적인 유지보수자 결정 후에만 변경합니다.
