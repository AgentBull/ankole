...
- **`runtime_fabric/`** — the `ankole.runtime_fabric.v1` protobuf namespace and the current wire protocol version 5: lanes, durability classes, correlation rules, and turn/control/progress/RPC body semantics, validated over host-encoded protobuf bytes. The single structural declaration is `proto/envelope.proto`; each host derives its own codec from it — `prost-build` in Rust, `protox` in Elixir, `protoc-gen-es` in TypeScript. No host invents the structure.
