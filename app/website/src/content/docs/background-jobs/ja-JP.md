...
Ankole が Background Agent Job と呼ぶのは、一時的な呼び出しではなく、完全なライフサイクルを持つ作業としてシステムが保存するからです。Job は Worker が中断された後も復旧できます。
...
- **古い Job がモデルプロファイルの変更を無視する:** provider バインディングは Job 作成時に固定されます。プロファイルの変更は新しい Job にのみ影響します。
