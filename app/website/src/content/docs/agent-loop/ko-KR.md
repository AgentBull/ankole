...
턴 오류 재시도 예산은 worker가 아니라 여기에 있습니다: 최대 5회 전달(`@worker_turn_error_dead_letter_attempts`)입니다. Worker의 일시적 실패는 5초에서 120초 사이의 지수 백오프로 대기합니다. 제공자 용량 실패(업스트림 `429`, `5xx`, 또는 소진된 credential pool)는 자체 래더로 대기합니다. 30초, 그다음 2분, 5분, 12.5분이므로 자동 재시도가 예약하는 백오프는 합계 20분을 넘지 않습니다. 실패한 시도마다 epoch가 증가하므로, 실패한 시도의 늦은 응답이 이후 재시도와 일치할 수 없습니다.
...
- **인프라** (Codex 런타임이 사용 중, 런타임 예외, 전달되지 못한 steer) — 중단은 작업의 실패가 아니므로 실패 예산을 소비하지 않습니다.
- **제공자 용량** (재시도 가능한 업스트림 `429` 또는 `5xx`, 또는 소진된 credential pool) — 이벤트는 `open` 상태를 유지하고, epoch가 증가하며, 런타임이 위의 용량 래더에 따라 재전달합니다. Worker는 이 클래스의 모델 호출을 로컬에서 반복하지 않습니다. 복구 시각이 남은 모든 대기 시간보다 뒤에 있는 credential pool은 즉시 dead-letter가 됩니다.
- **실행** (그 밖의 재시도 가능한 실패, 예를 들어 worker 트랜스포트 실패나 타임아웃) — 이벤트는 `open` 상태를 유지하고, epoch가 증가하며, 런타임이 지수 백오프 후 재전달하고, 그 시도는 예산에 차감됩니다.
- **Dead-letter** — 5회 전달 후, 또는 실패가 재시도 불가능할 때 이벤트는 `dead_letter`로 이동하고 턴은 재시도를 멈춥니다. 사용자는 자동 재시도가 중지되었다는 알림을 받고, 운영자가 이를 검사하고 해결합니다.
