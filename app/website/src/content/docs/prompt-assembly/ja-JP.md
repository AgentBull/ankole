...
システムプロンプトはコントロールプレーンではなく**Worker 上で組み立て**られます。コントロールプレーンは永続的な Agent ドキュメント、Skill、Brain スナップショット、Agent 設定、チャットコンテキストを 2 つの経路で供給します。Worker がこれらの事実を最終プロンプトに描画します。
...
| `AgentConversationContextBroker` RPC | 永続的な Agent ドキュメント（`SOUL`/`MISSION`/`DESIGN`）、有効な Skill、Brain スナップショット（固定メモ + channel エントリ）、会話の発信 channel、インスタンスタイムゾーン、agent プロファイル | ループ開始時に worker が RuntimeFabric 上の RPC を通じて取得 |
...
`Brain.Snapshot.get_or_create/1` は会話の Brain スコープを解決し、スナップショットを返します: agent の固定メモ（`agent_context`）と channel の永続的なコンテキスト（`group_context`）。これらは保存されたコンテキストエントリ — agent が覚えるよう言われた永続的な事実 — であり、完全な Brain knowledge ベースではありません。スナップショットはクエリではなくプロジェクションです。想起（完全な検索）はループ中に Brain ツールを通じて行われ、このスナップショットを通じては行われません。
...
worker の `system_prompt.ts` が最終プロンプトを構築します。そのモジュール doc は設計を述べています:「ゆっくり変わる指示を先頭に置き、会話スコープのランタイム、Skill、Brain スナップショットをサフィックスとして形成する。」ブロックは順に:

1. **コア指示** — agent の基本挙動コントラクト。ターンのコンテキストから組み立てられます。
2. **永続的な Agent ドキュメント** — `SOUL`、`MISSION`、`DESIGN`。ブローカーの応答から描画されます。
3. **永続的なコンテキスト** — Brain スナップショットの固定メモと channel エントリ。「項目が依然として有効な場合のみ使用し、そうでなければ無視する」というガイダンス付きで描画されます。
4. **Skill** — 有効な Skill の説明。モデルが何に手を伸ばせるかを伝えます。
5. **channel とランタイムコンテキスト** — 会話の発信 channel、ワークスペースパス、利用可能なツール名。
...
- **Brain 想起結果** — 想起（完全な検索）はシステムプロンプトブロックとしてではなく、ループ中の Brain ツールを通じて行われます。スナップショットは下限であり、想起は上限です。

この分離により、システムプロンプトは安定し（ペルソナ、Skill、memory が変わるときにだけ変わり、会話が長くなっても変わりません）、ターンごとのペイロードは小さく保たれます。
...
- 永続的コンテキストブロックに供給する Brain スナップショットについては、[Brain](../brain/) を読んでください。
