defmodule Ankole.ActorRuntimeWorkerE2ETest do
  ...
  @docker_image "ankole-agent-computer:ping-pong"
  ...
  test "Docker daemon worker drives mock IM input through real OpenRouter LLM tool loop" do
    if System.get_env("ANKOLE_REAL_LLM_E2E") == "1" do
      assert_docker_image!()
      ...
      assert %AgentLibraryContainerEntry{content: content} =
               AgentLibraryContainerEntry
               |> where([entry], entry.agent_uid == ^agent.uid)
               |> where([entry], entry.path == "skills/nano-pdf/AGENT_APPEND.md")
               |> where([entry], is_nil(entry.deleted_at))
               |> Repo.one()

      assert content == "E2E overlay: ANKOLE_E2E_OK"

      assert {:ok, [%{actor_input: tombstone_input}]} =
               adapter.ingress_module.handle_message_receive(
                 "mock.message.receive",
                 %{
                   ingress_event_id: "mock-real-llm-tombstone-1",
                   signal_channel_id: "mock:chat:real-llm-e2e",
                   provider_entry_id: "mock-real-llm-tombstone-message-1",
                   provider_thread_id: "mock-real-llm-thread",
                   text: """
                   ...
                   After the command succeeds, reply exactly ANKOLE_E2E_TOMBSTONED.
                   """,
                   explicit: true,
      ...
      assert {:ok, %OutboxEntry{} = tombstone_outbox} =
               wait_for_outbox_for_input(container, tombstone_input.id, real_deadline())

      assert tombstone_outbox.payload["text"] =~ "ANKOLE_E2E_TOMBSTONED"
      ...
      refute active_append

      assert %AgentLibraryContainerEntry{content: nil, deleted_at: %DateTime{}} =
               AgentLibraryContainerEntry
               |> where([entry], entry.agent_uid == ^agent.uid)
               |> where([entry], entry.path == "skills/nano-pdf/AGENT_APPEND.md")
  ...
  defp docker_database_url do
    ...
    "postgresql://#{URI.encode_www_form(username)}:#{URI.encode_www_form(password)}@host.docker.internal:#{port}/#{URI.encode_www_form(database)}"
  end
