defmodule AnkoleWeb.LlmProviderController do
  @moduledoc """
  Console REST API for operator-managed LLM providers and agent model profiles.
  """
  ...
  alias AnkoleWeb.Schemas.ConsoleApi.LlmProviderListResponse
  alias AnkoleWeb.Schemas.ConsoleApi.LlmProviderResponse
  alias AnkoleWeb.Schemas.ConsoleApi.LlmProviderSourceListResponse
  alias AnkoleWeb.Schemas.ConsoleApi.LlmProviderWriteRequest
  ...
  operation(:sources,
    summary: "List first-party LLM provider sources",
    responses: [
      ok: {"Provider sources", "application/json", LlmProviderSourceListResponse},
  ...
  operation(:index,
    ...
    responses: [
      ok: {"LLM providers", "application/json", LlmProviderListResponse},
      unauthorized: {"Unauthorized", "application/json", ErrorEnvelope},
      forbidden: {"Forbidden", "application/json", ErrorEnvelope}
    ]
  )
  ...
  operation(:put_provider,
    summary: "Create or update one LLM provider",
    parameters: [provider_id: [in: :path, type: :string, required: true]],
    request_body: {"LLM provider", "application/json", LlmProviderWriteRequest, required: true},
    responses: [
      ok: {"LLM provider", "application/json", LlmProviderResponse},
      unauthorized: {"Unauthorized", "application/json", ErrorEnvelope},
      forbidden: {"Forbidden", "application/json", ErrorEnvelope},
  ...
  operation(:delete_provider,
    summary: "Disable one LLM provider",
    parameters: [provider_id: [in: :path, type: :string, required: true]],
    responses: [
      ok: {"LLM provider", "application/json", LlmProviderResponse},
      unauthorized: {"Unauthorized", "application/json", ErrorEnvelope},
  ...
  def sources(conn, _params) do
    with :ok <- ConsolePolicy.authorize(conn, "llm_provider_sources", "read") do
      json(conn, %{data: LlmProviders.list_provider_sources()})
    else
  ...
  def index(conn, _params) do
    with :ok <- ConsolePolicy.authorize(conn, "llm_providers", "read") do
      json(conn, %{data: LlmProviders.list_providers()})
    else
      {:error, reason} -> error(conn, reason)
    end
  end
  ...
  def put_provider(conn, params) do
    with {:ok, provider_id} <- provider_id_param(params),
         :ok <- ConsolePolicy.authorize(conn, "llm_provider:#{provider_id}", "update"),
         {:ok, attrs} <- provider_attrs(provider_id, conn.body_params),
         {:ok, provider} <- put_provider_row(provider_id, attrs) do
      json(conn, %{data: LlmProviders.projection(provider)})
    else
      {:error, reason} -> error(conn, reason)
    end
  end
  ...
  def delete_provider(conn, params) do
    with {:ok, provider_id} <- provider_id_param(params),
         :ok <- ConsolePolicy.authorize(conn, "llm_provider:#{provider_id}", "delete"),
         {:ok, provider} <- LlmProviders.delete_provider(provider_id) do
      json(conn, %{data: LlmProviders.projection(provider)})
    else
      {:error, reason} -> error(conn, reason)
    end
  ...
  defp put_provider_row(provider_id, attrs) do
    case LlmProviders.fetch_provider(provider_id) do
      {:ok, _provider} -> LlmProviders.update_provider(provider_id, attrs)
      {:error, :not_found} -> LlmProviders.create_provider(attrs)
    end
  end
  ...
  defp error(conn, reason) do
    error(conn, 422, "invalid_value", "LLM runtime configuration is invalid", [
