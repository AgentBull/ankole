defmodule Ankole.SignalsGateway.ActorRuntime.FileTransferLane do
  @moduledoc """
  Active-transfer implementation for RuntimeFabric worker-file frames.
  ...
  Root policy, route choice, and the authoritative transfer byte bound are
  owned by `Ankole.WorkerFiles`. This lane validates paths structurally and
  enforces a byte bound only as a mechanism, when the caller passes
  `:max_bytes`.
  """
  ...
  With `max_bytes: bound`, a file whose authoritative `READ_READY` size exceeds
  the bound is rejected atomically: the lane sends `READ_ABORT` instead of
  granting any byte credit and returns `{:error, {:file_too_large, size, bound}}`.
  """
  ...
  defp redact_pending(%{mode: mode, route: route, expected_command: expected_command} = pending) do
    pending
    |> Map.take([:next_sequence, :next_offset, :credit, :commit_sent?, :eof?, :max_bytes])
    |> Map.merge(%{
      mode: mode,
      route: route,
  ...
  defp dispatch_response(
    ...
    with {:ok, size} <- parse_u64(size_frame),
         ...
         :ok <- check_read_bound(transfer_id, size, pending),
         {:ok, :sent_or_queued} <-
  ...
  # Rejecting at READ_READY must still abort: the worker already holds an open
  # read transfer and only releases it on READ_ABORT (same as the timeout path).
  defp check_read_bound(transfer_id, size, %{max_bytes: max_bytes} = pending)
       when is_integer(max_bytes) and size > max_bytes do
    abort_pending_request(pending.route, transfer_id, "READ_ABORT")
    {:error, {:file_too_large, size, max_bytes}}
  end

  defp check_read_bound(_transfer_id, _size, _pending), do: :ok
  ...
  defp virtual_path(root, relative_path, opts \\ []) do
    allow_root? = Keyword.get(opts, :allow_root?, false)

    with :ok <- validate_root_segment(root),
         {:ok, relative_path} <- normalize_relative_path(relative_path, allow_root?: allow_root?) do
      if relative_path == "" do
        {:ok, "/" <> root}
      else
  ...
  # Root membership is `Ankole.WorkerFiles` policy; the lane only requires one
  # well-formed leading path segment.
  defp validate_root_segment(root)
       when is_binary(root) and root not in ["", ".", ".."] do
    if String.contains?(root, "/") do
      {:error, {:unsupported_file_root, root}}
    else
      :ok
    end
  end

  defp validate_root_segment(root), do: {:error, {:unsupported_file_root, root}}
