defmodule Ankole.ActorRuntime.SubagentDelegationBroker do
  @moduledoc """
  ...
  def handle_get(%TurnRef{} = turn_ref, request, _route) when is_map(request) do
    ...
    with {:ok, %{delegation: %Delegation{} = delegation, last_event_seq: last_event_seq}} <-
  ...
  def handle_append_events(%TurnRef{} = turn_ref, request, _route) when is_map(request) do
    ...
    with :ok <- authorize_delegation_turn(turn_ref, delegation_id),
         {:ok, appended} <-
           SubagentDelegations.append_events(delegation_id, turn_ref.agent_uid, events) do
      {:ok,
       %{
         "request_id" => request_id,
