defmodule Ankole.AIGateway.ProviderConfigs.Provider do
  @moduledoc """
  Operator-configured AIGateway provider endpoint and encrypted credential row.
  """
  ...
  `provider_kind` and `credential_mode`. The selected provider implementation
  performs the semantic validation so plugins can add provider kinds without a
  database migration.
  """
  ...
  def changeset(provider, attrs) do
    ...
    |> normalize_blank([:provider_id, :provider_kind, :base_url, :encrypted_credential])
    |> normalize_lower([:provider_id, :provider_kind, :credential_mode])
    |> validate_required([:provider_id, :provider_kind, :credential_mode, :connection_options])
    |> validate_format(:provider_id, @slug_format)
    |> validate_format(:provider_kind, @slug_format)
    |> validate_format(:credential_mode, @slug_format)
    |> validate_base_url()
    ...
    |> check_constraint(:credential_mode, name: :ai_gateway_providers_credential_mode_format)
    |> check_constraint(:base_url, name: :ai_gateway_providers_base_url_present)
    |> check_constraint(:encrypted_credential,
      name: :ai_gateway_providers_encrypted_credential_present
    )
  ...
  defp validate_credential_mode(changeset) do
    validate_change(changeset, :credential_mode, fn :credential_mode, mode ->
      ...
      case Providers.validate_credential_mode(provider_kind, mode) do
        :ok -> []
