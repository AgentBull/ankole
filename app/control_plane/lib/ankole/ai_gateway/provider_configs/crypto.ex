defmodule Ankole.AIGateway.ProviderConfigs.Crypto do
  @moduledoc """
  AEAD wrapper for AIGateway provider credentials.
  ...
  @purpose "ai_gateway_provider_credential"

  @doc """
  Encrypts a plaintext provider credential for one provider row.
  """
  @spec seal(String.t(), String.t()) :: {:ok, String.t()} | {:error, term()}
  def seal(credential, provider_row_id)
      when is_binary(credential) and is_binary(provider_row_id) do
    with {:ok, row_key} <- row_key(provider_row_id),
  ...
  def seal(_credential, _provider_row_id), do: {:error, :invalid_credential}

  @doc """
  Decrypts a provider credential that was sealed for the same provider row id.
  """
  @spec unseal(String.t(), String.t()) :: {:ok, String.t()} | {:error, term()}
  def unseal(ciphertext, provider_row_id)
      when is_binary(ciphertext) and is_binary(provider_row_id) do
    with {:ok, row_key} <- row_key(provider_row_id),
  ...
  def unseal(_ciphertext, _provider_row_id), do: {:error, :invalid_credential}

  # The row id is part of the derived key context so ciphertexts cannot be moved
  # between provider rows without becoming undecryptable.
  defp row_key(provider_row_id) do
    with {:ok, secret} <- root_secret() do
      context = "ai_gateway_providers:#{provider_row_id}:credential"
