defmodule Ankole.Brain.Jobs.SummarizeChannel do
  @moduledoc """
  Summarizes one signal channel into Brain episodes.
  """
  ...
  def perform(%Oban.Job{args: %{"signal_channel_id" => signal_channel_id}} = job)
    ...
    case Brain.summarize_channel(signal_channel_id) do
      {:error, reason} = error ->
        if final_attempt?(job) do
          Brain.skip_failed_summary_window(signal_channel_id, reason)
        else
          error
        end
