defmodule Ankole.SignalsGateway.ActorRuntime.FileTransferLaneTest do
  ...
  test "get rejects an oversize READ_READY before granting credit", %{
    route: route,
    route_auth: route_auth
  } do
    parent = self()
    ...
    :ok =
      Broker.register_local_worker(route, fn {:file_transfer_lane, frames} ->
        respond_to_oversize_read(route_auth, parent, frames)
      end)

    oversize = 200 * 1024 * 1024
    max_bytes = 100 * 1024 * 1024

    assert {:error, {:file_too_large, ^oversize, ^max_bytes}} =
             FileTransferLane.get(route, "user_files", "attachments/huge.bin",
               max_bytes: max_bytes
             )

    assert_receive {:file_lane_read_aborted, _transfer_id}, 100
    refute_received {:file_lane_unexpected_credit, _transfer_id}
  end
  ...
  test "agent_sessions root resolves and round-trips list and stat", %{
    ...
    assert {:error, {:unsupported_file_root, "shared/files"}} =
             FileTransferLane.stat(route, "shared/files", "a.txt")
  end
  ...
  defp respond_to_oversize_read(route_auth, parent, [protocol, command, transfer_id | rest]) do
    case {command, rest} do
      {"READ_OPEN", [path, _fingerprint]} ->
        FileTransferLane.handle_worker_frame(route_auth, [
          protocol,
      ...
      {"CREDIT", [_credit]} ->
        send(parent, {:file_lane_unexpected_credit, transfer_id})
