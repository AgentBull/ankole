defmodule Ankole.Repo.Migrations.InstallGeneralSchemaPack do
  ...
  #
  # Every installed value below is the frozen fact of general pack 1.1.0 as it
  ...
  @content_hash "a7f0966b9008dedd7b10cf0a221d206d"
  ...
  @agent_skills_manifest_json ~S([{"name":"agent-skills","primitive":"concept","slug_prefix":"lazyload-agent-skills/","extractable":false,"expert_routing":false}])
  ...
  @types [
    ...
    {"agent-skills", "concept", "lazyload-agent-skills/", ~S([]), false, false},
  ...
  def up do
    if setup_completed?() do
      execute("""
      WITH base AS (
        SELECT $manifest$#{@manifest_json}$manifest$::jsonb AS manifest
      )
      INSERT INTO brain_schema_packs (id, name, version, content_hash, manifest)
      SELECT '#{UUIDv7.autogenerate()}', 'general', '1.1.0', '#{@content_hash}',
             manifest || jsonb_build_object(
               'version', '1.1.0',
               'page_types',
                 ((manifest->'page_types') - 9) ||
                 $agent_skills$#{@agent_skills_manifest_json}$agent_skills$::jsonb ||
                 jsonb_build_array(manifest->'page_types'->9)
             )
      FROM base
      """)
  ...
  def down do
    raise Ecto.MigrationError,
      message: "the installed v1 Brain schema state cannot be restored to its pre-v1 state"
  end
