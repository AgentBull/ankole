...
function renderPart(part: unknown) {
  ...
  if (record.type === 'text' || record.type === 'bidiIsolation') {
    return escapeHtml(record.value)
  }

  if (record.type === 'markup') {
    return renderMarkup(record)
  }

  if (Array.isArray(record.parts)) {
    return record.parts.map(subpart => escapeHtml(isRecord(subpart) ? subpart.value : subpart)).join('')
  }

  if (record.type === 'fallback' && typeof record.source === 'string') {
    return escapeHtml(`{${record.source}}`)
  }
